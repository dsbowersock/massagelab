...
test("the DNA renderer stays a scoped, non-interactive CSS DOM effect", () => {
  ...
  assert.match(componentCode, /minimumScale:\s*DNA_OPTION_BOUNDS\.scale\.minimum/)
  ...
  assert.match(componentCode, /"--ml-dna-node-size":/)
  assert.match(componentCode, /"--ml-dna-strand-offset":/)
  ...
  assert.match(stylesheetCode, /gap:\s*0/)
  assert.match(stylesheetCode, /\.strand\s*\{[\s\S]*?translate:\s*0 var\(--ml-dna-strand-offset\)/)
  assert.match(stylesheetCode, /\.node\s*\{[\s\S]*?height:\s*var\(--ml-dna-node-size\)/)
  assert.match(stylesheetCode, /animation-direction:\s*var\(--ml-dna-rotation-direction\)/)
  ...
  assert.match(componentCode, /data-strand-rotation-disabled=\{!strandRotationEnabled \|\| undefined\}/)
  assert.match(stylesheetCode, /\[data-strand-rotation-disabled\] \.composition \{[\s\S]*?animation:\s*none/)
...
test("DNA options extend the shared background effect contract", () => {
  ...
  assert.match(dnaOptions, /strandRotationEnabled: boolean;?/)
  assert.match(dnaOptions, /strandRotationDirection: "clockwise" \| "counterclockwise";?/)
  assert.match(dnaOptions, /nodeSize: number;?/)
  ...
  assert.match(effectProps, /massageLabDna\?: MassageLabDnaHostOptions;?/)
})
