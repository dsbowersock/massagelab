...
test("the Twisted Cubes renderer stays a scoped, non-interactive CSS DOM effect", () => {
  ...
  assert.match(componentCode, /"--ml-twisted-cubes-depth-scale": getTwistedCubeDepthScale\(\{/)
  ...
  assert.match(stylesheetCode, /\.view\s*\{[^}]*?transform:\s*scale\(var\(--ml-twisted-cubes-depth-scale\)\) rotateX\(var\(--ml-twisted-cubes-view-angle-x\)\) rotateY\(var\(--ml-twisted-cubes-view-angle-y\)\);/)
