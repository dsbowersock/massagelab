...
describe("premium background registry", () => {
  ...
  it("keeps MassageLab Grid Scan source-shaped, raw WebGL, customizable, and dependency-free", () => {
    ...
    assert.match(effectSource, /const float TUNNEL_HALF_EXTENT = 0\.5;/)
    assert.match(effectSource, /float pos = mix\(-TUNNEL_HALF_EXTENT, TUNNEL_HALF_EXTENT, mod\(float\(i\), 2\.0\)\);/)
    assert.match(effectSource, /abs\(hit\.y \+ TUNNEL_HALF_EXTENT\)/)
