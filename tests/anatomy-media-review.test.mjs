...
describe("Anatomy media review helpers", () => {
  ...
  it("builds BodyParts3D composer URLs and converts pasted composer maps into image URLs", () => {
    ...
    assert.equal(descriptor?.imageWidth, 700)
    assert.equal(descriptor?.imageHeight, 700)
  ...
  it("parses BodyParts3D image source descriptors for import identity checks", () => {
    ...
    const compactUrl = bodyParts3dImageUrl({ partIds: ["FMA37670"], view: "posterior", treeName: "partof", size: 300 })
    const largeUrl = bodyParts3dImageUrl({ partIds: ["FMA37670"], view: "posterior", treeName: "partof", size: 1200 })
    const compactDescriptor = bodyParts3dSourceDescriptor(compactUrl)
    const largeDescriptor = bodyParts3dSourceDescriptor(largeUrl)
    ...
    customMapConfig.set("iw", "1200")
    customMapConfig.set("ih", "900")
    ...
    assert.equal(descriptor?.cameraParameters, null)
    assert.equal(compactDescriptor?.imageWidth, 300)
    assert.equal(compactDescriptor?.imageHeight, 300)
    assert.equal(largeDescriptor?.imageWidth, 1200)
    assert.equal(largeDescriptor?.imageHeight, 1200)
    assert.notEqual(compactDescriptor?.sourceKey, largeDescriptor?.sourceKey)
    assert.deepEqual(customDescriptor?.partIds, ["FMA37670"])
    assert.equal(customDescriptor?.treeName, "partof")
    assert.equal(customDescriptor?.imageWidth, 1200)
    assert.equal(customDescriptor?.imageHeight, 900)
