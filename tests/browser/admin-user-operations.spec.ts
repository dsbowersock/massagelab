...
test.describe("Admin user operations", () => {
  ...
  test("Admin grants and append-only revokes one bounded temporary feature with Account expiration evidence", async ({ page, browser }, testInfo) => {
    ...
    try {
      ...
      const persistedTimestamps = activeGrant.locator("time")
      await expect(persistedTimestamps).toHaveCount(2)
      const persistedStartsAt = await persistedTimestamps.nth(0).getAttribute("datetime")
      const persistedExpiresAt = await persistedTimestamps.nth(1).getAttribute("datetime")
      ...
      const accountTemporaryAccess = targetPage.getByRole("heading", { name: "Temporary feature access" }).locator("..").locator("..")
