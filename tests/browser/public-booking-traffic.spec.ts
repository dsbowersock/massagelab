...
test("availability honors Retry-After and 503 without automatic replay", async ({ page }) => {
  ...
  const availabilityStatus = page.locator('p[role="status"].sr-only')
  await expect(availabilityStatus).toHaveText("Too many availability requests. Please wait before trying again.")
  await expect(page.locator('p:not([role="status"])').filter({
    hasText: "Too many availability requests. Please wait before trying again. Try again in 3 seconds.",
  })).toBeVisible()
  await page.clock.runFor(1_000)
  await expect(availabilityStatus).toHaveText("Too many availability requests. Please wait before trying again.")
  await expect(page.locator('p:not([role="status"])').filter({
    hasText: "Too many availability requests. Please wait before trying again. Try again in 2 seconds.",
  })).toBeVisible()
  ...
  await expect(availabilityStatus).toHaveText("You can try again now.")
  await expect(retry).toBeEnabled()
  await retry.click()
  await page.clock.fastForward(350)
...
test("guest booking keeps one request identity through pending and recovery, then rotates deliberately", async ({ page }) => {
  ...
  const bookingStatus = bookingForm.getByRole("status")
  await expect(bookingStatus).toHaveText("Too many requests. Please wait before trying again.")
  await expect(bookingForm.locator('p:not([role="status"])').filter({ hasText: "Try again in 3 seconds." })).toBeVisible()
  expect(await requestIdInput.inputValue()).toBe(originalRequestId)
  await page.clock.runFor(1_000)
  await expect(bookingStatus).toHaveText("Too many requests. Please wait before trying again.")
  await expect(bookingForm.locator('p:not([role="status"])').filter({ hasText: "Try again in 2 seconds." })).toBeVisible()
  await page.clock.runFor(1_999)
  expect((await actionSnapshot(page)).calls).toHaveLength(1)
  await page.clock.runFor(1)

  await expect(bookingStatus).toHaveText("You can try again now.")
  await bookingForm.getByRole("button", { name: "Book selected time" }).click()
