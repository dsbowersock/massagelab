...
describe("Anatomy admin browser table UI", () => {
  ...
  it("separates anatomy browser tabs and exposes body-system views", async () => {
    const pageSource = await readFile(new URL("../app/admin/anatomy/page.tsx", import.meta.url), "utf8")
    const browserDataSource = await readFile(new URL("../app/admin/anatomy/browser-data.ts", import.meta.url), "utf8")
    const browserTypesSource = await readFile(new URL("../app/admin/anatomy/browser-types.ts", import.meta.url), "utf8")
    const entityViewsStart = browserTypesSource.indexOf("const ENTITY_BROWSER_VIEWS")
    const hiddenEntityViewsStart = browserTypesSource.indexOf("const SYSTEM_REDUNDANT_ENTITY_BROWSER_VIEWS")
    const bodySystemConfigsStart = browserTypesSource.indexOf("const BODY_SYSTEM_CONFIGS")
    const tissueTypeConfigsStart = browserTypesSource.indexOf("const TISSUE_TYPE_CONFIGS")
    const entityViewsSource = browserTypesSource.slice(entityViewsStart, hiddenEntityViewsStart)
    const hiddenEntityViewsSource = browserTypesSource.slice(hiddenEntityViewsStart, browserTypesSource.indexOf("const BODY_SYSTEM_BROWSER_VIEWS"))
    const bodySystemConfigsSource = browserTypesSource.slice(bodySystemConfigsStart, tissueTypeConfigsStart)
    const tissueTypeConfigsSource = browserTypesSource.slice(tissueTypeConfigsStart, browserTypesSource.indexOf("export type TissueTypeBrowserView"))
    ...
    assert.match(browserDataSource, /relationshipType: "belongs_to_system"/)
    assert.match(browserDataSource, /relationshipType: "belongs_to_tissue_type"/)
    assert.match(browserDataSource, /relationshipType: "includes_structure"/)
    assert.match(browserTypesSource, /LEGACY_BROWSER_VIEW_REDIRECTS/)
  })

  it("scopes anatomy browser data loading to the active view", async () => {
    const pageSource = await readFile(new URL("../app/admin/anatomy/page.tsx", import.meta.url), "utf8")
    const browserDataSource = await readFile(new URL("../app/admin/anatomy/browser-data.ts", import.meta.url), "utf8")
    const needsStart = browserDataSource.indexOf("function anatomyBrowserDataNeeds")
    const loaderStart = browserDataSource.indexOf("async function getAnatomyBrowserData")
    const needsSource = browserDataSource.slice(needsStart, loaderStart)
    const loaderSource = browserDataSource.slice(loaderStart)

    assert.match(pageSource, /const needsMaintenanceData = selectedView === "maintenance"/)
    ...
    assert.match(needsSource, /default:[\s\S]*add\("concepts", "structures", "muscles", "bones", "joints", "ligaments", "nerves", "bloodSupply", "entityTerms", "systemRelationships"\)/)
    assert.match(loaderSource, /needsFullEntityNameIndex\(selectedView, selectedEntity\)/)
    assert.match(loaderSource, /shouldLoad\("citations"\) \? prisma\.anatomyCitation\.findMany/)
    assert.match(loaderSource, /shouldLoad\("mediaAssets"\) \? prisma\.anatomyMediaAsset\.findMany/)
  })
  ...
  it("surfaces full citation backlog and clickable external identifier detail", async () => {
    ...
    assert.match(browserDataSource, /const ANATOMY_DETAIL_LOOKUP_TAKE = 2000/)
    assert.match(browserDataSource, /prisma\.anatomyCitation\.findMany\({[\s\S]*take: ANATOMY_DETAIL_LOOKUP_TAKE/)
    assert.match(browserDataSource, /prisma\.externalAnatomyIdentifier\.findMany\({[\s\S]*take: ANATOMY_DETAIL_LOOKUP_TAKE/)
  ...
  it("surfaces spatial review state in counts, quick queries, and entity details", async () => {
    const pageSource = await readFile(new URL("../app/admin/anatomy/page.tsx", import.meta.url), "utf8")
    const browserDataSource = await readFile(new URL("../app/admin/anatomy/browser-data.ts", import.meta.url), "utf8")
    const anatomyQueriesSource = await readFile(new URL("../lib/anatomy-queries.ts", import.meta.url), "utf8")

    assert.match(anatomyQueriesSource, /spatial-review-queue/)
    assert.match(browserDataSource, /Spatial models/)
    assert.match(browserDataSource, /Spatial maps/)
    assert.match(browserDataSource, /Movement visuals/)
    ...
    assert.match(browserDataSource, /anatomySpatialEntityMap\.findMany/)
    assert.match(browserDataSource, /anatomyMovementVisualization\.findMany/)
  })
  ...
  it("surfaces visual media review, candidate approval, and BodyParts3D import controls", async () => {
    ...
    assert.match(browserDataSource, /take: 500/)
  ...
  it("adds a mobile-first image review queue backed by link review state", async () => {
    ...
    assert.match(queueSource, /studyCategories\.length > 0 \? \{ categories: studyCategories \} : \{\}/)
    ...
    assert.match(queueSource, /preset\.filters\.status \? "" : filters\.status/)
    assert.match(queueSource, /mediaReviewQueueHref\(linkFilters, \{ preset: preset\.key, offset: 0 \}\)/)
    ...
    assert.match(queueSource, /name="preset" value=\{filters\.preset\}/)
    ...
    assert.match(actionsSource, /mediaReviewQueueOffsetAfterDecision/)
    assert.doesNotMatch(queueSource, /mediaReviewQueueOffsetAfterDecision/)
