...
describe("calendar booking settings schema and route surface", () => {
  ...
  it("renders public booking as a wizard with weekly availability time selection", () => {
    ...
    assert.match(publicBookingPage, /items-start/)
    assert.match(publicBookingPage, /pt-2/)
    assert.match(publicBookingPage, /min-\[960px\]:max-w-lg/)
    assert.match(publicBookingPage, /flex min-w-0 items-start gap-3 p-3/)
    assert.match(publicBookingPage, /CalendarDays className="mt-0\.5 size-5 shrink-0/)
    assert.match(publicBookingPage, /text-pretty leading-relaxed/)
    ...
    assert.match(bookingPicker, /min-\[960px\]:hidden/)
