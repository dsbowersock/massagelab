...
describe("calendar booking settings schema and route surface", () => {
  ...
  it("allows anonymous public booking while keeping account-required gates explicit", () => {
    ...
    assert.match(loginForm, /safePostLegalAcceptanceCallback\(searchParams\.get\("callbackUrl"\), "\/account"\)/)
    ...
    assert.match(
      loginForm,
      /fetch\("\/api\/auth\/google\/intent", \{[\s\S]*method: "POST"[\s\S]*body: JSON\.stringify\(\{ purpose: "SIGN_IN_OR_LINK", callbackUrl: googleRedirectTo \}\)/,
    )
    assert.match(loginForm, /!response\.ok \|\| !result\.ok \|\| !result\.callbackUrl/)
    assert.match(loginForm, /signIn\("google", \{ redirectTo: result\.callbackUrl \}\)/)
