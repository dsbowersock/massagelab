...
describe("admin user operations browser fixture", () => {
  ...
  it("covers the disposable Admin temporary-access journey at every configured desktop and mobile project", () => {
    ...
    assert.match(browserSpecSource, /const persistedStartsAt = await persistedTimestamps\.nth\(0\)\.getAttribute\("datetime"\)/)
    assert.match(browserSpecSource, /const persistedExpiresAt = await persistedTimestamps\.nth\(1\)\.getAttribute\("datetime"\)/)
