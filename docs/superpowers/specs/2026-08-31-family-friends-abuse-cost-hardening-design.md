...
| Anatomime room join | same limits as anonymous because a participant is not trusted before joining | 30/network/15m; 100/network/24h; 20/network+room/10m |
| Anatomime realtime token | 6/player/10m | 60/network+room/10m before participant lookup; 40/room/10m after proof |
...
A poll first passes a bounded, non-consuming `peekIngress` check with `{ networkIdentifier, roomIdentifier, now }`. An ingress denial makes no credential preflight and consumes no local counters. An allowed peek then permits at most one narrow credential preflight selecting only the candidate player ID, user mapping, guest-token hash, and room binding. It does not hydrate teams, runs, guesses, scores, or room projections.

After the preflight proves `JOINED`, exactly one `consumeJoined` call receives `networkIdentifier`, `roomIdentifier`, `playerId`, and `now`, then synchronously and atomically checks every applicable network+room, room, and player rule before mutating its instance-local map. If any rule denies, it increments none; only when all rules allow does it increment every applicable counter and permit one full room hydration. The synchronous check-then-mutate operation contains no `await`, so a single shedder instance cannot expose a partial charge between rules.

A missing or failed credential preflight consumes/checks the durable network+room unjoined quota before any full room hydration. Missing credentials may then receive the allowed public room projection; a failed credential receives generic rejoin guidance without a full hydrate. Once that durable bucket denies, repeated bogus player/token candidates stop after the narrow preflight and never reach room hydration.

`UNJOINED` and `INVALID` durable quota semantics remain unchanged: both consume/check `ANATOMIME_UNJOINED_LOOKUP` after the one narrow preflight and before any public projection or rejoin result that would require protected work. They do not call `consumeJoined`.
...
The room is hydrated once per accepted credentialed poll; the route does not hydrate, update presence, and hydrate again.
...
Every in-scope route-handler `429` response includes an integer `Retry-After` header. Booking and waitlist Server Actions return the same integer as `retryAfterSeconds` in their typed result. Native donation forms use fixed `303` pricing notices, while JSON donation clients receive the header. Client controls remain visibly pending while the accepted request is running, then show plain-language retry guidance. Controls do not spin indefinitely or silently resubmit.
