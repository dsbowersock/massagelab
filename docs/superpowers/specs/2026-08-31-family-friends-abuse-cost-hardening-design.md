...
A poll first passes a bounded, non-consuming `peekIngress` check with `{ networkIdentifier, roomIdentifier, now }`. An ingress denial makes no credential or room lookup and consumes no local counters. After an allowed peek, one room query loads the snapshot needed for both authoritative viewer classification and any eventual public summary. A pre-resolution guard classifies that same snapshot before expiration or presence writes; polling does not issue a separate narrow preflight query.
