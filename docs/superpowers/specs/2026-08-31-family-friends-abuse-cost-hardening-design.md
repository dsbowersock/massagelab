...
A poll first passes the bounded instance-local room/network shedder, then performs at most one narrow credential preflight selecting only the candidate player ID, user mapping, guest-token hash, and room binding. It does not hydrate teams, runs, guesses, scores, or room projections. A valid authenticated mapping or guest token may proceed to one full room hydration.
...
Booking and waitlist quotas are consumed before availability recomputation, contact-owner creation, calendar locks, event writes, notifications, route revalidation, or Google Calendar work. A denied or unavailable limiter creates no booking, waitlist, calendar, notification, or sync work.
...
- same booking or waitlist request returns one durable result without consuming new quota or replaying provider work;
