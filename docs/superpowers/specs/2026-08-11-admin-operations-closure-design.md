...
| `qa` | `trusted_identity`, `orphan_scan`, `parent_data_safety`, `inherited_data_safety`, `migration_status`, `migration_deploy`, `fixture_provenance`, `browser_desktop`, `browser_mobile`, `fixture_cleanup`, `branch_deleted` | `DEPLOYED`, bound provenance, both browser projects, cleanup and absence |
...
- An async normalize-and-fingerprint preparation owner rejects unknown own keys, including `cursor` and `queryFingerprint`, then returns an opaque `PreparedAdminDirectoryNonCursorQuery` registered in a module-private runtime registry. The canonical URL builder accepts only that verified handle plus exactly one required `ParsedAdminUserCursor | null`; a full `AdminDirectoryNavigationQuery` variable cannot compile against the brand, and a cast, plain/cloned object, or JavaScript full object fails runtime validation. Thus the builder has exactly one cursor source.
