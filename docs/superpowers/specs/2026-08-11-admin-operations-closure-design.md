...
`receipt_lease_capture`, `server_ownership`, and `qa_sentinel` are three separate non-substitutable opaque-owner proof kinds, respectively produced only by `captureReceiptLease`, `recordServerOwnership`, and `recordQaSentinel`; a generic control-plane, command, status, cleanup, fixture, browser, or other lifecycle proof cannot stand in for any of them. Both browser envelopes require byte-identical references/digests for the capture proof, `fixture_state`, `smtp_isolation`, `billing_mutation_guard_armed`, server-ownership proof, and sentinel proof, plus the appropriate viewport-specific fields. Signing, serialization, QA acceptance-attestation creation/verification, and browser-evidence validation reject omission, substitution, field/reference swaps, unequal viewport references, and forged/cloned/stale/cross-target/cross-run/wrong-order/wrong-tree/wrong-build/wrong-sentinel proofs. Rehearsal attestation signing/verification rejects all three QA-only lifecycle kinds.
...
QA order is exact: parent gates -> `receipt_lease_capture` -> child identity/safety -> migration/status -> provenance -> fixture load/state -> SMTP/billing guards -> `server_ownership` -> `qa_sentinel` -> desktop/mobile -> zero mutation -> cleanup/deletion/absence. Capture binds parent/child targets, immutable receipt, lease, run ID, and creation order. Ownership binds launch/process tree/current build manifest; sentinel binds that live server to QA target/run. Both viewports bind identical receipt-capture, fixture-state, SMTP-isolation, billing-guard, server-ownership, and sentinel refs/digests. Fixture-row cleanup requires confirmed tree safety; deletion/absence remain unconditional after capture.

The QA owner records parent gates, `receipt_lease_capture`, then child identity. After state and guards it records `server_ownership` and `qa_sentinel` before browsers; viewport envelopes bind identical receipt-capture/fixture-state/SMTP-isolation/billing-guard/server-ownership/sentinel refs and reject omitted, substituted, stale, forged, cloned, cross-run, or mismatched proofs.
...
2. Immediately after creation record `receipt_lease_capture`, then child identity/safety, migration/status, and provenance. Continue exactly `fixture load -> fixture_state -> smtp_isolation -> billing_mutation_guard_armed -> server_ownership -> qa_sentinel -> browser_desktop -> browser_mobile -> billing_zero_mutation -> process-tree exit -> scratch_cleanup -> fixture_cleanup -> branch_deleted -> trusted absence`. Each browser binds identical receipt-capture/fixture-state/SMTP/billing-guard/server-ownership/sentinel refs. Missing, substituted, forged, cloned, stale, cross-run, or mismatched proof prevents browser work.
