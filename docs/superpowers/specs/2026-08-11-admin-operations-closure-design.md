...
- After branch creation, one `try/finally`-equivalent QA owner always attempts independent bounded teardown after every guard/proof/browser/process-tree/scratch/zero-mutation outcome: fixture cleanup when process-tree safety permits, exact branch deletion authorized only by the immutable creation receipt/run lease, and trusted absence verification after the deletion attempt. An unconfirmed process-tree exit may block unsafe row cleanup but cannot suppress receipt-authorized branch deletion/absence. Every reached PASS/FAIL and cleanup edge remains truthful attempt evidence; any required cleanup/deletion/absence failure blocks acceptance. A table-driven failure matrix covers every gate, both browsers, process-tree join, zero-mutation, and scratch cleanup.
