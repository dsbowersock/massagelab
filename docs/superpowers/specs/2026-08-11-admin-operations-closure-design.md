...
9. The canonical rehearsal evidence sequence is exactly `trusted_identity -> orphan_scan -> parent_data_safety -> inherited_data_safety -> migration_status -> migration_deploy -> rehearsal_post_status -> prisma_validate -> prisma_generate -> focused_tests -> typecheck -> lint -> unit_tests -> build -> rehearsal_read_only_smoke -> scratch_cleanup -> branch_deleted`. `rehearsal_post_status` is one complete `UP_TO_DATE` result with all four migrations `APPLIED`, bound to the rehearsal target/commit/run; generic aliases reject. Every child environment starts empty and allowlists exact keys. Status/deploy and only individually named tests/smoke proven to perform database I/O may receive database credentials. Validate receives only fixed nonsecret syntactically valid placeholder URL values and performs no network I/O; generate/typecheck/lint/build receive no database credentials and only explicit nonsecret run/scratch/manifest variables. Parent environment remains clean. The tracked cross-platform `tests/fixtures/admin-operations-env-capture.mjs <commandName>` fixture and denylist tests cover every command and reject inherited database/provider/auth/email secrets, real target bytes in validate, or undeclared keys. Every timeout/cancellation terminates and boundedly awaits the complete owned process tree. Inability to confirm descendant exit is blocking and prevents unsafe scratch/fixture-row cleanup, but the `finally` owner still independently uses the immutable receipt/lease to delete the exact branch and verify absence. Only passing cleanup proofs permit attestation.
...
`receipt_lease_capture`, `server_ownership`, and `qa_sentinel` are three separate non-substitutable opaque-owner proof kinds, respectively produced only by `captureReceiptLeaseAuthority`, `recordServerOwnership`, and `recordQaSentinel`; a generic control-plane, command, status, cleanup, fixture, browser, renewable verification, or other lifecycle proof cannot stand in for any of them. The capture check binds a stable authority digest. Both browser envelopes require the same byte-identical separately named `receiptLeaseVerificationRef`, bound to that stable digest, plus byte-identical references/digests for `fixture_state`, `smtp_isolation`, `billing_mutation_guard_armed`, server ownership, and sentinel, plus appropriate viewport-specific fields. Signing, serialization, QA acceptance-attestation creation/verification, and browser-evidence validation reject omission, substitution, field/reference swaps, unequal viewport references, expired renewal, and forged/cloned/replayed/cross-target/cross-run/wrong-order/wrong-tree/wrong-build/wrong-sentinel proofs. Rehearsal attestation signing/verification rejects all three QA-only lifecycle kinds and the renewal ref.
...
`lib/admin/admin-operations-qa-lifecycle-proofs.ts` alone owns opaque `captureReceiptLeaseAuthority`/`verifyReceiptLeaseAuthority`, `renewReceiptLeaseVerification`/`verifyReceiptLeaseVerification`, `recordServerOwnership`/`verifyServerOwnership`, and `recordQaSentinel`/`verifyQaSentinel` APIs using distinct private classes plus `WeakSet` registries with no public brand/constructor/register/unwrap. The stable canonical-v1 capture-authority digest binds parent/child targets, immutable creation receipt/lease, run, and creation sequence; it never expires or changes, and ownership binds it plus launch receipt, PID/tree, origin, commit/run, and build-manifest digest. Renewal accepts only the registered original authority, freshly verifies through the trusted control plane that target/receipt/lease/run/sequence are unchanged, and emits an owner-nonce-bearing opaque envelope bound to the stable digest and one QA completion. `QA_RECEIPT_VERIFICATION_MAX_AGE_MS = 300000`. After ownership and sentinel verification and immediately before `browser_desktop`, issue one renewed ref shared byte-identically by desktop/mobile, consume it only for those two named envelopes and final QA-completion verification, then seal it consumed, all before expiry. Sentinel remains bound to ownership, target/run/build, nonce/response digest, and timestamp. Fake-clock, long-run, renewal, and replay tests prove stable-authority continuity, old-envelope expiry, successful fresh renewal, and rejection of arbitrary inputs, changed lease/target/run/order, chained-envelope substitution, clock rollback, expiry between viewports or before completion, third use, cross-completion replay, and forged/cloned/mismatched owner evidence.

Each child begins with an empty environment. Exact allowlists are: status/post-status `{BOOT}, ADMIN_OPERATIONS_RUN_ID, ADMIN_OPERATIONS_MIGRATION_DIRECT_URL, DATABASE_URL, DIRECT_URL, ADMIN_OPERATIONS_TARGET_BINDING, ADMIN_OPERATIONS_SCRATCH_ROOT, ADMIN_OPERATIONS_INPUT_MANIFEST`; deploy adds only final attestation digest when applicable; validate gets bootstrap/run/scratch/input-manifest plus fixed nonsecret `DATABASE_URL=postgresql://validator:validator@127.0.0.1:1/validator` and identical `DIRECT_URL`, with no target binding and no DNS/connection/network I/O; generate has only bootstrap/run/scratch/input-manifest/generate-output; focused/unit tests get bootstrap/run/scratch/input-manifest and DB URL pair/target only in the named DB-I/O subprocess; smoke gets the same plus DB keys because it runs read-only queries; browser gets disposable DB pair/target, run, stable capture digest, renewed `receiptLeaseVerificationRef`, build/fixture/SMTP/billing-guard/server/sentinel refs, SMTP blanking and QA opt-in; typecheck/lint get bootstrap/run/scratch/input/generated manifest only; build gets bootstrap/run/scratch/build-workspace/input/dependency manifests and build flag only. Windows bootstrap is only `SystemRoot,WINDIR,ComSpec,PATHEXT,TEMP,TMP`; POSIX only `HOME,TMPDIR,LANG,LC_ALL`, each retained only when executable tests prove required. Everything else is denied. The tracked cross-platform `tests/fixtures/admin-operations-env-capture.mjs <commandName>` child runs once for each exact command name `status`, `post-status`, `deploy`, `validate`, `generate`, `focused-tests`, `unit-tests`, `smoke`, `browser`, `typecheck`, `lint`, and `build`; tests assert exact sorted maps, parent cleanliness, placeholder-only validate values, no real target bytes in validate, and rejection of each injected DB/provider/auth/email/system key.
...
The canonical QA evidence sequence is exactly `parent_trusted_identity -> orphan_scan -> parent_data_safety -> receipt_lease_capture -> qa_trusted_identity -> inherited_data_safety -> migration_status -> migration_deploy -> qa_post_status -> fixture_provenance -> fixture_state -> smtp_isolation -> billing_mutation_guard_armed -> server_ownership -> qa_sentinel -> browser_desktop -> browser_mobile -> billing_zero_mutation -> scratch_cleanup -> fixture_cleanup -> branch_deleted`. Operational fixture loading occurs between `fixture_provenance` and `fixture_state`, full process-tree exit is confirmed before `scratch_cleanup`, and trusted absence verification completes the `branch_deleted` proof; those operations do not add or rename matrix checks. Capture authority binds parent/child targets, immutable receipt, lease, run ID, and creation order. Ownership binds launch/process tree/current build manifest; sentinel binds that live server to QA target/run. Both viewports bind the same renewed receipt verification plus identical fixture-state, SMTP-isolation, billing-guard, server-ownership, and sentinel refs/digests. Fixture-row cleanup requires confirmed tree safety; deletion/absence remain unconditional after capture.

The QA owner follows the canonical QA evidence sequence above. After the long-running setup it records/verifies `server_ownership` and `qa_sentinel`, then immediately before `browser_desktop` renews the stable capture authority; viewport envelopes bind the same renewed receipt-verification ref and identical fixture-state/SMTP-isolation/billing-guard/server-ownership/sentinel refs and reject omitted, substituted, expired, forged, cloned, cross-run, or mismatched proofs.
...
2. Immediately after creation record `receipt_lease_capture`, then follow the canonical QA evidence sequence above without aliases or omitted checks. Fixture load, process-tree exit, and trusted absence occur at the operational boundaries stated above but do not alter that evidence sequence. Each browser binds the same renewed receipt-verification ref and identical fixture-state/SMTP/billing-guard/server-ownership/sentinel refs. Missing, substituted, forged, cloned, expired, cross-run, or mismatched proof prevents browser work.
