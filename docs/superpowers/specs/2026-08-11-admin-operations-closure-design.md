...
Each sealed execution workspace has a separate pre-seal run-owned writable scratch root with an exclusive random path and restrictive permissions. Generated Prisma output, `.next`/build output, test/browser results, traces, screenshots, videos, reports, caches, temp files, and server logs are redirected explicitly by child environment/config/arguments into allowlisted scratch subdirectories. Scratch is never in `cwd`, module resolution, import/config/spec/schema/server/browser executable paths, or the sealed manifest; runtime resolution/open/spawn guards reject loading executable/module/config/fixture input from it and permit only declared output writes. For the real pinned `prisma generate` child, snapshot construction creates and manifests a command-specific schema/config inside the workspace before sealing; that sealed input uses Prisma's supported generator-output configuration to name only the run-owned scratch generation directory. The child receives that sealed schema/config by absolute argument, and tests execute the real pinned generator while asserting generated bytes exist only in scratch and no sealed path is written. Byte/file/time bounds cancel the owned tree on exhaustion. Scratch remains until complete tree exit; teardown then attempts bounded verified scratch deletion before recording completion. Scratch deletion failure blocks completion evidence but never delays advisory unlock/connection close or suppresses independently mandatory fixture cleanup, receipt-authorized branch deletion, and trusted absence verification after confirmed process-tree exit. Tests prove sealed writes fail, outputs land only in scratch, injected scratch modules cannot resolve, bounds cancel safely, deletion failure is recorded, and external teardown still runs on every exit.
...
Proof-kind assignment is exact: identity/orphan/locked-refetch use control-plane proof; `migration_status`, distinct `rehearsal_post_status`, `qa_post_status`, `production_post_status`, and fingerprint match use non-substitutable status/fingerprint proofs; deploy/validate/generate/test/typecheck/lint/build use command proof; safety/provenance use opaque-owner proof; authorization uses authorization proof; `scratch_cleanup`, fixture cleanup, and branch deletion use distinct cleanup/absence proofs; and smoke uses the canonical envelope above. `scratch_cleanup` binds the run, workspace manifest, scratch identity, confirmed full-tree exit, bounded deletion attempt, and verified absence without serializing a path; rehearsal and QA completion require it after their last functional check and before external cleanup/deletion. A `FAIL` remains truthful attempt evidence while fixture/branch teardown continues. Production audit/status/final do not own an aggregate mode-level scratch: each sealed repository-CLI invocation owns a separate scratch, and its status/deploy command proof cannot pass unless that invocation records confirmed tree exit plus bounded deletion/absence. Therefore final has no separate `scratch_cleanup` matrix entry, and a cleanup failure blocks its command/evidence while unlock/close still runs. Before either browser project starts, `smtp_isolation` proves the isolated SMTP server/environment and `billing_mutation_guard_armed` proves instrumentation is armed to count forms, POSTs, and provider mutations. `browser_desktop`/`browser_mobile` each require a browser proof envelope binding QA target, commit, QA run/lease, configured Playwright project and viewport, exact spec/config digests, result, and references to those two already-passing guard proofs. Only after both browser trees exit does `billing_zero_mutation` assert the same instrumented run observed zero forms, POSTs, and provider mutations; its envelope binds both browser proof digests. Missing, swapped, cross-viewport/project/run, substituted, or prematurely asserted browser/guard proofs reject.
...
QA target semantics/order are exact: `parent_trusted_identity` binds only the parent; after creation, fresh `qa_trusted_identity` gates child database connections, inherited-data queries, fixture row mutations, and preview construction. It does not gate teardown. Exact branch deletion and trusted absence verification use the immutable creation receipt (project/branch ID/name/run suffix) plus run-owned lease, run bounded in `finally`, and are attempted even when child lookup is missing, times out, or mismatches; ambiguity blocks completion but never suppresses the cleanup attempt. Success order is parent identity, orphan scan, parent safety, create/receipt-and-lease, child identity, inherited safety, migration status/deploy, `qa_post_status`, provenance, `smtp_isolation`, `billing_mutation_guard_armed`, desktop browser, mobile browser, post-run `billing_zero_mutation`, `scratch_cleanup`, fixture cleanup, and receipt-authorized branch deletion/trusted absence. `scratch_cleanup: FAIL` blocks completion but remains truthful attempt evidence and does not suppress external cleanup.

The QA owner records `parent_trusted_identity -> orphan_scan -> parent_data_safety`, creates the branch and seals its creation receipt/lease, then performs fresh authenticated child lookup before any child database connection or mutation. Child rejection stops inherited proof, migration, fixture, preview, server, and browser work, while bounded receipt-authorized deletion/absence still runs. Immediately after deploy, `qa_post_status` proves complete `UP_TO_DATE` with all four `APPLIED`; passing provenance and both guard checks precede browser proofs. Browser project/viewport envelopes cannot substitute for each other or for guards.
...
2. Immediately after creation, prove inherited rows absent or approved-sanitized before migrations or fixtures. Then apply migrations and verify opaque `fixture_provenance` binding approved source/version/digest to the exact QA branch/database and clean commit. The exact continuation is provenance -> `smtp_isolation` -> `billing_mutation_guard_armed` -> fixture load/server -> `browser_desktop` -> `browser_mobile` -> post-run `billing_zero_mutation` -> `scratch_cleanup` -> fixture cleanup -> receipt-authorized branch deletion/trusted absence. Missing, forged/cloned, stale, wrong target/database/commit/version/digest, or row-bearing proof prevents load/server/browser while bounded external cleanup still runs. Safe serialization retains only version/digest/outcome/timestamp and bindings.
