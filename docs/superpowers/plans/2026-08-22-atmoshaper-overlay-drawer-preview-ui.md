...
**Goal:** Replace the fixed Current Mix column/bottom tray with one opposite-sidebar rail that widens inward as a continuous control surface, add one temporary sound-preview path that can promote smoothly into the committed mix, and make the Sound Library visually cohesive with the Atmosphere station experience.

**Architecture:** `MusicProvider` remains the sole global audio owner. The existing AtmoShaper controller gains one ephemeral preview slot alongside committed recipe handles; the provider coordinates preview-only ownership without publishing a global player or Media Session until the preview is promoted into a real mix. The UI uses the existing settings owner to derive the opposite rail side and one continuously mounted fixed-edge control tree whose width and focus behavior adapt to measured workspace geometry, plus focused AtmoShaper components for sortable rows, generated noise artwork, and library-source policies.
...
- **Approved requirements:** one opposite-sidebar full-height rail that expands inward in place; overlay panel on every geometry; modeless roomy behavior; focus-contained narrow behavior; first addition opens the panel; later additions do not; stable Mix/transport/volume/layer anchors; collapsed and expanded Mute/Solo controls; drag-handle reorder; Preview before Add; cohesive glow tabs/art/buttons.
...
- [x] **Step 8:** Reserve preview status, labeled preview-volume control, failure text/Retry, and Stop Preview inside the matching source card so preview state never shifts the category rail or card grid. Starting another Preview replaces the first through the provider.
...
- [x] **Step 11:** Add fluid card-grid, card-local preview, transparent glow-rail, backdrop-art, loading-width, contrast, and reduced-motion CSS without named-device selectors.
...
- [x] **Step 3:** Measure the workspace with `ResizeObserver`, consume `useSettings().settings.sidebarPosition`, and control one fixed-edge rail. Use modeless outside-click collapse for roomy mode; use a subtle backdrop plus focus containment for narrow mode. Place the rail opposite the app sidebar. The expected browser matrix is narrow at 375x667, 412x915, and 844x390, and roomy at 768x1024, 912x1368, 1440x900, and 2560x1440 when the measured workspace clears both thresholds.
- [x] **Step 4:** Create one continuously mounted full-height screen-edge rail with a Mix toggle, one master Play/Stop toggle, a compact whole-mix Volume opener, and ordered layer tiles. Expansion animates that same rail's width inward from the fixed outer edge; Mix, transport, volume, and layer controls retain DOM identity and vertical anchors while their slots stretch into the expanded layout. Play starts the committed recipe; the destructive Stop state retires both the committed mix and any active preview. Each square collapsed tile uses a full-width source label to open/focus its layer, followed by equal, centered Mute/Unmute and transient Solo/Unsolo icon actions plus loading/muted/failed state. Solo remains available in the expanded row, survives the layout change, and snapshots/restores the exact prior mute pattern through existing recipe actions; preview never appears as a rail layer.
...
- [x] **Step 8:** Make `CurrentMix` focus/scroll the active layer after panel mount, order its Mix toggle, Play/Stop, whole-mix volume, and layer rows to match the collapsed rail, use one forest-green Play/destructive Stop toggle, label whole-mix volume, align destructive Remove and the reorder handle with the layer name, place Mute and Solo beside layer volume, and preserve retry semantics. Expanded binaural and isochronic rows render the existing live `BrainwaveLayerControls`, mapping `carrierHz` plus `beatHz` or `pulseHz` into `actions.updateLayer` (and retained-row restoration when required). Station and noise rows keep only the generic controls because they have no other supported source settings.
- [x] **Step 9:** Record the exact connected `HTMLElement` that requested each open: primary rail toggle, whole-mix Volume, individual rail layer tile, existing-source selection, or first-add discovery. On collapse, restore focus to that still-mounted control when appropriate, with the primary Mix toggle as the final fallback.
- [x] **Step 10:** Replace the two-column/tray CSS with a full-width library plus one fixed rail whose width expands at the same edge. Bound roomy width, make narrow width nearly full content width, close the roomy modeless rail from outside-library interaction, reserve player/app-bar/sidebar/safe areas, and prevent document overflow. Remove the old `.ml-atmoshaper-current-mix-desktop`, `.ml-atmoshaper-mix-tray`, tray transport, and bottom-sheet rules.
...
- [x] **Step 14:** Commit the verified implementation, acceptance, and docs slice as `fix(atmoshaper): refine mixer workspace`.
