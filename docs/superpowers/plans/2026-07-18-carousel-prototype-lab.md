...
> Post-implementation visual review on 2026-07-20 superseded the first-pass geometry, centered-only video rule, and `massagelab-carousel-lab-v1` defaults. The correction contract is recorded in the design specification's visual-review addendum and in `docs/carousel-sources.md`; the task steps below remain the historical execution record.
