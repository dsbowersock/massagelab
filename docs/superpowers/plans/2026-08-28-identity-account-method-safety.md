...
Leave the current `AuthAttempt` model exactly as deployed for compatibility with pre-bridge instances during expansion and drain. After bridge cutover, those instances are not rollback targets:
...
Describe `20260828120000_identity_method_safety` truthfully as an expansion migration: it creates `AuthRateLimitBucket` while preserving `AuthAttempt` for pre-bridge instances through their bounded drain and for schema compatibility during the bridge-capable rollback window. Document that Task 2 atomically changes the code contract, but pre-bridge instances may still write `AuthAttempt` until the paused bridge deployment is serving and the configured-maximum-invocation-lifetime drain proves them gone; only then do raw legacy identifiers stop growing and `AuthAttempt` cease to be active limiter state. After that bridge cutover, rollback is only to bridge-capable code with membership writes paused exactly `1`, never to a pre-bridge writer. Document read-only fingerprinting and `npm run auth:cleanup-legacy-attempts -- --expected-fingerprint=<64 lowercase hex> --max-rows=<1..100>` as later release actions that may run only after deployment/cutover/drain evidence and separate authorization for the exact production fingerprint and mutation scope; record counts only, and do not imply cleanup or a future table-drop contract migration occurred.
