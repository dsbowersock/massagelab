...
The browser owner builds one complete sealed execution workspace containing the verified Node runtime; package closure and installed Chromium; Playwright config, Admin spec and fixtures; browser-acceptance/fixture scripts; the full application/server source and generated runtime; required Next config, `.next` server/static output, `public` assets, and every build-traced server dependency. It launches `<sealedRoot>/runtime/node <sealedRoot>/workspace/node_modules/@playwright/test/cli.js` with `cwd=<sealedRoot>/workspace` and absolute snapshot-local arguments `test <sealedRoot>/workspace/tests/browser/admin-user-operations.spec.ts --config=<sealedRoot>/workspace/playwright.config.ts --project=desktop-chromium --project=mobile-chromium`. The Playwright-owned server executable, cwd, config, app root, static/public roots, and every environment path are snapshot-local; the environment clears checkout/module/config fallbacks. Resolution/open/spawn guards reject any path outside the workspace except trusted OS libraries. In `finally`, terminate and await the complete Playwright/server tree, release the seal, and attempt bounded verified scratch/workspace deletion before recording completion. Whether that deletion succeeds or fails, continue independently mandatory fixture cleanup and receipt-authorized exact branch deletion/trusted absence verification; deletion failure makes the run non-completing but cannot strand external QA data. If descendant exit cannot be confirmed, do not delete mutable external state automatically and fail closed under orphan-recovery rules. Hard-termination recovery retains the established prefix-scan and verified-owner/lease/staleness rules.

The planned full-acceptance isolation test creates a disposable mirror of the exact clean checkout solely as snapshot source, builds/seals the complete workspace, then renames or denies the mirror's `node_modules`, `app`, `lib`, `scripts`, `tests`, `prisma`, configs, `.next`, `public`, and generated outputs before the browser child is consumed. It runs the same desktop/mobile Admin acceptance command and requires both projects, Playwright-owned server, bounded verified scratch/workspace deletion, fixtures, receipt-authorized branch deletion, and absence checks to pass from the sealed root. A deletion-failure injection must block completion while still proving fixture cleanup and branch absence. Resolution/open/spawn telemetry must contain no mirror or original-checkout path. A control intentionally omits one build-traced server artifact and must fail before server spawn, proving the success is not a checkout fallback. The test restores/removes only its disposable mirror after the sealed process tree exits; it never renames the user's worktree.
