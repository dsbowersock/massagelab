...
- Create `lib/admin/admin-operations-data-safety.ts`: server-only owner of the versioned canonical PHI-bearing table/workflow scope and approved authenticated compliance query. It alone returns opaque module-branded parent or inherited data-safety evidence; caller objects, claimed digests, fixture provenance, and raw query results cannot create evidence.
...
export type TrustedAdminOperationsDataSafetyEvidence = object // opaque; created only by the server-only policy/query owner
...
export function verifyTrustedAdminOperationsDataSafetyEvidence(input: {
  ...
  target: { projectId: string; branchId: string; databaseName: string }
  policyVersion: ApprovedAdminOperationsDataSafetyPolicyVersion
  checkedAt: string
  outcome: "PASS"
  aggregateScopeOutcomeDigest: string
}
...
Drive the data-safety owner only through an injected approved authenticated compliance source. `ApprovedAuthenticatedComplianceSource` carries a module-private opaque brand created only by that authenticated source owner; callers cannot construct it. `ApprovedAdminOperationsDataSafetyPolicyVersion` is the exact branded literal union `"admin-operations-rehearsal-no-phi-v1" | "admin-operations-qa-full-schema-v1"`; each policy owns its scope plus immutable exported `ADMIN_OPERATIONS_DATA_SAFETY_MAX_AGE_MS = 300000`. The rehearsal `NO_PHI` policy covers the canonical PHI-bearing workflow/table scope. The QA `DATA_FREE`/`APPROVED_SANITIZED` policy covers every application-schema table and relation, including users, emails/PII, billing, and Admin evidence; its sanitized alternative enumerates the only allowed residual aggregate classes across that full scope. Callers cannot choose policy, supply/narrow scope, residual classes, or freshness. The verifier accepts evaluation `now` only and derives policy from purpose: rehearsal purposes require `NO_PHI`; QA purposes require full-schema `DATA_FREE` or `APPROVED_SANITIZED`. Evidence binds purpose, trusted target, policy, finite in-bound `checkedAt`, complete aggregate/table-scope outcomes, `PASS`, and owner digest without rows/values/PHI/PII/secrets. Reject forged/plain source or policy, caller digest, stale/wrong target/purpose/policy, incomplete scope, unallowlisted residual class, non-`PASS`, and row-bearing evidence. If a compatibility wrapper accepts max age, reject negative, zero, nonfinite, or over-bound values. Assert authenticated injected-source success for all purposes. Add a QA fixture containing a non-PHI Production-derived `User`/email or billing row and prove QA evidence fails despite the row being outside the rehearsal PHI scope. Wrappers verify brand, purpose-derived policy, target, freshness, full scope, residual-class allowlist, and `PASS` before create/work. Serializer tests retain only approved digest, aggregate outcome, policy version, and timestamp.
...
| `qa` | `trusted_identity`, `orphan_scan`, `parent_data_safety`, `inherited_data_safety`, `migration_status`, `migration_deploy`, `browser_desktop`, `browser_mobile`, `fixture_cleanup`, `branch_deleted` | `DEPLOYED`; both browser projects, exact cleanup, and verified branch absence |
