...
```bash
git add lib/admin/operations-activation-contract.ts lib/admin/neon-control-plane.ts lib/admin/admin-operations-data-safety.ts lib/admin/admin-operations-fixture-provenance.ts lib/admin/admin-operations-fixture-state.ts tests/admin-operations-activation.test.mjs
...
Assert QA order `parentTrustedIdentity -> orphanScan -> parentDataSafety -> createBranch/creationReceiptLease -> qaTrustedIdentity -> inheritedDataSafety -> migrationStatus -> migrationDeploy -> qaPostStatus -> fixtureProvenance -> fixtureLoad -> fixtureState -> smtpIsolation -> billingMutationGuardArmed -> browserServer -> browserDesktop -> browserMobile -> billingZeroMutation -> scratchCleanup -> fixtureCleanup -> receiptAuthorizedDelete -> trustedAbsence`. Child identity gates database connection, fixture row mutation, and preview/server/browser work, not control-plane cleanup. `fixtureState` is the QA-only sanitized aggregate digest, and both browser envelopes must bind its identical proof ref/digest plus the SMTP/armed-guard proofs. `scratchCleanup: FAIL` is retained truthfully and never suppresses external teardown. Assert the same behavior for rehearsal's cleanup suffix.
...
The authoritative QA evidence order is `parent_trusted_identity -> orphan_scan -> parent_data_safety -> creation receipt/lease -> qa_trusted_identity -> inherited_data_safety -> migration_status -> migration_deploy -> qa_post_status -> fixture_provenance -> fixture load -> fixture_state -> smtp_isolation -> billing_mutation_guard_armed -> browser_desktop -> browser_mobile -> billing_zero_mutation -> scratch_cleanup -> fixture_cleanup -> branch_deleted -> trusted absence`. `fixture_state` is a QA-only canonical sanitized aggregate digest bound to exact QA target/commit/run/lease and provenance; it exposes no rows/PHI. Both browser envelopes bind its same proof ref/digest plus the SMTP/armed-guard references. Child identity authorizes only child database work, not teardown. Valid receipt/lease deletion and absence still run after any later failure; wrong/missing capture permits no unauthorized row cleanup or branch deletion and blocks acceptance.

In the execution narrative below, browser work expands to provenance, fixture load, canonical `fixture_state`, SMTP isolation, armed billing guard, desktop/mobile proofs bound to the same fixture-state ref, post-run zero mutation, scratch cleanup, fixture cleanup, exact deletion, and trusted absence. It permits no omitted or substituted proof.

Execute QA in the authoritative order. After passing provenance, load fixtures, compute/verify `fixture_state`, then establish SMTP and billing guards before server/browser proofs. Both browser proofs bind the identical fixture-state digest. Fixture-row cleanup runs only after positively confirmed full process-tree exit/safety; otherwise record it truthfully as `SKIPPED`/`BLOCKED` without row mutation. Missing receipt/lease capture authorizes neither cleanup nor deletion. Once capture succeeds, exact branch deletion and trusted absence remain unconditional after every failure; fixture-cleanup failure cannot suppress them and deletion failure cannot suppress absence. Every failure/throw/timeout blocks acceptance. Never reuse rehearsal; proofs expose no rows, PHI, or secrets. Invoke the pinned owner:
