...
| `qa` | `trusted_identity`, `orphan_scan`, `parent_data_safety`, `inherited_data_safety`, `migration_status`, `migration_deploy`, `fixture_provenance`, `browser_desktop`, `browser_mobile`, `fixture_cleanup`, `branch_deleted` | `DEPLOYED`; approved bound fixture provenance, both browser projects, exact cleanup, and verified branch absence |
...
With fully injected rehearsal and QA owners, assert exact success order starts `parentDataSafety -> createBranch -> inheritedDataSafety`; rehearsal then runs status/deploy/validate/generate/focused/full/smoke, while QA runs status/deploy/`fixtureProvenance -> fixtureLoad -> browserServer -> browser`, followed by cleanup, exact deletion, and trusted absence verification. Table-drive parent and inherited failures as above. Additionally, missing/forged/cloned/stale/wrong target/database/commit/version/digest/row-bearing provenance prevents fixture load, server, and browser but still runs cleanup, exact deletion, and absence verification. Synthetic fixture success cannot substitute for either safety or provenance proof.
...
Assert the Production smoke owner cannot acquire, inherit, retain, delay release of, or execute under the migration advisory-lock connection. Deploy plus verified post-status consumes the nonce and releases the lock, but final evidence remains partial until the separate `production_read_only_smoke: PASS`; smoke failure records truthful failed attempt evidence without reopening the deploy lock.
...
After the exact QA parent has passed the pre-creation data-free/approved-sanitized gate, create a second disposable branch named `admin-operations-qa-YYYYMMDD-HHMMSS-<runSuffix>`; never reuse the rehearsal branch. Generate at least 128 random bits for the suffix, check the exact candidate in the trusted control plane, retry collisions within a small bound, and bind exact name/suffix/returned branch ID to the verified run-owner lease. Immediately after creation, independently verify inherited rows are absent or approved-sanitized before migrations, tests, browser server, or fixtures; fixture loading cannot satisfy this gate. Sanitized proof exposes no rows, PHI, or secrets, and ambiguity triggers deletion/absence verification. Before loading fixtures, require `lib/admin/admin-operations-fixture-provenance.ts` to verify its opaque proof against the immutable approved source/version allowlist, owner-controlled five-minute freshness bound, exact QA target/database, and clean commit. Its source, proof, and verified result use separate module-private runtime registries; plain/forged/cloned/cast or row-bearing objects cannot pass, and serialization retains only safe version/digest/outcome/timestamp and bindings. Wrap provisioning, approved synthetic/sanitized fixture setup, test execution, exact fixture cleanup, and branch deletion in `try/finally` for graceful exits. Apply all migrations through the verified repository-local Prisma owner, set the operator-expected four-field binding and opt-in only for the Playwright child, require the independently authenticated Neon lookup to return the same binding, blank SMTP, and invoke the pinned browser owner:
