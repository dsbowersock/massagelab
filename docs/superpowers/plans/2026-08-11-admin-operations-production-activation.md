...
- Create `lib/admin/admin-operations-qa-lifecycle-proofs.ts`: sole opaque runtime owner/API for `receipt_lease_capture`, `server_ownership`, and `qa_sentinel`, using module-private classes/registries and canonical digest/freshness verification.
...
The QA lifecycle kinds are likewise exact, non-substitutable opaque-owner proof kinds: `receipt_lease_capture` can be produced only by `captureReceiptLease`, `server_ownership` only by `recordServerOwnership`, and `qa_sentinel` only by `recordQaSentinel`; none may be represented by a generic control-plane, command, status, cleanup, fixture, or browser proof. Each `browser_desktop` and `browser_mobile` envelope must carry byte-identical proof references/digests for `receipt_lease_capture`, `fixture_state`, `smtp_isolation`, `billing_mutation_guard_armed`, `server_ownership`, and `qa_sentinel`, in addition to its viewport-specific binding. Signing, serialization, QA acceptance-attestation construction/verification, and final browser-evidence validation reject an omitted kind, a kind substitution, a reference copied into a different field, different desktop/mobile references, or any forged, cloned, stale, cross-target, cross-run, wrong-order, wrong-tree, wrong-build, or wrong-sentinel owner proof. Rehearsal attestation signing/verification rejects all three QA lifecycle kinds as mode-incompatible.
...
`lib/admin/admin-operations-qa-lifecycle-proofs.ts` alone exposes `captureReceiptLease(...)`, `verifyReceiptLeaseCapture(...)`, `recordServerOwnership(...)`, `verifyServerOwnership(...)`, `recordQaSentinel(...)`, and `verifyQaSentinel(...)`. Returned proofs require module-private class/private fields plus separate `WeakSet` registries; there is no public constructor, brand, registration, or unwrap API. Canonical v1 digests bind: capture to parent/child targets, immutable receipt, owner lease, run ID, and monotonic creation sequence; ownership to capture digest, launch receipt, root PID/process-tree ID, origin, commit/run, and validated build-manifest digest; sentinel to ownership digest, target/run/build digest, nonce, response digest, and checkedAt. `QA_LIFECYCLE_PROOF_MAX_AGE_MS = 300000`; verification occurs immediately before child identity for capture, after readiness/before browsers for ownership/sentinel, and again while constructing each browser proof. Plain/cloned/spread/cast, stale, wrong order/target/run/commit/tree/build/origin/nonce/digest, or mismatched viewport refs reject.
...
| browser owner/server | `{BOOT}`, exact disposable `DATABASE_URL`, `DIRECT_URL`, `ADMIN_OPERATIONS_TARGET_BINDING`, `ADMIN_OPERATIONS_RUN_ID`, build/fixture/server/sentinel manifest refs, SMTP blanking and QA opt-in; no Production/provider/auth secret |
...
Assert QA order `parentTrustedIdentity -> orphanScan -> parentDataSafety -> receiptLeaseCapture -> qaTrustedIdentity -> inheritedDataSafety -> migrationStatus -> migrationDeploy -> qaPostStatus -> fixtureProvenance -> fixtureLoad -> fixtureState -> smtpIsolation -> billingMutationGuardArmed -> serverOwnership -> qaSentinel -> browserDesktop -> browserMobile -> billingZeroMutation -> scratchCleanup -> fixtureCleanup -> receiptAuthorizedDelete -> trustedAbsence`. Browser envelopes bind identical fixture-state, server-ownership, sentinel, SMTP, and billing-guard refs. Tests reject missing/order-changed capture and forged/stale/mismatched server/sentinel evidence.
