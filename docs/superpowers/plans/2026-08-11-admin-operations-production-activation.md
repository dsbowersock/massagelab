...
- Immediately before every individual status, deploy, validate, generate, or Playwright spawn, re-resolve and re-verify the lockfile entry, installed package/version, and exact bin target. Carry opened verified artifact handles/identity through spawn where the platform supports it; otherwise the final synchronous spawn-adapter boundary re-stats and re-digests the exact package/bin against the verified snapshot and rejects any change before `execFile` of that exact path with `shell: false`. Tests swap an artifact after verification but before the spawn adapter consumes it and prove no child starts.
- The 30-second monotonic advisory-lock deadline begins before creating the direct connection. The connector accepts the same `AbortSignal` and provides a confirmed cancellation/close handshake bounded by remaining time. Connect and handshake consume that same bound; the caller returns/throws by the deadline and never awaits an unbounded original promise. If a driver can settle later, a detached quarantined handler consumes rejection or destroys eventual success, cannot reach lock/status/authorization/deploy or mutate completion, and reports cleanup safely without extending activation.
...
For `qa` mode, `parentTarget` and `qaTarget` are both required and must be distinct: `parent_trusted_identity` binds `parentTarget`, while the fresh post-create `qa_trusted_identity` binds `qaTarget`. Every other mode requires both fields to be `null`. The serializer copies only the four safe binding fields into each target and never spreads source objects. Attempt/completion validation, evidence-store CAS bindings, and fixture-provenance validation must use the correct target reference. Tests retain both QA targets and reject a missing, identical, swapped, wrong, or extra target; non-QA evidence with either target rejects.
...
| `rehearsal` | `trusted_identity`, `orphan_scan`, `parent_data_safety`, `inherited_data_safety`, `migration_status`, `migration_deploy`, `prisma_validate`, `prisma_generate`, `focused_tests`, `typecheck`, `lint`, `unit_tests`, `build`, `rehearsal_read_only_smoke`, `branch_deleted` | `DEPLOYED`; nonempty audited suffix, complete four-`APPLIED` post-status, and verified branch absence |
| `qa` | `parent_trusted_identity`, `orphan_scan`, `parent_data_safety`, `qa_trusted_identity`, `inherited_data_safety`, `migration_status`, `migration_deploy`, `qa_post_status`, `fixture_provenance`, `browser_desktop`, `browser_mobile`, `fixture_cleanup`, `branch_deleted` | `DEPLOYED`; distinct parent/child targets, complete QA post-status, provenance/browsers/cleanup |
...
Assert QA order `parentTrustedIdentity -> orphanScan -> parentDataSafety -> createBranch -> qaTrustedIdentity -> inheritedDataSafety -> migrationStatus -> migrationDeploy -> qaPostStatus -> fixtureProvenance -> fixtureLoad -> browserServer -> browser`. Parent evidence targets only the parent; fresh authenticated child lookup immediately follows creation and binds returned child project/branch/database/direct-host. Missing/stale/mismatched child evidence stops inherited proof and every mutation while teardown runs. Serializer tests retain both identity checks with distinct sanitized targets.

Compile/import the script helpers without executing Prisma, reading activation environment, printing, or spawning. Assert it rejects trusted-binding mismatch before spawning, sets both `DATABASE_URL` and `DIRECT_URL` only in the Prisma child's environment, leaves the parent environment unchanged, and never copies URLs or raw stdout/stderr into evidence. Drive direct `main` with a trusted Production binding and assert it fails before the spawn double is called; drive it with a trusted `admin-operations-migration-rehearsal-...` binding and assert status succeeds with the same child-only environment boundaries. Production status is implemented inside the Production wrapper rather than exposed through an export or forgeable environment nonce. Exercise the shared serializer with a source object containing every retained field plus credentials, URLs, rows, emails, raw command output, unknown fields, and provider transaction/payment/customer/subscription IDs; assert the exact retained object and exact omission of every prohibited field when that object is printed as status/audit output or embedded in checkpoint/Aegis destinations. Separately test all five modes: `BLOCKED`/`FAILED` attempt evidence accepts only truthful reached order prefixes plus documented cleanup edges and retains `FAIL`, including failed parent/inherited safety; successful completion accepts only the exact full ordered set with every check `PASS`. Reject missing completion checks, invented/unreached attempt checks, out-of-order, duplicate, additional, mode-incompatible, and unknown email-, URL-, provider-ID-, or database-looking names. Assert audit `[]` emits the completed no-deploy `NO_OP` status packet with its full passing audit set and cannot call rehearsal/final, while attestation creation and final mode both reject `[]`; assert its checkpoint and deployed final each remain unfinalized until their mode-specific Production smoke CAS passes.

With fully injected owners, assert rehearsal order after safety is status/deploy/validate/generate/focused/typecheck/lint/unit/build/smoke. QA order is exactly `parent_trusted_identity -> orphan_scan -> parent_data_safety -> create -> qa_trusted_identity -> inherited_data_safety -> migration_status -> deploy -> qa_post_status -> provenance -> load -> server -> browser`, then cleanup/deletion/absence. Parent evidence targets only the parent; immediately after creation, fresh authenticated child evidence targets and binds the exact created child project/branch/database/direct-host before any child migration, fixture, or database mutation. Rejection stops before the first mutation. `qa_post_status` must be exact complete `UP_TO_DATE` with all four `APPLIED`; missing/wrong/truncated blocks provenance/load/server/browser while cleanup still runs. Preserve the parent/inherited/provenance failure tables and synthetic-fixture non-substitution tests.
...
With injected checkout inspector, attestation verifier, durable nonce store, connection factory, lock connection, trusted lookup, private Production status runner, authorization callback, and deploy runner, assert audit order is exactly `trusted_identity -> orphan_scan -> migration_status -> NO_OP/PENDING_SUFFIX decision`. The blocking orphan scan occurs before connection/status/no-op exit; an orphan failure records truthful attempt evidence and calls neither status nor any downstream owner. Connection creation starts under the same pre-existing 30-second deadline/abort bound; the connector accepts its `AbortSignal` and confirms cancellation/close within remaining time. Tests prove a never-settling connect returns/throws within 30 seconds, the abort handshake is bounded, and the caller never awaits the original promise after timeout. A quarantined handler consumes late rejection without an unhandled rejection and destroys late success without reaching try-lock/status/deploy or mutating completion; observing/logging cleanup cannot extend activation. After timely connection, acquire the target lock and run status. Audit `[]` records its completed no-deploy `UP_TO_DATE`/`NO_OP` status packet only after the passing scan and status, then unlocks/closes; it never calls rehearsal/final/deploy, and its checkpoint remains unfinalized pending the mode-specific smoke CAS. Separately assert final mode rejects `[]` and follows this exact nonempty-suffix order:
...
-> create 30-second monotonic deadline and AbortSignal -> race abort-aware connection plus bounded cancellation/close handshake inside that bound -> poll target-scoped pg_try_advisory_lock with remaining time
...
The wrapper must require exact `audit` or `final` mode and obtain the direct URL and expected tuple only through its child-scoped environment input. It runs Git itself in the exact working directory used for Prisma: require attached HEAD, exact `git rev-parse HEAD`, and empty tracked/untracked status; caller-provided SHA values are comparison inputs only and can never establish checkout identity. Before either mode creates its dedicated direct PostgreSQL connection, it creates the 30-second monotonic deadline and `AbortSignal`; the connector accepts that signal and exposes a cancellation/close handshake that confirms within remaining time. Connect and handshake consume the same bound, and the caller returns/throws by its deadline without awaiting an unbounded original promise. A possibly late driver settlement is handled only by a detached quarantine that consumes rejection or destroys eventual success, cannot reach lock/status/deploy or mutate completion, and safely observes/logs cleanup without extending activation. Only after timely success does the wrapper derive the stable advisory-lock key and poll `pg_try_advisory_lock` every 250 ms using only remaining time; every probe uses that same signal/bound. Set `lockAcquired = true` only from the successful database result. In `finally`, first require the process-tree owner to terminate/await every outstanding child and descendant; only after confirmed quiescence call `pg_advisory_unlock` when `lockAcquired` is true, then close the connection, preserving the original failure if cleanup also fails. Audit mode runs one preliminary trusted target/status read, records sanitized audit evidence, then exits; its code path has no authorization or deploy call.
...
Expected: PASS, including combined-generation crash/recovery and evidence-finalizer cases, and no secret-bearing output.
...
Execute QA in this exact order: authenticate and record the parent binding as `parent_trusted_identity`, run `orphan_scan`, prove parent data safety, create a second disposable branch named `admin-operations-qa-YYYYMMDD-HHMMSS-<runSuffix>`, perform a fresh authenticated child lookup and record `qa_trusted_identity`, prove inherited-data safety, run migration status and deploy, require exact `qa_post_status` with all four migrations `APPLIED`, verify fixture provenance, then load fixtures, start the server, and run the browser projects. Never reuse the rehearsal branch. Generate at least 128 random bits for the suffix, check the exact candidate in the trusted control plane, retry collisions within a small bound, and bind exact name/suffix/returned branch ID to the verified run-owner lease. Any failed gate blocks every downstream step, while process teardown, fixture cleanup when safe, exact branch deletion, and trusted absence verification always run in `try/finally`. Fixture loading cannot satisfy either safety gate. Sanitized proof exposes no rows, PHI, or secrets, and ambiguity triggers deletion/absence verification. Fixture provenance must bind the immutable approved source/version allowlist, owner-controlled five-minute freshness bound, exact `qaTarget`/database, and clean commit; plain/forged/cloned/cast or row-bearing objects cannot pass, and serialization retains only safe version/digest/outcome/timestamp and bindings. Apply migrations through the verified repository-local Prisma owner, set the operator-expected four-field binding and opt-in only for the Playwright child, blank SMTP, and invoke the pinned browser owner:
...
Authoritative QA prefix is `parent_trusted_identity -> orphan_scan -> parent_data_safety -> create -> qa_trusted_identity -> inherited_data_safety`. The parent check serializes only the parent target and cannot authorize the child; the fresh post-create child check serializes the exact returned QA target and must pass before inherited proof or mutation. This replaces any older `trusted_identity` shorthand below.

Await authenticated parent binding and record `parent_trusted_identity`; after the orphan scan and parent-data safety proof pass and the child is created, perform a fresh authenticated child lookup and record `qa_trusted_identity`; the child identity must pass before inherited proof or any migration, fixture, or database mutation. After migration deploy, require allowlisted `qa_post_status` to be one complete `UP_TO_DATE` result with all four migrations `APPLIED` before provenance/load/server/browser. Missing, wrong, or truncated post-status blocks all downstream calls while `finally` cleanup, exact deletion, and trusted absence verification still run.

After another blocking prefix scan with the same alert/verified-owner/lease/stale-proof rules, obtain trusted proof that the exact QA parent is data-free or approved-sanitized before branch creation. Create a fresh identified QA branch distinct from the rehearsal clone only after that proof passes. Immediately after creation, independently prove inherited rows absent or approved-sanitized before any status, deploy, validation, test, smoke, browser, or fixture work. Only then apply migrations, have the server-only fixture-provenance owner verify its opaque proof, load the approved fixture, start the browser server, and run both projects inside `try/finally`. Missing/forged/plain/cloned/cast/stale/late/wrong source/version/target/database/commit/digest/row-bearing provenance prevents load/server/browser while finally still performs cleanup, exact deletion, and absence verification; an authenticated injected approved source succeeds. On normal completion or catchable failure/cancellation, verify fixture cleanup, delete the branch, and block until the trusted control plane verifies absence; a resumed run must assume hard termination may have bypassed cleanup and repeat the startup check.
