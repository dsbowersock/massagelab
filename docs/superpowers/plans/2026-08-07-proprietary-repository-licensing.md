...
- Modify `package.json`: npm ecosystem signal that the package is unlicensed for reuse.
...
Create `tests/proprietary-license.test.mjs` with this exact contract:
...
describe("proprietary repository licensing", () => {
  it("keeps every public repository surface explicitly proprietary", async () => {
    ...
    assert.match(license, /provided "AS IS"/)
...
- [ ] **Step 3: Add the root proprietary license**
...
- [ ] **Step 4: Replace the provisional README notice and set package metadata**
...
Add the top-level field directly below `"private": true` in `package.json`:

```json
"license": "UNLICENSED",
```

- [ ] **Step 5: Add the reusable shared-footer notice**
...
- [ ] **Step 7: Run the full repository unit suite**
...
- [ ] **Step 8: Run static validation separately**
...
- [ ] **Step 9: Review the exact diff and whitespace gate**
...
Expected: `git diff --check` exits `0`; only the five implementation files are uncommitted; the diff contains no repository-visibility, legal-page, purchase-license, CodeRabbit, attribution, or immersive-layout changes.
...
Expected: one implementation commit containing only the five approved implementation files. Keep the previously committed design spec and implementation plan as separate documentation history.
