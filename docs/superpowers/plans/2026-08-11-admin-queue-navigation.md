...
Expected: after fixture provenance, the wrapper records `smtp_isolation: PASS` and `billing_mutation_guard_armed: PASS`, then runs distinct desktop and mobile Chromium projects whose proofs bind both guard references. Only after both browser trees exit may `billing_zero_mutation: PASS` prove zero billing-goodwill forms, POSTs, and provider mutations while binding both browser proofs. It then records bounded `scratch_cleanup`, performs receipt-authorized fixture cleanup, deletes only the exact disposable Neon branch named by the immutable creation receipt/run lease, and verifies trusted-control-plane absence. `scratch_cleanup: FAIL` blocks completion but remains truthful attempt evidence and never suppresses fixture/branch teardown; missing or failed guard, browser, zero-mutation, tree-exit, scratch, fixture, deletion, or absence proof blocks acceptance.
