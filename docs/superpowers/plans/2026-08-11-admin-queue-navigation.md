...
Expected: one outer `try/finally`-equivalent browser-acceptance owner atomically captures the immutable creation receipt/run lease before any child database work and owns every later gate plus teardown. Capture failure fails closed and authorizes neither fixture-row cleanup nor branch deletion. After provenance passes, it loads fixtures and records QA-only `fixture_state: PASS`: a canonical sanitized aggregate-state digest bound to exact QA target, commit, run/lease, and provenance, with no rows/PHI. It then records SMTP and armed-billing proofs; distinct desktop/mobile envelopes bind the same byte-identical fixture-state proof ref/digest plus both guards. After both trees exit, zero mutation may pass. Once receipt/lease capture succeeds, `finally` attempts fixture-row cleanup only when complete process-tree exit and cleanup safety are positively confirmed; otherwise it records that edge truthfully as `SKIPPED`/`BLOCKED` and performs no row mutation. Exact receipt/lease branch deletion and trusted absence remain independent and bounded after every post-capture outcome. Fixture-cleanup failure still permits deletion/absence; deletion failure still permits absence. Every reached/skipped/failure outcome is truthful and blocks acceptance when required.

Add a table-driven failure matrix injecting failure/throw/timeout at receipt/lease capture; parent/child identity, safety, migration/status, provenance, fixture load and `fixture_state`; SMTP/billing guards, server, each browser, tree join, zero mutation, scratch cleanup; fixture cleanup, branch deletion, and trusted absence verification. Capture failure proves no database/fixture work, row cleanup, or deletion without authority. Every post-capture row asserts fixture-row cleanup is attempted only with confirmed full-tree exit/safety and otherwise is truthfully `SKIPPED`/`BLOCKED`; exact deletion is always attempted once and absence is attempted even after deletion failure. Serializer/acceptance tests require `fixture_state` only in QA at the exact position, reject forged/row-bearing/cross-binding state and mismatched browser refs, retain truthful outcomes, and block every failure.
