...
Expected: the outer owner records `receipt_lease_capture` before child identity, binding parent/child targets, immutable receipt, lease, run, and creation order. After fixture state and guards it records `server_ownership` bound to launch/process tree/current build manifest and `qa_sentinel` bound to the live QA target/run; both browsers bind identical receipt-capture, fixture-state, SMTP-isolation, billing-guard, server-ownership, and sentinel refs/digests. Then the established safe cleanup/deletion/absence rules apply.

Add failure/throw/timeout rows for receipt capture, every gate, `server_ownership`, `qa_sentinel`, browsers/tree/zero/scratch, fixture cleanup, deletion, and absence. Reject missing/order-changed capture plus forged/stale/cross-run/tree/build/sentinel proofs and mismatched viewport refs. Row cleanup requires tree safety; deletion/absence remain unconditional post-capture.
