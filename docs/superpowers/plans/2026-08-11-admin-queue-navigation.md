...
Expected: one outer `try/finally`-equivalent browser-acceptance owner captures the immutable creation receipt/run lease immediately at creation and owns every later gate plus teardown. After fixture provenance, it records `smtp_isolation: PASS` and `billing_mutation_guard_armed: PASS`, then distinct desktop/mobile proofs; only after both browser trees exit may `billing_zero_mutation: PASS` bind them. Its `finally` path always attempts, in order and with independent bounded outcomes, receipt-authorized fixture cleanup, exact receipt/lease Neon branch deletion, and trusted absence verification after any identity, provenance, SMTP, billing guard, browser, process-tree exit, zero-mutation, or `scratch_cleanup` failure. A failed/unconfirmed process-tree exit may prevent unsafe database fixture deletion but never suppresses exact control-plane branch deletion/absence attempted solely from the receipt/lease. Retain every reached PASS/FAIL plus cleanup edge truthfully; any required cleanup/deletion/absence failure blocks acceptance. `scratch_cleanup: FAIL` also blocks completion but never suppresses the external `finally` attempts.

Add a table-driven teardown failure matrix injecting failure/throw/timeout at parent/child identity, inherited safety, migration/status, provenance, SMTP, billing-arm, fixture load, server start, each browser, process-tree join, zero-mutation assertion, and scratch cleanup. Every row asserts fixture cleanup is attempted when safe, exact receipt/lease branch deletion is always attempted once after creation, trusted absence verification is always attempted after deletion, all cleanup outcomes are serialized truthfully, and acceptance remains blocked for any failed or inconclusive required proof.
