...
- Last verified checks: `npm run prisma:validate`, `npm run typecheck`, `npm run lint`, `npm run test`, and `npm run build` passed for the anatomy image review queue branch. The configured database has the flashcard deck, flashcard mastery, anatomy media review, anatomy media review default/backfill, and `20260612110000_anatomy_media_view_requests` migrations applied; the anatomy foundation seed has also been rerun to persist expanded body-system and tissue-type membership relationships and remove obsolete broad-inference taxonomy rows.
- Migration count at last verification: 25 migrations in the repo.
- Latest migration area: anatomy media desired-view request queue for transverse/custom/wider-context media review and automation handoff.
