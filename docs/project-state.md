...
- Last verified checks: `npm run prisma:validate` and `npm run typecheck` passed for the anatomy media view coverage branch. The configured database has the flashcard deck, flashcard mastery, anatomy media review, anatomy media review default/backfill, and `20260612110000_anatomy_media_view_requests` migrations applied.
