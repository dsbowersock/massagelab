...
- Added the [Atmosphere Wellness Surface plan](superpowers/plans/2026-06-18-atmosphere-wellness-surface.md) and exposed the hosted station UI at `/wellness/atmosphere` as a public Wellness tool while keeping `/browse` as a compatibility workbench. The branch adds Wellness page, sidebar, homepage, and onboarding shortcut discovery for Atmosphere, keeps playback route-persistent through the existing global mini-player, and adds a first public Calmness-style breathing guide beside the station controls.
