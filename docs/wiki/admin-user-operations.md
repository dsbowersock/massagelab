...
Branch 2 is the foundation owner for authorization, audit, activity, email intents, and capability-aware dashboard access. Branch 3 consumes those owners and completed Tasks 7-9: the full-Admin directory, bounded account detail, signed-in Account Activity, safe aggregate dashboard metrics, and the audited failed-email retry seam. Branch 4 owns the completed delegated-role mutation/UI and shared JWT invalidation seam. Branch 5 Tasks 12-13 now own the bounded security services and confirmed Security/Activity controls described above. Do not develop serial branches concurrently or copy foundation files into a stale worktree.
