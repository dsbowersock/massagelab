...
- `/atmosphere` exposes the full 57-piece Alex Bainter Generative.fm package catalog through MassageLab's global music provider, swipe/scroll category rails, deterministic organic-geometric SVG station artwork, and persistent mini-player. `/wellness/atmosphere` redirects there, and `/browse` remains available as a compatibility grid/workbench for the same station runtime.
