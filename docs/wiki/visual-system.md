...
Route changes use the App Router's root `loading.tsx` boundary. `RouteLoadingFeedback` mounts a fixed 3px top bar that is always `pointer-events: none`, then waits 180ms before showing the shared Loader so quick transitions do not flash a large indicator. The boundary exposes one polite `Loading page` status, stays clear of app-bar and music-player controls, and lives below the persistent root shell so route changes do not remount long-lived music or timer state. Reduced motion keeps the feedback visible; built-app acceptance proves that the real Loader canvas contributes composited pixels and that two rendered samples 400ms apart are identical.
