...
- The exact wholly pending order before any runtime or deployment is: apply `20260828120000_identity_method_safety` as expansion; apply the dedicated one-statement `20260828121000_identity_normalized_email_index`; then apply `20260828130000_membership_subscription_convergence`. The first migration creates the hashed `AuthRateLimitBucket` and private intent/notice tables and expands the shared `AuthAttemptPurpose` enum with `GOOGLE_INTENT` for forward/rollback compatibility. The second contains only PostgreSQL `CREATE UNIQUE INDEX CONCURRENTLY` for `User_normalized_email_key` after the zero-collision preflight, avoiding Prisma 7's multi-statement execution boundary for nontransactional DDL. Monitor the build. If it fails, stop the migration and runtime rollout; an invalid index may remain. Use a separately reviewed recovery to resolve collisions, remove the invalid index, recreate and verify the concurrent index, and reconcile Prisma's failed-migration record only after all expansion objects are proven correct. Only after that concurrent index verifies successfully may the membership migration proceed. The legacy `AuthAttempt` table's columns, indexes, and data remain untouched for old instances and rollback. All three migrations are committed local code only and none was applied by this work.
...
- After the zero-collision preflight and both ordered identity migrations above verify successfully, deploy `20260828130000_membership_subscription_convergence` as the third additive expansion before serving the convergence runtime. It creates `MembershipWebhookReceipt`, a unique `(provider, providerEventId)` receipt key, bounded status/attempt/failure/timing metadata, and nullable `lastStripeEventId`, `lastStripeEventCreatedAt`, and `lastStripeAuthoritativeAt` columns. It must not backfill, rewrite, tighten, or default existing membership rows. Apply it only through the separately authorized exact direct maintenance target, verify all three reviewed migrations current with no extras or failure, and preserve the prior runtime for rollback until the reviewed runtime is serving. This local work did not connect to a database or apply any migration.
...
- Production remains separately gated on exact-target authorization/application of all three reviewed migrations in order, deployed-commit equality, production migration status, live-mode Price/tax/webhook/Portal readback, provider callback/origin and delivery health, pool/capacity/alert posture, and registration/Checkout pause controls. Any test or live Stripe payment, Checkout, event emission, refund, cancellation, provider/Portal setting mutation, database connection or private-row fixture, deployment, push, or merge needs its own authority. Historical billing smoke is useful context only and must not be reported as exact-head proof.
