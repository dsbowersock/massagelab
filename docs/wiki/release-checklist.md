...
- The 2026-08-29 local Task 4 receipt passed the exact-lane checks 21/21, the focused Node matrix 49/49, and the focused built-app interaction spec in both projects at 14 public passes plus 3 explicit private authorization-gated skips per project. The complete Browser-QA release gate did not pass: a one-worker run reached 711/721 with 39 failure artifacts in other spec families and then stalled during worker teardown without final totals. Treat this as a red/incomplete release gate, not as launch approval; obtain a clean exact-candidate full Browser-QA exit before release.

## Identity And Account-Method Gate

- Run the count-only normalized-email preflight before migration or runtime deployment: `npm run auth:check-normalized-emails` against the separately selected direct, non-pooler maintenance target. Release evidence may contain only `normalized_collision_count=<number>`, and deployment stops unless it is zero. Do not repair accounts, apply a migration, or expose target/account values during the preflight.
- The exact wholly pending order before any runtime or deployment is: apply `20260828120000_identity_method_safety` as expansion; apply the dedicated one-statement `20260828121000_identity_normalized_email_index`; then apply `20260828130000_membership_subscription_convergence`. The first migration creates the hashed `AuthRateLimitBucket` and private intent/notice tables and expands the shared `AuthAttemptPurpose` enum with `GOOGLE_INTENT` for forward compatibility. The second contains only PostgreSQL `CREATE UNIQUE INDEX CONCURRENTLY` for `User_normalized_email_key` after the zero-collision preflight, avoiding Prisma 7's multi-statement execution boundary for nontransactional DDL. Monitor the build. If it fails, stop the migration and runtime rollout; an invalid index may remain. Use a separately reviewed recovery to resolve collisions, remove the invalid index, recreate and verify the concurrent index, and reconcile Prisma's failed-migration record only after all expansion objects are proven correct. Only after that concurrent index verifies successfully may the membership migration proceed. The legacy `AuthAttempt` table's columns, indexes, and data remain untouched through the pre-bridge drain and bridge-capable rollback window, but pre-bridge code is never a rollback target after bridge cutover. All three migrations are committed local code only and none was applied by this work.
- Ship the Task 2 identity cutover inside the first exact-`1` paused bridge deployment below, not as a separate earlier runtime deployment. No new serving caller may read or write `AuthAttempt`, dual-write it, or fall back to it. During that bridge deployment, old pre-bridge instances may still write raw legacy keys. The same configured-maximum-invocation-lifetime drain proof must establish that every pre-bridge instance stopped before treating `AuthAttempt` as inactive state.
...
- Keep dangerous automatic email linking absent. Prove each Google start uses the private, 10-minute `ml-auth-method-binding` cookie contract; no intent/binding/provider proof or OAuth token may reach URLs, client storage, rendered data, logs, or evidence. Matching-email linking requires the same browser's provider proof plus explicit fresh Credentials confirmation, provider/cross-account conflicts fail closed, and method removal cannot leave zero credentials.
...
- For browser acceptance, private identity rows may run only after the fingerprint guard proves the exact opted-in, non-Production disposable runtime/direct pair and those URLs equal the process `DATABASE_URL`/`DIRECT_URL`. Apply the approved migrations first, use the exact fingerprint-checked wrapper, and verify all private fixtures are removed afterward. Ordinary public rows run without a database and private rows must hard-skip when this authorization is absent.
...
- After the zero-collision preflight and both ordered identity migrations above verify successfully, deploy `20260828130000_membership_subscription_convergence` as the third additive expansion before any membership runtime cutover. It creates `MembershipWebhookReceipt`, a unique `(provider, providerEventId)` receipt key, bounded status/attempt/failure/timing metadata, and nullable `lastStripeEventId`, `lastStripeEventCreatedAt`, and `lastStripeAuthoritativeAt` columns. It must not backfill, rewrite, tighten, or default existing membership rows. Apply it only through the separately authorized exact direct maintenance target and verify all three reviewed migrations current with no extras or failure. This local work did not connect to a database or apply any migration.
- Perform exactly two separately authorized runtime deployments after the migrations. First deploy the bridge-capable runtime with `MASSAGELAB_MEMBERSHIP_WEBHOOK_WRITES_PAUSED` exactly `1`, prove the Production alias serves its exact SHA, record the current configured maximum invocation lifetime, wait at least that long after alias cutover, and prove no pre-bridge SHA is receiving or executing webhook requests. The exact-`1` bridge returns private retryable `503` for signed membership deliveries before membership provider, database, or cache work; Stripe retains and retries those deliveries, access may show processing, and operators must not manually replay or acknowledge them. Non-membership Checkout and background commerce remain on their existing routes.
- Only after the writer drain is proven may the second deployment serve the same bridge-capable runtime with `MASSAGELAB_MEMBERSHIP_WEBHOOK_WRITES_PAUSED` absent or exactly `0`. Verify the Production alias/deployed SHA and migration gate again, then prove held deliveries reach `2xx`, their receipts become terminal, and persisted access converges without manual replay. If rollback is required after bridge cutover, use only a bridge-capable deployment with the flag exactly `1`; never restore a pre-bridge legacy writer. Keep the additive schema and do not run a destructive migration rollback. Retain the bridge through held-delivery/backlog proof and the rollout window; removing it requires a later separately reviewed retirement.
...
- Keep receipt creation race recovery exact. Retry once from a fresh transaction only for `P2002` on the `MembershipWebhookReceipt` model with exact ordered target `provider, providerEventId` or connector-safe index `MembershipWebhookReceipt_provider_providerEventId_key`, or for installed PrismaNeon metadata that additionally proves `DriverAdapterError`, `UniqueConstraintViolation`, and PostgreSQL `23505` with the same exact ordered fields/index. Wrong codes, models, field order, indexes, mixed constraint metadata, or malformed shapes are not duplicate-delivery proof.
...
- Production remains separately gated on exact-target authorization/application of all three reviewed migrations in order, the two-deployment bridge/drain/unpause protocol, deployed-commit equality, production migration status, held-delivery `2xx`/terminal-receipt/access convergence, live-mode Price/tax/webhook/Portal readback, provider callback/origin and delivery health, pool/capacity/alert posture, and registration/Checkout pause controls. Any test or live Stripe payment, Checkout, event emission or replay, refund, cancellation, provider/Portal setting or environment mutation, database connection or private-row fixture, deployment, push, or merge needs its own authority. Historical billing smoke is useful context only and must not be reported as exact-head proof.
