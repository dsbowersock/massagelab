...
- The exact wholly pending order before any runtime or deployment is: apply `20260828120000_identity_method_safety` as expansion; apply the dedicated one-statement `20260828121000_identity_normalized_email_index`; apply `20260828130000_membership_subscription_convergence`; apply `20260901100000_auth_method_intent_two_factor_purposes`; then apply `20260901101000_auth_method_intent_registration_callback`. The first migration creates the hashed `AuthRateLimitBucket` and private intent/notice tables and expands the shared `AuthAttemptPurpose` enum with `GOOGLE_INTENT` for forward compatibility. The second contains only PostgreSQL `CREATE UNIQUE INDEX CONCURRENTLY` for `User_normalized_email_key` after the zero-collision preflight, avoiding Prisma 7's multi-statement execution boundary for nontransactional DDL. Monitor the build. If it fails, stop the migration and runtime rollout; an invalid index may remain. Use a separately reviewed recovery to resolve collisions, remove the invalid index, recreate and verify the concurrent index, and reconcile Prisma's failed-migration record only after all expansion objects are proven correct. Only after that concurrent index verifies successfully may the membership migration proceed. The fourth migration adds the three two-factor `AuthMethodIntentPurpose` values, and the fifth adds the bounded registration `AuthMethodIntent.callbackPath`. The legacy `AuthAttempt` table's columns, indexes, and data remain untouched through the pre-bridge drain and bridge-capable rollback window, but pre-bridge code is never a rollback target after bridge cutover. All five migrations are committed local code only and remain unapplied locally; no live application is claimed.
...
- For browser acceptance, private identity rows may run only after the fingerprint guard proves the exact opted-in, non-Production disposable runtime/direct pair and those URLs equal the process `DATABASE_URL`/`DIRECT_URL`. Apply and verify all five reviewed migrations in the exact order above first, use the exact fingerprint-checked wrapper, and verify all private fixtures are removed afterward. Ordinary public rows run without a database and private rows must hard-skip when this authorization is absent.
...
- After the zero-collision preflight and both ordered identity migrations above verify successfully, deploy `20260828130000_membership_subscription_convergence` as the third additive expansion, followed by `20260901100000_auth_method_intent_two_factor_purposes` and `20260901101000_auth_method_intent_registration_callback`, before any membership runtime cutover. The membership migration creates `MembershipWebhookReceipt`, a unique `(provider, providerEventId)` receipt key, bounded status/attempt/failure/timing metadata, and nullable `lastStripeEventId`, `lastStripeEventCreatedAt`, and `lastStripeAuthoritativeAt` columns. It must not backfill, rewrite, tighten, or default existing membership rows. Apply the exact five only through the separately authorized direct maintenance target and verify all five reviewed migrations current with no extras or failure. This local work did not connect to a database or apply any migration.
...
- Production remains separately gated on exact-target authorization/application of all five reviewed migrations in order, the two-deployment bridge/drain/unpause protocol, deployed-commit equality, production migration status, held-delivery `2xx`/terminal-receipt/access convergence, live-mode Price/tax/webhook/Portal readback, provider callback/origin and delivery health, pool/capacity/alert posture, and registration/Checkout pause controls. Any test or live Stripe payment, Checkout, event emission or replay, refund, cancellation, provider/Portal setting or environment mutation, database connection or private-row fixture, deployment, push, or merge needs its own authority. Historical billing smoke is useful context only and must not be reported as exact-head proof.
