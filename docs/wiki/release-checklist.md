...
- Deploy `20260828130000_membership_subscription_convergence` as an additive expansion before serving the convergence runtime. It creates `MembershipWebhookReceipt`, a unique `(provider, providerEventId)` receipt key, bounded status/attempt/failure/timing metadata, and nullable `lastStripeEventId`, `lastStripeEventCreatedAt`, and `lastStripeAuthoritativeAt` columns. It must not backfill, rewrite, tighten, or default existing membership rows. Apply it only through the separately authorized exact direct maintenance target, verify migration status, and preserve the prior runtime for rollback until the reviewed runtime is serving. This local work did not connect to a database or apply the migration.
- Preserve the signed ordering contract. A terminal receipt is a duplicate no-op; a newer unambiguous Stripe event may apply; an older event must be ignored without overwriting the stored snapshot; equal provider timestamps, legacy rows with null watermarks, membership Checkout completion, and every different event after a successful authoritative read must retrieve current Stripe state outside the transaction. After retrieval, use only a short compare-and-commit transaction. Compare Stripe event time only with the stored Stripe event watermark and the local authoritative marker only with the captured local marker; never compare the two clocks.
...
- A receipt that fails before terminal completion remains `RECEIVED` with only an allowlisted failure code. There is no durable membership background worker, so the signed route must return private retryable HTTP `503` and let Stripe retry. Do not convert provider, price, ownership, malformed-event, or concurrent uncertainty into a successful acknowledgement. Cache invalidation is permitted only after the convergence owner commits `changed: true`.
...
- Checkout return performs exactly five persisted reads with delays `[0,1000,2000,4000,8000]`. Active access may settle immediately. Billing-attention or no-active status may settle only after that polling run observes a revision different from its baseline; otherwise it must exhaust to still-processing guidance and offer only a safe status retry. Portal may display current persisted state on its first read while watching the same bounded revision window. Access actions use persisted feature keys such as `premium_backgrounds`, never displayed plan names, redirect state, or browser storage. Ordinary page rendering never retrieves Stripe; current-subscription retrieval is limited to Checkout completion and ambiguous, legacy, or post-authoritative signed membership events.
- Before release, rerun the exact clean candidate gate: Prisma validate/generate, TypeScript, lint, complete tests, ordinary Production build, `git diff --check`, fresh Browser-QA build, and focused desktop/mobile `membership-return-status.spec.ts`. Do not run the browser specs from stale output. Database-backed browser rows require the separately approved non-Production disposable runtime/direct target, exact SHA-256 fingerprint, this migration applied, fresh-process wrappers, and exact cleanup; otherwise those rows must hard-skip while public rows still run.
- Current local evidence at exact implementation head `04881a884d6e5cce656e1ff939bd3b998c67bc18` is not yet a Production release pass. Prisma validation/generation, TypeScript, lint, a 379/379 focused subscription/Stripe/background matrix, the corrected user-facing copy contract at 1/1, `git diff --check`, and the ordinary 114-page Production build pass. The Windows complete suite reports 3,342 total, 3,329 pass, 10 fail, and 3 intentional skips; all ten failures are the known host line-ending baseline: nine established Atmoshaper CRLF fixture failures with committed blobs matching their pinned hashes and one Windows CRLF exact-newline failure in `auth-schema-migration.test.mjs`. No branch-related test failure remains. A fresh Browser-QA build failed only because restricted access could not fetch Inter from Google Fonts, so desktop/mobile membership-return attempts did not run. Obtain fresh hosted Linux and Browser-QA evidence and rerun the exact candidate gate before Production release.
- Production remains separately gated on exact migration target/application, deployed-commit equality, production migration status, live-mode Price/tax/webhook/Portal readback, provider callback/origin and delivery health, pool/capacity/alert posture, and registration/Checkout pause controls. Any test or live Stripe payment, Checkout, event emission, refund, cancellation, provider/Portal setting mutation, database connection or private-row fixture, deployment, push, or merge needs its own authority. Historical billing smoke is useful context only and must not be reported as exact-head proof.
...
- Apply `20260828120000_identity_method_safety` as expansion, then apply the dedicated one-statement `20260828121000_identity_normalized_email_index`, before runtime. The first migration creates the hashed `AuthRateLimitBucket` and private intent/notice tables and expands the shared `AuthAttemptPurpose` enum with `GOOGLE_INTENT` for forward/rollback compatibility. The second contains only PostgreSQL `CREATE UNIQUE INDEX CONCURRENTLY` for `User_normalized_email_key` after the zero-collision preflight, avoiding Prisma 7's multi-statement execution boundary for nontransactional DDL. Monitor the build. If it fails, stop the migration and runtime rollout; an invalid index may remain. Use a separately reviewed recovery to resolve collisions, remove the invalid index, recreate and verify the concurrent index, and reconcile Prisma's failed-migration record only after all expansion objects are proven correct. The legacy `AuthAttempt` table's columns, indexes, and data remain untouched for old instances and rollback. Both migrations are committed local code only and neither was applied by this work.
- Deploy the Task 2 cutover as one reviewed runtime: no new serving caller may read or write `AuthAttempt`, dual-write it, or fall back to it. During the rolling deployment, old instances may still write raw legacy keys. Record exact deployment and drain evidence before asserting those writes stopped or treating `AuthAttempt` as inactive state.
