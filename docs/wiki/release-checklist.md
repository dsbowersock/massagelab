...
- Apply `20260828120000_identity_method_safety` as expansion before runtime. It creates the hashed `AuthRateLimitBucket` and private intent/notice tables and expands the shared `AuthAttemptPurpose` enum with `GOOGLE_INTENT` for forward/rollback compatibility. The legacy `AuthAttempt` table's columns, indexes, and data remain untouched for old instances and rollback. The migration is committed local code only and was not applied by Task 7.
