...
- Apply `20260828120000_identity_method_safety` as expansion before runtime. It creates the hashed `AuthRateLimitBucket` and private intent/notice tables while preserving `AuthAttempt` unchanged for old instances and rollback. The migration is committed local code only and was not applied by Task 7.
