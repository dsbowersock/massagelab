...
export function ColorPickerSwatch({ id, label, value, fallback, disabled, readOnly, onChange, className, buttonClassName }: ColorPickerSwatchProps) {
  ...
  const updatePopoverPosition = useCallback(() => {
    ...
    const popoverWidth = Math.min(COLOR_PICKER_POPOVER_MAX_WIDTH, Math.max(0, viewportWidth - COLOR_PICKER_POPOVER_MARGIN * 2))
    const spaceBelow = viewportHeight - triggerBounds.bottom
    const shouldOpenAbove = spaceBelow < COLOR_PICKER_POPOVER_ESTIMATED_HEIGHT && triggerBounds.top > spaceBelow

    const unclampedLeft = triggerBounds.left + triggerBounds.width / 2 - popoverWidth / 2
    const left = clampNumber(unclampedLeft, COLOR_PICKER_POPOVER_MARGIN, Math.max(COLOR_PICKER_POPOVER_MARGIN, viewportWidth - popoverWidth - COLOR_PICKER_POPOVER_MARGIN))
    const top = shouldOpenAbove ? Math.max(COLOR_PICKER_POPOVER_MARGIN, triggerBounds.top - COLOR_PICKER_POPOVER_ESTIMATED_HEIGHT - COLOR_PICKER_POPOVER_GAP) : Math.min(Math.max(COLOR_PICKER_POPOVER_MARGIN, triggerBounds.bottom + COLOR_PICKER_POPOVER_GAP), Math.max(COLOR_PICKER_POPOVER_MARGIN, viewportHeight - COLOR_PICKER_POPOVER_MARGIN - COLOR_PICKER_POPOVER_ESTIMATED_HEIGHT))
  ...
  return (
    <div ref={containerRef} className={cn(styles.colorPickerShell, className)}>
      ...
      {!readOnly && isOpen && hasMounted && typeof document !== "undefined"
        ? createPortal(
            <div ref={popoverRef} className={styles.colorPickerPopover} style={popoverStyle} role="dialog" aria-label={`${label} picker`} aria-modal="false" data-chimer-control-portal="true">
              <div
                ref={colorAreaRef}
              ...
              <div className={styles.colorPickerSliderRow}>
                <button type="button" className={styles.colorPickerEyeDropper} onClick={handleEyeDropperClick} disabled={disabled || !eyeDropperSupported} title={eyeDropperSupported ? "Pick a color from the screen" : "Eye dropper is not supported in this browser"} aria-label="Pick a color from the screen">
...
export function ColorPickerInput({ onValueChange, buttonClassName, ...props }: ColorPickerInputProps) {
  return <ColorPickerSwatch {...props} onChange={onValueChange} buttonClassName={cn("ml-color-picker-input", buttonClassName)} />
}
