...
import type { BackgroundEffectProps, ReactBitsGridScanOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]
type ScanDirection = "forward" | "backward" | "pingpong"
type LineStyle = "solid" | "dashed" | "dotted"
type Vec2 = { x: number; y: number }
type ResolvedGridScanOptions = Required<ReactBitsGridScanOptions>
...
const DEFAULT_REACT_BITS_GRID_SCAN: ResolvedGridScanOptions = {
  sensitivity: 0.55,
  lineThickness: 1,
  linesColor: "#2F293A",
  scanColor: "#FF9FFC",
...
export default function ReactBitsGridScanBackground({
  className,
  reactBitsGridScan,
  ...
  return (
    <div className={cn(styles.reactBitsGridScan, className)} aria-hidden="true">
      <canvas ref={canvasRef} className={styles.reactBitsGridScanCanvas} />
    </div>
  )
}
...
function getUniformLocation(
  ...
  if (!location) {
    throw new Error(`Missing React Bits Grid Scan uniform: ${name}`)
  }
  return location
}

function resolveGridScanOptions(options?: ReactBitsGridScanOptions): ResolvedGridScanOptions {
  return {
    sensitivity: clampNumber(options?.sensitivity, DEFAULT_REACT_BITS_GRID_SCAN.sensitivity, 0, 1),
    lineThickness: clampNumber(options?.lineThickness, DEFAULT_REACT_BITS_GRID_SCAN.lineThickness, 0.2, 6),
    linesColor: normalizeHexColor(options?.linesColor, DEFAULT_REACT_BITS_GRID_SCAN.linesColor),
    scanColor: normalizeHexColor(options?.scanColor, DEFAULT_REACT_BITS_GRID_SCAN.scanColor),
    scanOpacity: clampNumber(options?.scanOpacity, DEFAULT_REACT_BITS_GRID_SCAN.scanOpacity, 0, 1),
    gridScale: clampNumber(options?.gridScale, DEFAULT_REACT_BITS_GRID_SCAN.gridScale, 0.02, 0.5),
    lineStyle: normalizeLineStyle(options?.lineStyle),
    lineJitter: clampNumber(options?.lineJitter, DEFAULT_REACT_BITS_GRID_SCAN.lineJitter, 0, 1),
    scanDirection: normalizeScanDirection(options?.scanDirection),
    noiseIntensity: clampNumber(options?.noiseIntensity, DEFAULT_REACT_BITS_GRID_SCAN.noiseIntensity, 0, 0.25),
    bloomOpacity: clampNumber(options?.bloomOpacity, DEFAULT_REACT_BITS_GRID_SCAN.bloomOpacity, 0, 2),
    scanGlow: clampNumber(options?.scanGlow, DEFAULT_REACT_BITS_GRID_SCAN.scanGlow, 0.1, 3),
    scanSoftness: clampNumber(options?.scanSoftness, DEFAULT_REACT_BITS_GRID_SCAN.scanSoftness, 0.2, 6),
    scanPhaseTaper: clampNumber(options?.scanPhaseTaper, DEFAULT_REACT_BITS_GRID_SCAN.scanPhaseTaper, 0, 0.49),
    scanDuration: clampNumber(options?.scanDuration, DEFAULT_REACT_BITS_GRID_SCAN.scanDuration, 0.05, 10),
    scanDelay: clampNumber(options?.scanDelay, DEFAULT_REACT_BITS_GRID_SCAN.scanDelay, 0, 10),
