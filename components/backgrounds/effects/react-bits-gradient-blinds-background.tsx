...
import type { BackgroundEffectProps, ReactBitsGradientBlindsOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]
type GradientBlindsBlendMode = NonNullable<CSSProperties["mixBlendMode"]>
type ResolvedGradientBlindsOptions = Omit<Required<ReactBitsGradientBlindsOptions>, "gradientColors" | "mixBlendMode"> & {
...
const DEFAULT_REACT_BITS_GRADIENT_BLINDS: ResolvedGradientBlindsOptions = {
  dpr: 1,
...
export default function ReactBitsGradientBlindsBackground({
  className,
  ...
  return (
    <canvas
      ref={canvasRef}
      aria-hidden="true"
      className={cn(styles.reactBitsGradientBlindsCanvas, className)}
...
function getUniformLocation(
  ...
  if (!location) {
    throw new Error(`Missing React Bits Gradient Blinds uniform: ${name}`)
  }
  return location
}
...
function resolveGradientBlindsOptions(options?: ReactBitsGradientBlindsOptions): ResolvedGradientBlindsOptions {
  return {
    dpr: clampNumber(options?.dpr, DEFAULT_REACT_BITS_GRADIENT_BLINDS.dpr, 0.5, 2),
    gradientColors: normalizeColorList(options?.gradientColors, DEFAULT_REACT_BITS_GRADIENT_BLINDS.gradientColors),
    angle: clampNumber(options?.angle, DEFAULT_REACT_BITS_GRADIENT_BLINDS.angle, -180, 180),
    noise: clampNumber(options?.noise, DEFAULT_REACT_BITS_GRADIENT_BLINDS.noise, 0, 1),
    blindCount: Math.round(clampNumber(options?.blindCount, DEFAULT_REACT_BITS_GRADIENT_BLINDS.blindCount, 1, 80)),
    blindMinWidth: clampNumber(options?.blindMinWidth, DEFAULT_REACT_BITS_GRADIENT_BLINDS.blindMinWidth, 0, 240),
    mouseDampening: clampNumber(
      options?.mouseDampening,
      DEFAULT_REACT_BITS_GRADIENT_BLINDS.mouseDampening,
      0,
      1,
    ),
    ...
    spotlightRadius: clampNumber(
      ...
      DEFAULT_REACT_BITS_GRADIENT_BLINDS.spotlightRadius,
      0.05,
      1.5,
    ),
    spotlightSoftness: clampNumber(
      options?.spotlightSoftness,
      DEFAULT_REACT_BITS_GRADIENT_BLINDS.spotlightSoftness,
      0.2,
      4,
    ),
    spotlightOpacity: clampNumber(
      options?.spotlightOpacity,
      DEFAULT_REACT_BITS_GRADIENT_BLINDS.spotlightOpacity,
      0,
      2,
    ),
    distortAmount: clampNumber(options?.distortAmount, DEFAULT_REACT_BITS_GRADIENT_BLINDS.distortAmount, 0, 5),
    shineDirection: options?.shineDirection === "right" ? "right" : "left",
    mixBlendMode: normalizeBlendMode(options?.mixBlendMode, DEFAULT_REACT_BITS_GRADIENT_BLINDS.mixBlendMode),
