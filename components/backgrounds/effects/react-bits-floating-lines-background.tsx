...
import type { BackgroundEffectProps, ReactBitsFloatingLinesOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]
type ResolvedFloatingLinesOptions = Required<Omit<ReactBitsFloatingLinesOptions, "linesGradient">> & {
...
const DEFAULT_REACT_BITS_FLOATING_LINES: ResolvedFloatingLinesOptions = {
  linesGradient: [],
  enableTop: true,
  enableMiddle: true,
  enableBottom: true,
...
// React Bits Floating Lines ships as a Three.js ShaderMaterial. MassageLab
// keeps the shader model and interaction math while owning the WebGL shell.
export default function ReactBitsFloatingLinesBackground({
  className,
  reactBitsFloatingLines,
}: BackgroundEffectProps) {
  const containerRef = useRef<HTMLDivElement | null>(null)
  const canvasRef = useRef<HTMLCanvasElement | null>(null)
  const gradientKey = reactBitsFloatingLines?.linesGradient?.join("|") ?? ""
  const options = useMemo(
    () => resolveFloatingLinesOptions(reactBitsFloatingLines),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [
      gradientKey,
      reactBitsFloatingLines?.enableTop,
      reactBitsFloatingLines?.enableMiddle,
      reactBitsFloatingLines?.enableBottom,
      reactBitsFloatingLines?.topLineCount,
      reactBitsFloatingLines?.middleLineCount,
      reactBitsFloatingLines?.bottomLineCount,
      reactBitsFloatingLines?.topLineDistance,
      reactBitsFloatingLines?.middleLineDistance,
      reactBitsFloatingLines?.bottomLineDistance,
      reactBitsFloatingLines?.topWaveX,
      reactBitsFloatingLines?.topWaveY,
      reactBitsFloatingLines?.topWaveRotate,
      reactBitsFloatingLines?.middleWaveX,
      reactBitsFloatingLines?.middleWaveY,
      reactBitsFloatingLines?.middleWaveRotate,
      reactBitsFloatingLines?.bottomWaveX,
      reactBitsFloatingLines?.bottomWaveY,
      reactBitsFloatingLines?.bottomWaveRotate,
      reactBitsFloatingLines?.animationSpeed,
      reactBitsFloatingLines?.interactive,
      reactBitsFloatingLines?.bendRadius,
      reactBitsFloatingLines?.bendStrength,
      reactBitsFloatingLines?.mouseDamping,
      reactBitsFloatingLines?.parallax,
      reactBitsFloatingLines?.parallaxStrength,
      reactBitsFloatingLines?.mixBlendMode,
    ],
  )
  ...
  return (
    <div
      ref={containerRef}
      className={cn(styles.effectLayer, styles.reactBitsFloatingLines, className)}
      style={{ mixBlendMode: options.mixBlendMode } as CSSProperties}
      aria-hidden="true"
    >
      <canvas ref={canvasRef} className={styles.reactBitsFloatingLinesCanvas} />
    </div>
  )
}
...
function getUniformLocation(
  ...
  if (!location) {
    throw new Error(`Missing React Bits Floating Lines shader uniform: ${name}`)
  }
...
function resolveFloatingLinesOptions(
  options: ReactBitsFloatingLinesOptions | undefined,
): ResolvedFloatingLinesOptions {
  return {
    linesGradient: resolveGradient(options?.linesGradient),
    enableTop: resolveBoolean(options?.enableTop, DEFAULT_REACT_BITS_FLOATING_LINES.enableTop),
    enableMiddle: resolveBoolean(options?.enableMiddle, DEFAULT_REACT_BITS_FLOATING_LINES.enableMiddle),
    enableBottom: resolveBoolean(options?.enableBottom, DEFAULT_REACT_BITS_FLOATING_LINES.enableBottom),
    topLineCount: Math.trunc(resolveNumber(options?.topLineCount, DEFAULT_REACT_BITS_FLOATING_LINES.topLineCount, 0, MAX_LINE_COUNT)),
    middleLineCount: Math.trunc(resolveNumber(options?.middleLineCount, DEFAULT_REACT_BITS_FLOATING_LINES.middleLineCount, 0, MAX_LINE_COUNT)),
    bottomLineCount: Math.trunc(resolveNumber(options?.bottomLineCount, DEFAULT_REACT_BITS_FLOATING_LINES.bottomLineCount, 0, MAX_LINE_COUNT)),
    topLineDistance: resolveNumber(options?.topLineDistance, DEFAULT_REACT_BITS_FLOATING_LINES.topLineDistance, 0.1, 20),
    middleLineDistance: resolveNumber(options?.middleLineDistance, DEFAULT_REACT_BITS_FLOATING_LINES.middleLineDistance, 0.1, 20),
    bottomLineDistance: resolveNumber(options?.bottomLineDistance, DEFAULT_REACT_BITS_FLOATING_LINES.bottomLineDistance, 0.1, 20),
    topWaveX: resolveNumber(options?.topWaveX, DEFAULT_REACT_BITS_FLOATING_LINES.topWaveX, -20, 20),
    topWaveY: resolveNumber(options?.topWaveY, DEFAULT_REACT_BITS_FLOATING_LINES.topWaveY, -4, 4),
    topWaveRotate: resolveNumber(options?.topWaveRotate, DEFAULT_REACT_BITS_FLOATING_LINES.topWaveRotate, -4, 4),
    middleWaveX: resolveNumber(options?.middleWaveX, DEFAULT_REACT_BITS_FLOATING_LINES.middleWaveX, -20, 20),
    middleWaveY: resolveNumber(options?.middleWaveY, DEFAULT_REACT_BITS_FLOATING_LINES.middleWaveY, -4, 4),
    middleWaveRotate: resolveNumber(options?.middleWaveRotate, DEFAULT_REACT_BITS_FLOATING_LINES.middleWaveRotate, -4, 4),
    bottomWaveX: resolveNumber(options?.bottomWaveX, DEFAULT_REACT_BITS_FLOATING_LINES.bottomWaveX, -20, 20),
    bottomWaveY: resolveNumber(options?.bottomWaveY, DEFAULT_REACT_BITS_FLOATING_LINES.bottomWaveY, -4, 4),
    bottomWaveRotate: resolveNumber(options?.bottomWaveRotate, DEFAULT_REACT_BITS_FLOATING_LINES.bottomWaveRotate, -4, 4),
    animationSpeed: resolveNumber(options?.animationSpeed, DEFAULT_REACT_BITS_FLOATING_LINES.animationSpeed, 0, 4),
    interactive: resolveBoolean(options?.interactive, DEFAULT_REACT_BITS_FLOATING_LINES.interactive),
    bendRadius: resolveNumber(options?.bendRadius, DEFAULT_REACT_BITS_FLOATING_LINES.bendRadius, 0.1, 20),
    bendStrength: resolveNumber(options?.bendStrength, DEFAULT_REACT_BITS_FLOATING_LINES.bendStrength, -2, 2),
    mouseDamping: resolveNumber(options?.mouseDamping, DEFAULT_REACT_BITS_FLOATING_LINES.mouseDamping, 0.01, 1),
    parallax: resolveBoolean(options?.parallax, DEFAULT_REACT_BITS_FLOATING_LINES.parallax),
    parallaxStrength: resolveNumber(options?.parallaxStrength, DEFAULT_REACT_BITS_FLOATING_LINES.parallaxStrength, 0, 1),
