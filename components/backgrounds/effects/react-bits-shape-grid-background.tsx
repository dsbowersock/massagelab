...
import type { BackgroundEffectProps, ReactBitsShapeGridOptions } from "./css-backgrounds"
...
type ResolvedShapeGridOptions = Required<ReactBitsShapeGridOptions> & {
  shape: ShapeGridShape
  direction: ShapeGridDirection
}

const DEFAULT_REACT_BITS_SHAPE_GRID: ResolvedShapeGridOptions = {
  direction: "right",
  speed: 1,
  borderColor: "#999999",
  squareSize: 40,
...
// React Bits Shape Grid is a moving canvas grid. This port keeps the source
// shape drawing, directional wrap, hover cell, and trail-opacity model.
export default function ReactBitsShapeGridBackground({
  className,
  reactBitsShapeGrid,
  ...
  return (
    <canvas
      ref={canvasRef}
      aria-hidden="true"
      className={cn(styles.reactBitsShapeGridCanvas, className)}
    />
  )
}

function resolveShapeGridOptions(options?: ReactBitsShapeGridOptions): ResolvedShapeGridOptions {
  return {
    direction: resolveDirection(options?.direction),
    speed: resolveNumber(options?.speed, DEFAULT_REACT_BITS_SHAPE_GRID.speed, 0.1, 8),
    borderColor: resolveHex(options?.borderColor, DEFAULT_REACT_BITS_SHAPE_GRID.borderColor),
    squareSize: resolveNumber(options?.squareSize, DEFAULT_REACT_BITS_SHAPE_GRID.squareSize, 12, 96),
    hoverFillColor: resolveHex(options?.hoverFillColor, DEFAULT_REACT_BITS_SHAPE_GRID.hoverFillColor),
    shape: resolveShape(options?.shape),
    hoverTrailAmount: Math.trunc(resolveNumber(
      options?.hoverTrailAmount,
      DEFAULT_REACT_BITS_SHAPE_GRID.hoverTrailAmount,
      0,
      24,
    )),
    cursorInteraction: options?.cursorInteraction ?? DEFAULT_REACT_BITS_SHAPE_GRID.cursorInteraction,
  }
}
