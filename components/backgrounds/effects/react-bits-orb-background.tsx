...
import type { BackgroundEffectProps, ReactBitsOrbOptions } from "./css-backgrounds"

type ResolvedOrbOptions = Required<ReactBitsOrbOptions>
...
const DEFAULT_REACT_BITS_ORB: ResolvedOrbOptions = {
  hue: 0,
  hoverIntensity: 0.2,
  rotateOnHover: true,
...
// React Bits Orb is an OGL shader. This keeps the source hue, hover, rotation,
// and background uniforms while using owned raw WebGL setup and cleanup.
export default function ReactBitsOrbBackground({
  className,
  reactBitsOrb,
  ...
  return (
    <canvas
      ref={canvasRef}
      aria-hidden="true"
      className={cn(styles.reactBitsOrbCanvas, className)}
    />
  )
}
...
function getUniformLocation(gl: WebGLRenderingContext, program: WebGLProgram, name: string) {
  ...
  if (!location) {
    throw new Error(`Missing React Bits Orb uniform: ${name}`)
  }
  return location
}

function resolveOrbOptions(options?: ReactBitsOrbOptions): ResolvedOrbOptions {
  return {
    hue: resolveNumber(options?.hue, DEFAULT_REACT_BITS_ORB.hue, 0, 360),
    hoverIntensity: resolveNumber(options?.hoverIntensity, DEFAULT_REACT_BITS_ORB.hoverIntensity, 0, 1),
    rotateOnHover: options?.rotateOnHover ?? DEFAULT_REACT_BITS_ORB.rotateOnHover,
    forceHoverState: options?.forceHoverState ?? DEFAULT_REACT_BITS_ORB.forceHoverState,
    backgroundColor: resolveHex(options?.backgroundColor, DEFAULT_REACT_BITS_ORB.backgroundColor),
    cursorInteraction: options?.cursorInteraction ?? DEFAULT_REACT_BITS_ORB.cursorInteraction,
  }
}
