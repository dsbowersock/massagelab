...
import type { BackgroundEffectProps, ReactBitsRadarOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]
type ResolvedRadarOptions = Required<ReactBitsRadarOptions>
...
const DEFAULT_REACT_BITS_RADAR: ResolvedRadarOptions = {
  speed: 1,
  scale: 0.5,
  ringCount: 10,
  spokeCount: 10,
...
// React Bits Radar ships as an OGL full-screen shader. MassageLab keeps the
// source ring, spoke, sweep, falloff, and optional mouse-offset math in WebGL.
export default function ReactBitsRadarBackground({
  className,
  reactBitsRadar,
  ...
  return (
    <div className={cn(styles.effectLayer, styles.reactBitsRadar, className)} ref={containerRef}>
      <canvas className={styles.reactBitsRadarCanvas} ref={canvasRef} />
    </div>
  )
}
...
function getUniformLocation(
  ...
  if (!location) {
    throw new Error(`Missing React Bits Radar uniform: ${name}`)
  }

  return location
}

function resolveRadarOptions(options?: ReactBitsRadarOptions): ResolvedRadarOptions {
  return {
    speed: clampNumber(options?.speed, DEFAULT_REACT_BITS_RADAR.speed, 0, 3),
    scale: clampNumber(options?.scale, DEFAULT_REACT_BITS_RADAR.scale, 0.1, 2),
    ringCount: clampNumber(options?.ringCount, DEFAULT_REACT_BITS_RADAR.ringCount, 1, 40),
    spokeCount: clampNumber(options?.spokeCount, DEFAULT_REACT_BITS_RADAR.spokeCount, 1, 40),
    ringThickness: clampNumber(options?.ringThickness, DEFAULT_REACT_BITS_RADAR.ringThickness, 0.001, 0.25),
    spokeThickness: clampNumber(options?.spokeThickness, DEFAULT_REACT_BITS_RADAR.spokeThickness, 0.001, 0.1),
    sweepSpeed: clampNumber(options?.sweepSpeed, DEFAULT_REACT_BITS_RADAR.sweepSpeed, 0, 4),
    sweepWidth: clampNumber(options?.sweepWidth, DEFAULT_REACT_BITS_RADAR.sweepWidth, 0.1, 12),
    sweepLobes: clampNumber(options?.sweepLobes, DEFAULT_REACT_BITS_RADAR.sweepLobes, 1, 12),
    color: normalizeHexColor(options?.color, DEFAULT_REACT_BITS_RADAR.color),
    backgroundColor: normalizeHexColor(options?.backgroundColor, DEFAULT_REACT_BITS_RADAR.backgroundColor),
    falloff: clampNumber(options?.falloff, DEFAULT_REACT_BITS_RADAR.falloff, 0, 8),
    brightness: clampNumber(options?.brightness, DEFAULT_REACT_BITS_RADAR.brightness, 0, 3),
    enableMouseInteraction: typeof options?.enableMouseInteraction === "boolean"
      ? options.enableMouseInteraction
      : DEFAULT_REACT_BITS_RADAR.enableMouseInteraction,
    mouseInfluence: clampNumber(options?.mouseInfluence, DEFAULT_REACT_BITS_RADAR.mouseInfluence, 0, 1),
  }
}
