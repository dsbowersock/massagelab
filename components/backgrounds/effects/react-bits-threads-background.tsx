...
import type { BackgroundEffectProps, ReactBitsThreadsOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]
type ResolvedThreadsOptions = Required<ReactBitsThreadsOptions>
...
const DEFAULT_REACT_BITS_THREADS: ResolvedThreadsOptions = {
  color: "#FFFFFF",
  amplitude: 1,
  distance: 0,
...
// React Bits Threads ships as an OGL shader. MassageLab keeps the source
// Perlin thread field and mouse smoothing in raw WebGL without adding OGL.
export default function ReactBitsThreadsBackground({
  className,
  reactBitsThreads,
  ...
  return (
    <div className={cn(styles.effectLayer, styles.reactBitsThreads, className)} ref={containerRef}>
      <canvas className={styles.reactBitsThreadsCanvas} ref={canvasRef} />
    </div>
  )
}
...
function getUniformLocation(
  ...
  if (!location) {
    throw new Error(`Missing React Bits Threads uniform: ${name}`)
  }

  return location
}

function resolveThreadsOptions(options?: ReactBitsThreadsOptions): ResolvedThreadsOptions {
  return {
    color: normalizeHexColor(options?.color, DEFAULT_REACT_BITS_THREADS.color),
    amplitude: clampNumber(options?.amplitude, DEFAULT_REACT_BITS_THREADS.amplitude, 0, 3),
    distance: clampNumber(options?.distance, DEFAULT_REACT_BITS_THREADS.distance, -1, 1.5),
    enableMouseInteraction: typeof options?.enableMouseInteraction === "boolean"
      ? options.enableMouseInteraction
      : DEFAULT_REACT_BITS_THREADS.enableMouseInteraction,
  }
}
