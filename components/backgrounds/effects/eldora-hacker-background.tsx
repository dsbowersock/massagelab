...
import type { BackgroundEffectProps, EldoraHackerOptions } from "./css-backgrounds"

type ResolvedHackerOptions = Required<EldoraHackerOptions>

const DEFAULT_ELDORA_HACKER: ResolvedHackerOptions = {
  color: "#00FF00",
  fontSize: 14,
  speed: 1,
}

const HACKER_CHARACTERS = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789@#$%^&*()_+"
const SOURCE_FRAME_INTERVAL_MS = 33

// Eldora UI's source is a 2D Matrix-rain canvas. MassageLab keeps that source
// shape while adapting sizing and lifecycle cleanup to the shared background host.
export default function EldoraHackerBackground({
  className,
  eldoraHacker,
}: BackgroundEffectProps) {
  const containerRef = useRef<HTMLDivElement | null>(null)
  const canvasRef = useRef<HTMLCanvasElement | null>(null)
  const color = eldoraHacker?.color
  const fontSize = eldoraHacker?.fontSize
  const speed = eldoraHacker?.speed
  const options = useMemo(
    () => resolveHackerOptions({
      color,
      fontSize,
      speed,
    }),
  ...
  useEffect(() => {
    ...
    const drawMatrix = (advance: boolean) => {
      ...
      for (let column = 0; column < drops.length; column += 1) {
        ...
        const text = HACKER_CHARACTERS[Math.floor(random() * HACKER_CHARACTERS.length)]
  ...
  return (
    <div
      ref={containerRef}
      className={cn(styles.effectLayer, styles.eldoraHackerBackground, className)}
      aria-hidden="true"
    >
      <canvas ref={canvasRef} className={styles.eldoraHackerCanvas} />
    </div>
  )
}

function resolveHackerOptions(options: EldoraHackerOptions): ResolvedHackerOptions {
  return {
    color: normalizeHexColor(options.color, DEFAULT_ELDORA_HACKER.color),
    fontSize: clampNumber(options.fontSize, DEFAULT_ELDORA_HACKER.fontSize, 8, 28),
    speed: clampNumber(options.speed, DEFAULT_ELDORA_HACKER.speed, 0.05, 3),
  }
}
