...
import type { BackgroundEffectProps, ReactBitsGridDistortionOptions } from "./css-backgrounds"

type ResolvedGridDistortionOptions = Required<ReactBitsGridDistortionOptions>
...
const DEFAULT_REACT_BITS_GRID_DISTORTION: ResolvedGridDistortionOptions = {
  grid: 15,
  mouse: 0.1,
  strength: 0.15,
  relaxation: 0.9,
...
// React Bits Grid Distortion uses a Three.js plane, image texture, and float
// data texture. This port keeps that shader/data-texture model in raw WebGL.
export default function ReactBitsGridDistortionBackground({
  className,
  ...
  return (
    <div className={cn(styles.effectLayer, styles.reactBitsGridDistortion, className)} ref={containerRef}>
      <canvas className={styles.reactBitsGridDistortionCanvas} ref={canvasRef} />
    </div>
  )
}
...
function createGridDistortionResources(
  ...
  if (!program) {
    throw new Error("Unable to create React Bits Grid Distortion program")
  }
  ...
  if (!context.getProgramParameter(program, context.LINK_STATUS)) {
    throw new Error(context.getProgramInfoLog(program) ?? "Unable to link React Bits Grid Distortion program")
  }
  ...
  if (!dataTexture) {
    throw new Error("Unable to create React Bits Grid Distortion data texture")
  }
...
function createImageTexture(context: WebGLRenderingContext, options: ResolvedGridDistortionOptions) {
  ...
  if (!textureContext) {
    throw new Error("Unable to create React Bits Grid Distortion image texture canvas")
  }
  ...
  if (!texture) {
    throw new Error("Unable to create React Bits Grid Distortion image texture")
  }
...
function createBuffer(context: WebGLRenderingContext, data: Float32Array) {
  ...
  if (!buffer) {
    throw new Error("Unable to create React Bits Grid Distortion buffer")
  }
...
function compileShader(context: WebGLRenderingContext, type: number, source: string) {
  ...
  if (!shader) {
    throw new Error("Unable to create React Bits Grid Distortion shader")
  }
  context.shaderSource(shader, source)
  context.compileShader(shader)
  if (!context.getShaderParameter(shader, context.COMPILE_STATUS)) {
    throw new Error(context.getShaderInfoLog(shader) ?? "Unable to compile React Bits Grid Distortion shader")
  }
  return shader
}
...
function getUniformLocation(context: WebGLRenderingContext, program: WebGLProgram, name: string) {
  ...
  if (!location) {
    throw new Error(`Missing React Bits Grid Distortion uniform: ${name}`)
  }
  return location
}

function resolveGridDistortionOptions(options?: ReactBitsGridDistortionOptions): ResolvedGridDistortionOptions {
  return {
    grid: Math.round(resolveNumber(options?.grid, DEFAULT_REACT_BITS_GRID_DISTORTION.grid, 4, 40)),
    mouse: resolveNumber(options?.mouse, DEFAULT_REACT_BITS_GRID_DISTORTION.mouse, 0.02, 0.5),
    strength: resolveNumber(options?.strength, DEFAULT_REACT_BITS_GRID_DISTORTION.strength, 0, 0.6),
    relaxation: resolveNumber(options?.relaxation, DEFAULT_REACT_BITS_GRID_DISTORTION.relaxation, 0.75, 0.99),
    colorOne: resolveHex(options?.colorOne, DEFAULT_REACT_BITS_GRID_DISTORTION.colorOne),
    colorTwo: resolveHex(options?.colorTwo, DEFAULT_REACT_BITS_GRID_DISTORTION.colorTwo),
    colorThree: resolveHex(options?.colorThree, DEFAULT_REACT_BITS_GRID_DISTORTION.colorThree),
    cursorInteraction: options?.cursorInteraction ?? DEFAULT_REACT_BITS_GRID_DISTORTION.cursorInteraction,
  }
}
