...
import type { BackgroundEffectProps, ReactBitsLightRaysOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]
type LightRaysOrigin = NonNullable<ReactBitsLightRaysOptions["raysOrigin"]>
type ResolvedLightRaysOptions = Required<ReactBitsLightRaysOptions>
...
const DEFAULT_REACT_BITS_LIGHT_RAYS: ResolvedLightRaysOptions = {
...
// React Bits Light Rays ships as an OGL full-screen shader. MassageLab ports
// the source shader, placement helper, and optional mouse-follow math directly.
export default function ReactBitsLightRaysBackground({
  className,
  reactBitsLightRays,
  ...
  return (
    <div
      ref={containerRef}
      className={cn(styles.effectLayer, styles.reactBitsLightRays, className)}
      aria-hidden="true"
    >
      <canvas ref={canvasRef} className={styles.reactBitsLightRaysCanvas} />
    </div>
  )
}
...
function getUniformLocation(
  ...
  if (!location) {
    throw new Error(`Missing React Bits Light Rays shader uniform: ${name}`)
  }
...
function resolveLightRaysOptions(options: ReactBitsLightRaysOptions | undefined): ResolvedLightRaysOptions {
  return {
    raysOrigin: resolveOrigin(options?.raysOrigin),
    raysColor: resolveHexColor(options?.raysColor, DEFAULT_REACT_BITS_LIGHT_RAYS.raysColor),
    raysSpeed: resolveNumber(options?.raysSpeed, DEFAULT_REACT_BITS_LIGHT_RAYS.raysSpeed, 0, 4),
    lightSpread: resolveNumber(options?.lightSpread, DEFAULT_REACT_BITS_LIGHT_RAYS.lightSpread, 0.1, 4),
    rayLength: resolveNumber(options?.rayLength, DEFAULT_REACT_BITS_LIGHT_RAYS.rayLength, 0.25, 5),
    pulsating: options?.pulsating === true,
    fadeDistance: resolveNumber(options?.fadeDistance, DEFAULT_REACT_BITS_LIGHT_RAYS.fadeDistance, 0.1, 3),
    saturation: resolveNumber(options?.saturation, DEFAULT_REACT_BITS_LIGHT_RAYS.saturation, 0, 3),
    followMouse: options?.followMouse === true,
    mouseInfluence: resolveNumber(options?.mouseInfluence, DEFAULT_REACT_BITS_LIGHT_RAYS.mouseInfluence, 0, 1),
    noiseAmount: resolveNumber(options?.noiseAmount, DEFAULT_REACT_BITS_LIGHT_RAYS.noiseAmount, 0, 1),
    distortion: resolveNumber(options?.distortion, DEFAULT_REACT_BITS_LIGHT_RAYS.distortion, 0, 2),
  }
}

function resolveOrigin(value: ReactBitsLightRaysOptions["raysOrigin"] | undefined): LightRaysOrigin {
  if (
    value === "top-left"
    || value === "top-center"
    || value === "top-right"
  ...
  return DEFAULT_REACT_BITS_LIGHT_RAYS.raysOrigin
}
...
function parseHexColorToRgb(color: string): RgbColor {
  const normalized = resolveHexColor(color, DEFAULT_REACT_BITS_LIGHT_RAYS.raysColor).replace("#", "")
  return [
