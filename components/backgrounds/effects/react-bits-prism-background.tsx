...
import type { BackgroundEffectProps, ReactBitsPrismOptions } from "./css-backgrounds"

type PrismAnimationType = NonNullable<ReactBitsPrismOptions["animationType"]>

type ResolvedPrismOptions = Required<ReactBitsPrismOptions>
...
const DEFAULT_REACT_BITS_PRISM: ResolvedPrismOptions = {
  height: 3.5,
  baseWidth: 5.5,
...
// React Bits Prism ships as an OGL component. MassageLab ports the same ray-marched
// prism shader and source rotation logic directly to native WebGL.
export default function ReactBitsPrismBackground({
  className,
  reactBitsPrism,
}: BackgroundEffectProps) {
  const containerRef = useRef<HTMLDivElement | null>(null)
  const canvasRef = useRef<HTMLCanvasElement | null>(null)
  const height = reactBitsPrism?.height
  const baseWidth = reactBitsPrism?.baseWidth
  const animationType = reactBitsPrism?.animationType
  const glow = reactBitsPrism?.glow
  const offsetX = reactBitsPrism?.offsetX
  const offsetY = reactBitsPrism?.offsetY
  const noise = reactBitsPrism?.noise
  const transparent = reactBitsPrism?.transparent
  const scale = reactBitsPrism?.scale
  const hueShift = reactBitsPrism?.hueShift
  const colorFrequency = reactBitsPrism?.colorFrequency
  const hoverStrength = reactBitsPrism?.hoverStrength
  const inertia = reactBitsPrism?.inertia
  const bloom = reactBitsPrism?.bloom
  const timeScale = reactBitsPrism?.timeScale
  ...
  return (
    <div
      ref={containerRef}
      className={cn(styles.effectLayer, styles.reactBitsPrism, className)}
      aria-hidden="true"
    >
      <canvas ref={canvasRef} className={styles.reactBitsPrismCanvas} />
    </div>
  )
}
...
function getUniformLocation(context: WebGLRenderingContext, program: WebGLProgram, name: string) {
  ...
  if (location === null) {
    throw new Error(`Missing React Bits Prism shader uniform: ${name}`)
  }

  return location
}

function resolvePrismOptions(options: ReactBitsPrismOptions | undefined): ResolvedPrismOptions {
  const height = clampNumber(options?.height, DEFAULT_REACT_BITS_PRISM.height, 0.5, 8)
  const baseWidth = clampNumber(options?.baseWidth, DEFAULT_REACT_BITS_PRISM.baseWidth, 0.5, 10)

  return {
    height,
    baseWidth,
    animationType: resolveAnimationType(options?.animationType),
    glow: clampNumber(options?.glow, DEFAULT_REACT_BITS_PRISM.glow, 0, 3),
    offsetX: clampNumber(options?.offsetX, DEFAULT_REACT_BITS_PRISM.offsetX, -400, 400),
    offsetY: clampNumber(options?.offsetY, DEFAULT_REACT_BITS_PRISM.offsetY, -400, 400),
    noise: clampNumber(options?.noise, DEFAULT_REACT_BITS_PRISM.noise, 0, 1),
    transparent:
      typeof options?.transparent === "boolean"
        ? options.transparent
        : DEFAULT_REACT_BITS_PRISM.transparent,
    scale: clampNumber(options?.scale, DEFAULT_REACT_BITS_PRISM.scale, 0.5, 7),
    hueShift: clampNumber(options?.hueShift, DEFAULT_REACT_BITS_PRISM.hueShift, -Math.PI, Math.PI),
    colorFrequency: clampNumber(options?.colorFrequency, DEFAULT_REACT_BITS_PRISM.colorFrequency, 0.1, 3),
    hoverStrength: clampNumber(options?.hoverStrength, DEFAULT_REACT_BITS_PRISM.hoverStrength, 0, 4),
    inertia: clampNumber(options?.inertia, DEFAULT_REACT_BITS_PRISM.inertia, 0.01, 0.4),
    bloom: clampNumber(options?.bloom, DEFAULT_REACT_BITS_PRISM.bloom, 0, 3),
    timeScale: clampNumber(options?.timeScale, DEFAULT_REACT_BITS_PRISM.timeScale, 0, 2),
  }
}
...
function resolveAnimationType(value: PrismAnimationType | undefined): PrismAnimationType {
  return value === "hover" || value === "3drotate" || value === "rotate"
    ? value
    : DEFAULT_REACT_BITS_PRISM.animationType
}
