...
import type { BackgroundEffectProps, ChamaacGridBloomOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]

type ResolvedGridBloomOptions = Required<ChamaacGridBloomOptions>
...
const DEFAULT_CHAMAAC_GRID_BLOOM: ResolvedGridBloomOptions = {
  color: "#E040FB",
  speed: 1,
  gridScale: 12,
  rotationSpeed: 0,
...
// Chamaac UI Grid Bloom ships as a Three/R3F shader. MassageLab keeps the
// source's passive shader uniforms in a native WebGL background layer.
export default function ChamaacGridBloomBackground({
  className,
  chamaacGridBloom,
}: BackgroundEffectProps) {
  const containerRef = useRef<HTMLDivElement | null>(null)
  const canvasRef = useRef<HTMLCanvasElement | null>(null)
  const color = chamaacGridBloom?.color
  const speed = chamaacGridBloom?.speed
  const gridScale = chamaacGridBloom?.gridScale
  const rotationSpeed = chamaacGridBloom?.rotationSpeed
  const fadeFalloff = chamaacGridBloom?.fadeFalloff
  const distortionAmount = chamaacGridBloom?.distortionAmount
  const flowSpeedX = chamaacGridBloom?.flowSpeedX
  const flowSpeedY = chamaacGridBloom?.flowSpeedY
  ...
  return (
    <div
      ref={containerRef}
      className={cn(styles.effectLayer, styles.chamaacGridBloom, className)}
      aria-hidden="true"
    >
      <canvas ref={canvasRef} className={styles.chamaacGridBloomCanvas} />
    </div>
  )
}
...
function getUniformLocation(context: WebGLRenderingContext, program: WebGLProgram, name: string) {
  ...
  if (location === null) {
    throw new Error(`Missing Chamaac Grid Bloom shader uniform: ${name}`)
  }

  return location
}

function resolveGridBloomOptions(options: ChamaacGridBloomOptions): ResolvedGridBloomOptions {
  return {
    color: normalizeHexColor(options.color, DEFAULT_CHAMAAC_GRID_BLOOM.color),
    speed: clampNumber(options.speed, DEFAULT_CHAMAAC_GRID_BLOOM.speed, 0.1, 3),
    gridScale: clampNumber(options.gridScale, DEFAULT_CHAMAAC_GRID_BLOOM.gridScale, 4, 32),
    rotationSpeed: clampNumber(options.rotationSpeed, DEFAULT_CHAMAAC_GRID_BLOOM.rotationSpeed, -3, 3),
    fadeFalloff: clampNumber(options.fadeFalloff, DEFAULT_CHAMAAC_GRID_BLOOM.fadeFalloff, 1, 24),
    distortionAmount: clampNumber(options.distortionAmount, DEFAULT_CHAMAAC_GRID_BLOOM.distortionAmount, 0, 0.5),
    flowSpeedX: clampNumber(options.flowSpeedX, DEFAULT_CHAMAAC_GRID_BLOOM.flowSpeedX, -2, 2),
    flowSpeedY: clampNumber(options.flowSpeedY, DEFAULT_CHAMAAC_GRID_BLOOM.flowSpeedY, -2, 2),
  }
}
