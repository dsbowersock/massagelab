...
import type { BackgroundEffectProps, ReactBitsLineWavesOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]
type ResolvedLineWavesOptions = Required<ReactBitsLineWavesOptions>
...
const DEFAULT_REACT_BITS_LINE_WAVES: ResolvedLineWavesOptions = {
  speed: 0.3,
  innerLineCount: 32,
  outerLineCount: 36,
  warpIntensity: 1,
...
// React Bits Line Waves ships as an OGL full-screen shader. MassageLab keeps
// the source displacement, line, color-cycle, and mouse-warp math in raw WebGL.
export default function ReactBitsLineWavesBackground({
  className,
  reactBitsLineWaves,
  ...
  return (
    <div className={cn(styles.effectLayer, styles.reactBitsLineWaves, className)} ref={containerRef}>
      <canvas className={styles.reactBitsLineWavesCanvas} ref={canvasRef} />
    </div>
  )
}
...
function getUniformLocation(
  ...
  if (!location) {
    throw new Error(`Missing React Bits Line Waves uniform: ${name}`)
  }

  return location
}

function resolveLineWavesOptions(options?: ReactBitsLineWavesOptions): ResolvedLineWavesOptions {
  return {
    speed: clampNumber(options?.speed, DEFAULT_REACT_BITS_LINE_WAVES.speed, 0, 3),
    innerLineCount: clampNumber(options?.innerLineCount, DEFAULT_REACT_BITS_LINE_WAVES.innerLineCount, 1, 96),
    outerLineCount: clampNumber(options?.outerLineCount, DEFAULT_REACT_BITS_LINE_WAVES.outerLineCount, 1, 96),
    warpIntensity: clampNumber(options?.warpIntensity, DEFAULT_REACT_BITS_LINE_WAVES.warpIntensity, 0, 3),
    rotation: clampNumber(options?.rotation, DEFAULT_REACT_BITS_LINE_WAVES.rotation, -180, 180),
    edgeFadeWidth: clampNumber(options?.edgeFadeWidth, DEFAULT_REACT_BITS_LINE_WAVES.edgeFadeWidth, -1, 1),
    colorCycleSpeed: clampNumber(options?.colorCycleSpeed, DEFAULT_REACT_BITS_LINE_WAVES.colorCycleSpeed, 0, 4),
    brightness: clampNumber(options?.brightness, DEFAULT_REACT_BITS_LINE_WAVES.brightness, 0, 1.5),
    color1: normalizeHexColor(options?.color1, DEFAULT_REACT_BITS_LINE_WAVES.color1),
    color2: normalizeHexColor(options?.color2, DEFAULT_REACT_BITS_LINE_WAVES.color2),
    color3: normalizeHexColor(options?.color3, DEFAULT_REACT_BITS_LINE_WAVES.color3),
    enableMouseInteraction: typeof options?.enableMouseInteraction === "boolean"
      ? options.enableMouseInteraction
      : DEFAULT_REACT_BITS_LINE_WAVES.enableMouseInteraction,
    mouseInfluence: clampNumber(options?.mouseInfluence, DEFAULT_REACT_BITS_LINE_WAVES.mouseInfluence, 0, 4),
  }
}
