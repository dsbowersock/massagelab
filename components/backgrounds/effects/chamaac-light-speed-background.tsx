...
import type { BackgroundEffectProps, ChamaacLightSpeedOptions } from "./css-backgrounds"
...
type ResolvedLightSpeedOptions = Required<ChamaacLightSpeedOptions>

const DEFAULT_CHAMAAC_LIGHT_SPEED: ResolvedLightSpeedOptions = {
  particleCount: 200,
  warpSpeed: 1,
  lightColor: "#B026FF",
  intensity: 3,
...
const CHAMAAC_LIGHT_SPEED_RENDER_SCALE = 0.1
const TWO_PI = Math.PI * 2

// Chamaac UI Light Speed uses a WebGL/R3F cylinder of stretched particles. This
// canvas adaptation keeps the same public prop model without adding that stack.
export default function ChamaacLightSpeedBackground({
  className,
  chamaacLightSpeed,
}: BackgroundEffectProps) {
  const containerRef = useRef<HTMLDivElement | null>(null)
  const canvasRef = useRef<HTMLCanvasElement | null>(null)
  const particleCount = chamaacLightSpeed?.particleCount
  const warpSpeed = chamaacLightSpeed?.warpSpeed
  const lightColor = chamaacLightSpeed?.lightColor
  const intensity = chamaacLightSpeed?.intensity
  const radius = chamaacLightSpeed?.radius
  const cylinderLength = chamaacLightSpeed?.cylinderLength
  ...
  useEffect(() => {
    ...
    const drawParticles = (elapsedSeconds: number, shouldMove: boolean) => {
      ...
      const renderWarpSpeed = options.warpSpeed * CHAMAAC_LIGHT_SPEED_RENDER_SCALE
  ...
  return (
    <div
      ref={containerRef}
      className={cn(styles.effectLayer, styles.chamaacLightSpeed, className)}
      aria-hidden="true"
    >
      <canvas ref={canvasRef} className={styles.chamaacLightSpeedCanvas} />
    </div>
  )
}

function resolveLightSpeedOptions(options: ChamaacLightSpeedOptions): ResolvedLightSpeedOptions {
  return {
    particleCount: Math.trunc(clampNumber(
      options.particleCount,
      DEFAULT_CHAMAAC_LIGHT_SPEED.particleCount,
      20,
      200,
    )),
    warpSpeed: clampNumber(options.warpSpeed, DEFAULT_CHAMAAC_LIGHT_SPEED.warpSpeed, 0.1, 24),
    lightColor: normalizeHexColor(options.lightColor, DEFAULT_CHAMAAC_LIGHT_SPEED.lightColor),
    intensity: clampNumber(options.intensity, DEFAULT_CHAMAAC_LIGHT_SPEED.intensity, 0.25, 6),
    radius: clampNumber(options.radius, DEFAULT_CHAMAAC_LIGHT_SPEED.radius, 6, 60),
    cylinderLength: clampNumber(options.cylinderLength, DEFAULT_CHAMAAC_LIGHT_SPEED.cylinderLength, 40, 300),
  }
}
