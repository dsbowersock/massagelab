...
import type { BackgroundEffectProps, ReactBitsLightningOptions } from "./css-backgrounds"

type ResolvedLightningOptions = Required<ReactBitsLightningOptions>
...
const DEFAULT_REACT_BITS_LIGHTNING: ResolvedLightningOptions = {
  hue: 230,
  xOffset: 0,
  speed: 1,
  intensity: 1,
...
// React Bits Lightning already ships as a raw WebGL shader. MassageLab wraps the
// same shader with reduced-motion, resize, visibility, and resource cleanup.
export default function ReactBitsLightningBackground({
  className,
  reactBitsLightning,
  ...
  return (
    <canvas
      ref={canvasRef}
      aria-hidden="true"
      className={cn(styles.reactBitsLightningCanvas, className)}
    />
  )
}
...
function getUniformLocation(
  ...
  if (!location) {
    throw new Error(`Missing React Bits Lightning shader uniform: ${name}`)
  }

  return location
}

function resolveLightningOptions(options: ReactBitsLightningOptions | undefined): ResolvedLightningOptions {
  return {
    hue: resolveNumber(options?.hue, DEFAULT_REACT_BITS_LIGHTNING.hue, 0, 360),
    xOffset: resolveNumber(options?.xOffset, DEFAULT_REACT_BITS_LIGHTNING.xOffset, -2, 2),
    speed: resolveNumber(options?.speed, DEFAULT_REACT_BITS_LIGHTNING.speed, 0, 5),
    intensity: resolveNumber(options?.intensity, DEFAULT_REACT_BITS_LIGHTNING.intensity, 0.1, 5),
    size: resolveNumber(options?.size, DEFAULT_REACT_BITS_LIGHTNING.size, 0.2, 5),
  }
}
