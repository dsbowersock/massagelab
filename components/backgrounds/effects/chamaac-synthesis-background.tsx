...
import type { BackgroundEffectProps, ChamaacSynthesisOptions } from "./css-backgrounds"
...
const DEFAULT_CHAMAAC_SYNTHESIS: ResolvedSynthesisOptions = {
  color1: "#0F172A",
  color2: "#3B0764",
  color3: "#0EA5E9",
  speed: 0.4,
...
// Chamaac UI Synthesis is a Three/R3F shader component. This internal WebGL
// renderer ports the source fragment shader directly without adding that stack.
export default function ChamaacSynthesisBackground({
  className,
  chamaacSynthesis,
}: BackgroundEffectProps) {
  const containerRef = useRef<HTMLDivElement | null>(null)
  const canvasRef = useRef<HTMLCanvasElement | null>(null)
  const color1 = chamaacSynthesis?.color1
  const color2 = chamaacSynthesis?.color2
  const color3 = chamaacSynthesis?.color3
  const speed = chamaacSynthesis?.speed
  const complexity = chamaacSynthesis?.complexity
  const scale = chamaacSynthesis?.scale
  const distortion = chamaacSynthesis?.distortion
  const glowIntensity = chamaacSynthesis?.glowIntensity
  const flowFrequency = chamaacSynthesis?.flowFrequency
  const contrast = chamaacSynthesis?.contrast
  ...
  return (
    <div
      ref={containerRef}
      className={cn(styles.effectLayer, styles.chamaacSynthesis, className)}
      aria-hidden="true"
    >
      <canvas ref={canvasRef} className={styles.chamaacSynthesisCanvas} />
    </div>
  )
}
...
function getUniformLocation(context: WebGLRenderingContext, program: WebGLProgram, name: string) {
  ...
  if (location === null) {
    throw new Error(`Missing Chamaac Synthesis shader uniform: ${name}`)
  }

  return location
}

function resolveSynthesisOptions(options: ChamaacSynthesisOptions): ResolvedSynthesisOptions {
  return {
    color1: normalizeHexColor(options.color1, DEFAULT_CHAMAAC_SYNTHESIS.color1),
    color2: normalizeHexColor(options.color2, DEFAULT_CHAMAAC_SYNTHESIS.color2),
    color3: normalizeHexColor(options.color3, DEFAULT_CHAMAAC_SYNTHESIS.color3),
    speed: clampNumber(options.speed, DEFAULT_CHAMAAC_SYNTHESIS.speed, 0.1, 2),
    complexity: Math.trunc(clampNumber(options.complexity, DEFAULT_CHAMAAC_SYNTHESIS.complexity, 1, 20)),
    scale: clampNumber(options.scale, DEFAULT_CHAMAAC_SYNTHESIS.scale, 0.1, 5),
    distortion: clampNumber(options.distortion, DEFAULT_CHAMAAC_SYNTHESIS.distortion, 0, 2),
    glowIntensity: clampNumber(options.glowIntensity, DEFAULT_CHAMAAC_SYNTHESIS.glowIntensity, 0, 2),
    flowFrequency: clampNumber(options.flowFrequency, DEFAULT_CHAMAAC_SYNTHESIS.flowFrequency, 0.5, 10),
    contrast: clampNumber(options.contrast, DEFAULT_CHAMAAC_SYNTHESIS.contrast, 0.5, 2),
  }
}
