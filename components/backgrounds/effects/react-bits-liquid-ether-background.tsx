...
import type { BackgroundEffectProps, ReactBitsLiquidEtherOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]
type Vector2 = [number, number]
type ResolvedLiquidEtherOptions = Required<Omit<ReactBitsLiquidEtherOptions, "colors">> & {
  colors: string[]
}
...
const DEFAULT_REACT_BITS_LIQUID_ETHER: ResolvedLiquidEtherOptions = {
...
// Direct raw-WebGL port of the React Bits Liquid Ether simulation passes.
...
export default function ReactBitsLiquidEtherBackground({
  className,
  reactBitsLiquidEther,
}: BackgroundEffectProps) {
  const containerRef = useRef<HTMLDivElement | null>(null)
  const canvasRef = useRef<HTMLCanvasElement | null>(null)
  const colorsKey = (reactBitsLiquidEther?.colors ?? []).join("|")
  const mouseInteraction = reactBitsLiquidEther?.mouseInteraction
  const mouseForce = reactBitsLiquidEther?.mouseForce
  const cursorSize = reactBitsLiquidEther?.cursorSize
  const isViscous = reactBitsLiquidEther?.isViscous
  const viscous = reactBitsLiquidEther?.viscous
  const iterationsViscous = reactBitsLiquidEther?.iterationsViscous
  const iterationsPoisson = reactBitsLiquidEther?.iterationsPoisson
  const dt = reactBitsLiquidEther?.dt
  const bfecc = reactBitsLiquidEther?.bfecc
  const resolution = reactBitsLiquidEther?.resolution
  const isBounce = reactBitsLiquidEther?.isBounce
  const autoDemo = reactBitsLiquidEther?.autoDemo
  const autoSpeed = reactBitsLiquidEther?.autoSpeed
  const autoIntensity = reactBitsLiquidEther?.autoIntensity
  const autoResumeDelay = reactBitsLiquidEther?.autoResumeDelay
  const autoRampDuration = reactBitsLiquidEther?.autoRampDuration
  const opacity = reactBitsLiquidEther?.opacity
  ...
  return (
    <div
      ref={containerRef}
      className={cn(styles.effectLayer, styles.reactBitsLiquidEther, className)}
      data-background-id="react-bits-liquid-ether"
    >
      <canvas ref={canvasRef} className={styles.reactBitsLiquidEtherCanvas} />
    </div>
  )
}

function resolveLiquidEtherOptions(input: ReactBitsLiquidEtherOptions): ResolvedLiquidEtherOptions {
  return {
    colors: prepColorStops(input.colors),
    mouseInteraction: input.mouseInteraction ?? DEFAULT_REACT_BITS_LIQUID_ETHER.mouseInteraction,
    mouseForce: clampNumber(input.mouseForce, DEFAULT_REACT_BITS_LIQUID_ETHER.mouseForce, 0, 80),
    cursorSize: clampNumber(input.cursorSize, DEFAULT_REACT_BITS_LIQUID_ETHER.cursorSize, 20, 280),
    isViscous: input.isViscous ?? DEFAULT_REACT_BITS_LIQUID_ETHER.isViscous,
    viscous: clampNumber(input.viscous, DEFAULT_REACT_BITS_LIQUID_ETHER.viscous, 0, 80),
    iterationsViscous: Math.trunc(clampNumber(
      input.iterationsViscous,
      DEFAULT_REACT_BITS_LIQUID_ETHER.iterationsViscous,
      4,
      64,
    )),
    iterationsPoisson: Math.trunc(clampNumber(
      input.iterationsPoisson,
      DEFAULT_REACT_BITS_LIQUID_ETHER.iterationsPoisson,
      4,
      64,
    )),
    dt: clampNumber(input.dt, DEFAULT_REACT_BITS_LIQUID_ETHER.dt, 0.004, 0.04),
    bfecc: input.bfecc ?? DEFAULT_REACT_BITS_LIQUID_ETHER.bfecc,
    resolution: clampNumber(input.resolution, DEFAULT_REACT_BITS_LIQUID_ETHER.resolution, 0.2, 1),
    isBounce: input.isBounce ?? DEFAULT_REACT_BITS_LIQUID_ETHER.isBounce,
    autoDemo: input.autoDemo ?? DEFAULT_REACT_BITS_LIQUID_ETHER.autoDemo,
    autoSpeed: clampNumber(input.autoSpeed, DEFAULT_REACT_BITS_LIQUID_ETHER.autoSpeed, 0.05, 2),
    autoIntensity: clampNumber(input.autoIntensity, DEFAULT_REACT_BITS_LIQUID_ETHER.autoIntensity, 0, 5),
    autoResumeDelay: clampNumber(
      input.autoResumeDelay,
      DEFAULT_REACT_BITS_LIQUID_ETHER.autoResumeDelay,
      250,
      5000,
    ),
    autoRampDuration: clampNumber(
      input.autoRampDuration,
      DEFAULT_REACT_BITS_LIQUID_ETHER.autoRampDuration,
      0,
      3,
    ),
    opacity: clampNumber(input.opacity, DEFAULT_REACT_BITS_LIQUID_ETHER.opacity, 0.05, 1),
  }
}
...
function prepColorStops(colors?: string[]) {
  const stops = Array.isArray(colors) && colors.length > 0
    ...
    : DEFAULT_REACT_BITS_LIQUID_ETHER.colors

  if (stops.length === 1) {
    return [stops[0], stops[0]]
  }

  return stops.length > 0 ? stops : DEFAULT_REACT_BITS_LIQUID_ETHER.colors
}
...
function createProgramInfo<TUniforms extends string>(
  ...
  if (!program) {
    throw new Error("Could not create React Bits Liquid Ether shader program.")
  }
  ...
  if (position < 0) {
    context.deleteProgram(program)
    throw new Error("Missing React Bits Liquid Ether shader attribute: aPosition")
  }
  ...
  for (const uniformName of uniforms) {
    ...
    if (location === null) {
      context.deleteProgram(program)
      throw new Error(`Missing React Bits Liquid Ether shader uniform: ${uniformName}`)
    }
...
function compileShader(context: WebGLRenderingContext, type: number, source: string) {
  ...
  if (!shader) {
    throw new Error("Could not create React Bits Liquid Ether shader.")
  }
...
function createQuadBuffer(context: WebGLRenderingContext) {
  ...
  if (!buffer) {
    throw new Error("Could not create React Bits Liquid Ether quad buffer.")
  }
...
function createRenderTarget(
  ...
  if (!texture || !framebuffer) {
    throw new Error("Could not create React Bits Liquid Ether render target.")
  }
  ...
  if (context.checkFramebufferStatus(context.FRAMEBUFFER) !== context.FRAMEBUFFER_COMPLETE) {
    ...
    throw new Error("React Bits Liquid Ether render target is incomplete.")
  }
...
function createPaletteTexture(context: WebGLRenderingContext, palette: RgbColor[]) {
  ...
  if (!texture) {
    throw new Error("Could not create React Bits Liquid Ether palette texture.")
  }
