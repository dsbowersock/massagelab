...
import type { BackgroundEffectProps, ReactBitsRippleGridOptions } from "./css-backgrounds"

type ResolvedRippleGridOptions = Required<ReactBitsRippleGridOptions>
...
const DEFAULT_REACT_BITS_RIPPLE_GRID: ResolvedRippleGridOptions = {
  enableRainbow: false,
  gridColor: "#FFFFFF",
...
// React Bits Ripple Grid is an OGL shader. MassageLab keeps the source ripple
// grid uniforms while replacing the OGL wrapper with raw WebGL and cleanup.
export default function ReactBitsRippleGridBackground({
  className,
  reactBitsRippleGrid,
  ...
  return (
    <canvas
      ref={canvasRef}
      aria-hidden="true"
      className={cn(styles.reactBitsRippleGridCanvas, className)}
    />
  )
}
...
function createRippleGridResources(gl: WebGLRenderingContext): RippleGridResources | null {
  ...
  if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
    console.error("React Bits Ripple Grid program link failed", gl.getProgramInfoLog(program))
...
function compileShader(gl: WebGLRenderingContext, type: number, source: string): WebGLShader | null {
  ...
  if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
    console.error("React Bits Ripple Grid shader compile failed", gl.getShaderInfoLog(shader))
...
function getUniform(gl: WebGLRenderingContext, program: WebGLProgram, name: string): WebGLUniformLocation {
  ...
  if (!location) {
    throw new Error(`Missing React Bits Ripple Grid shader uniform: ${name}`)
  }

  return location
}

function resolveRippleGridOptions(options: ReactBitsRippleGridOptions | undefined): ResolvedRippleGridOptions {
  return {
    enableRainbow: options?.enableRainbow ?? DEFAULT_REACT_BITS_RIPPLE_GRID.enableRainbow,
    gridColor: resolveHex(options?.gridColor, DEFAULT_REACT_BITS_RIPPLE_GRID.gridColor),
    rippleIntensity: resolveNumber(options?.rippleIntensity, DEFAULT_REACT_BITS_RIPPLE_GRID.rippleIntensity, 0, 0.3),
    gridSize: resolveNumber(options?.gridSize, DEFAULT_REACT_BITS_RIPPLE_GRID.gridSize, 2, 30),
    gridThickness: resolveNumber(options?.gridThickness, DEFAULT_REACT_BITS_RIPPLE_GRID.gridThickness, 1, 50),
    fadeDistance: resolveNumber(options?.fadeDistance, DEFAULT_REACT_BITS_RIPPLE_GRID.fadeDistance, 0.2, 5),
    vignetteStrength: resolveNumber(options?.vignetteStrength, DEFAULT_REACT_BITS_RIPPLE_GRID.vignetteStrength, 0.1, 6),
    glowIntensity: resolveNumber(options?.glowIntensity, DEFAULT_REACT_BITS_RIPPLE_GRID.glowIntensity, 0, 1),
    opacity: resolveNumber(options?.opacity, DEFAULT_REACT_BITS_RIPPLE_GRID.opacity, 0, 1),
    gridRotation: resolveNumber(options?.gridRotation, DEFAULT_REACT_BITS_RIPPLE_GRID.gridRotation, -180, 180),
    mouseInteraction: options?.mouseInteraction ?? DEFAULT_REACT_BITS_RIPPLE_GRID.mouseInteraction,
    mouseInteractionRadius: resolveNumber(
      options?.mouseInteractionRadius,
      DEFAULT_REACT_BITS_RIPPLE_GRID.mouseInteractionRadius,
      0.1,
      5,
    ),
  }
