...
import type { BackgroundEffectProps, ReactBitsSideRaysOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]
type ResolvedSideRaysOptions = Required<ReactBitsSideRaysOptions>
...
const DEFAULT_REACT_BITS_SIDE_RAYS: ResolvedSideRaysOptions = {
  speed: 2.5,
  rayColor1: "#EAB308",
  rayColor2: "#96C8FF",
  intensity: 2,
...
// React Bits Side Rays ships as an OGL full-screen shader. MassageLab keeps
// the shader and origin math while replacing OGL with a native WebGL shell.
export default function ReactBitsSideRaysBackground({
  className,
  reactBitsSideRays,
}: BackgroundEffectProps) {
  const containerRef = useRef<HTMLDivElement | null>(null)
  const canvasRef = useRef<HTMLCanvasElement | null>(null)
  const colorKey = `${reactBitsSideRays?.rayColor1 ?? ""}|${reactBitsSideRays?.rayColor2 ?? ""}`
  const options = useMemo(
    () => resolveSideRaysOptions(reactBitsSideRays),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [
      colorKey,
      reactBitsSideRays?.speed,
      reactBitsSideRays?.intensity,
      reactBitsSideRays?.spread,
      reactBitsSideRays?.origin,
      reactBitsSideRays?.tilt,
      reactBitsSideRays?.saturation,
      reactBitsSideRays?.blend,
      reactBitsSideRays?.falloff,
      reactBitsSideRays?.opacity,
    ],
  )
  ...
  return (
    <div
      ref={containerRef}
      className={cn(styles.effectLayer, styles.reactBitsSideRays, className)}
      aria-hidden="true"
    >
      <canvas ref={canvasRef} className={styles.reactBitsSideRaysCanvas} />
    </div>
  )
}
...
function getUniformLocation(
  ...
  if (!location) {
    throw new Error(`Missing React Bits Side Rays shader uniform: ${name}`)
  }

  return location
}

function resolveSideRaysOptions(options: ReactBitsSideRaysOptions | undefined): ResolvedSideRaysOptions {
  return {
    speed: resolveNumber(options?.speed, DEFAULT_REACT_BITS_SIDE_RAYS.speed, 0, 8),
    rayColor1: resolveHexColor(options?.rayColor1, DEFAULT_REACT_BITS_SIDE_RAYS.rayColor1),
    rayColor2: resolveHexColor(options?.rayColor2, DEFAULT_REACT_BITS_SIDE_RAYS.rayColor2),
    intensity: resolveNumber(options?.intensity, DEFAULT_REACT_BITS_SIDE_RAYS.intensity, 0, 6),
    spread: resolveNumber(options?.spread, DEFAULT_REACT_BITS_SIDE_RAYS.spread, 0.1, 5),
    origin: resolveOrigin(options?.origin),
    tilt: resolveNumber(options?.tilt, DEFAULT_REACT_BITS_SIDE_RAYS.tilt, -90, 90),
    saturation: resolveNumber(options?.saturation, DEFAULT_REACT_BITS_SIDE_RAYS.saturation, 0, 3),
    blend: resolveNumber(options?.blend, DEFAULT_REACT_BITS_SIDE_RAYS.blend, 0, 1),
    falloff: resolveNumber(options?.falloff, DEFAULT_REACT_BITS_SIDE_RAYS.falloff, 0.2, 4),
    opacity: resolveNumber(options?.opacity, DEFAULT_REACT_BITS_SIDE_RAYS.opacity, 0, 1),
  }
}

function originToFlip(origin: ReactBitsSideRaysOptions["origin"]): [number, number] {
  switch (origin) {
    case "top-left":
      return [1, 0]
    case "bottom-right":
...
function resolveOrigin(value: ReactBitsSideRaysOptions["origin"] | undefined): ResolvedSideRaysOptions["origin"] {
  if (value === "top-left" || value === "bottom-right" || value === "bottom-left" || value === "top-right") {
    return value
  }

  return DEFAULT_REACT_BITS_SIDE_RAYS.origin
}

function parseHexColorToRgb(color: string): RgbColor {
  const normalized = resolveHexColor(color, DEFAULT_REACT_BITS_SIDE_RAYS.rayColor1).replace("#", "")
  return [
