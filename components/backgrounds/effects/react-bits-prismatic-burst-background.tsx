...
import type { BackgroundEffectProps, ReactBitsPrismaticBurstOptions } from "./css-backgrounds"

type ResolvedPrismaticBurstOptions = Required<Omit<ReactBitsPrismaticBurstOptions, "colors">> & {
  colors: string[]
}
...
const DEFAULT_REACT_BITS_PRISMATIC_BURST: ResolvedPrismaticBurstOptions = {
  intensity: 2,
  speed: 0.5,
...
// React Bits Prismatic Burst is a WebGL2/OGL shader. MassageLab keeps the
// shader math and gradient texture model while owning WebGL lifecycle cleanup.
export default function ReactBitsPrismaticBurstBackground({
  className,
  ...
  return (
    <canvas
      ref={canvasRef}
      aria-hidden="true"
      className={cn(styles.reactBitsPrismaticBurstCanvas, className)}
      style={{ mixBlendMode }}
    />
  )
}
...
function getUniformLocation(
  ...
  if (!location) {
    throw new Error(`Missing React Bits Prismatic Burst shader uniform: ${name}`)
  }
...
function resolvePrismaticBurstOptions(
  options: ReactBitsPrismaticBurstOptions | undefined,
  ...
  const colors = Array.isArray(options?.colors)
    ...
    : DEFAULT_REACT_BITS_PRISMATIC_BURST.colors

  return {
    intensity: resolveNumber(options?.intensity, DEFAULT_REACT_BITS_PRISMATIC_BURST.intensity, 0, 5),
    speed: resolveNumber(options?.speed, DEFAULT_REACT_BITS_PRISMATIC_BURST.speed, 0, 3),
    animationType: resolveChoice(
      options?.animationType,
      DEFAULT_REACT_BITS_PRISMATIC_BURST.animationType,
      ["rotate", "rotate3d", "hover"],
    ),
    colors,
    distort: resolveNumber(options?.distort, DEFAULT_REACT_BITS_PRISMATIC_BURST.distort, 0, 50),
    offsetX: resolveNumber(options?.offsetX, DEFAULT_REACT_BITS_PRISMATIC_BURST.offsetX, -1000, 1000),
    offsetY: resolveNumber(options?.offsetY, DEFAULT_REACT_BITS_PRISMATIC_BURST.offsetY, -1000, 1000),
    hoverDampness: resolveNumber(
      options?.hoverDampness,
      DEFAULT_REACT_BITS_PRISMATIC_BURST.hoverDampness,
      0,
      1,
    ),
    rayCount: Math.trunc(resolveNumber(options?.rayCount, DEFAULT_REACT_BITS_PRISMATIC_BURST.rayCount, 0, 64)),
    mixBlendMode: resolveChoice(
      options?.mixBlendMode,
      DEFAULT_REACT_BITS_PRISMATIC_BURST.mixBlendMode,
      ["lighten", "screen", "none"],
    ),
  }
}
