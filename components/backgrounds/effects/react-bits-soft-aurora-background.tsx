...
import type { BackgroundEffectProps, ReactBitsSoftAuroraOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]
type ResolvedSoftAuroraOptions = Required<ReactBitsSoftAuroraOptions>
...
const DEFAULT_REACT_BITS_SOFT_AURORA: ResolvedSoftAuroraOptions = {
  speed: 0.6,
  scale: 1.5,
  brightness: 1,
  color1: "#F7F7F7",
...
// React Bits Soft Aurora ships as an OGL full-screen shader. MassageLab keeps
// the source Perlin band math and optional mouse shift while avoiding OGL.
export default function ReactBitsSoftAuroraBackground({
  className,
  reactBitsSoftAurora,
  ...
  return (
    <div className={cn(styles.effectLayer, styles.reactBitsSoftAurora, className)} ref={containerRef}>
      <canvas className={styles.reactBitsSoftAuroraCanvas} ref={canvasRef} />
    </div>
  )
}
...
function getUniformLocation(
  ...
  if (!location) {
    throw new Error(`Missing React Bits Soft Aurora uniform: ${name}`)
  }

  return location
}

function resolveSoftAuroraOptions(options?: ReactBitsSoftAuroraOptions): ResolvedSoftAuroraOptions {
  return {
    speed: clampNumber(options?.speed, DEFAULT_REACT_BITS_SOFT_AURORA.speed, 0, 3),
    scale: clampNumber(options?.scale, DEFAULT_REACT_BITS_SOFT_AURORA.scale, 0.1, 4),
    brightness: clampNumber(options?.brightness, DEFAULT_REACT_BITS_SOFT_AURORA.brightness, 0, 3),
    color1: normalizeHexColor(options?.color1, DEFAULT_REACT_BITS_SOFT_AURORA.color1),
    color2: normalizeHexColor(options?.color2, DEFAULT_REACT_BITS_SOFT_AURORA.color2),
    noiseFrequency: clampNumber(
      options?.noiseFrequency,
      DEFAULT_REACT_BITS_SOFT_AURORA.noiseFrequency,
      0.1,
      8,
    ),
    noiseAmplitude: clampNumber(
      options?.noiseAmplitude,
      DEFAULT_REACT_BITS_SOFT_AURORA.noiseAmplitude,
      0,
      4,
    ),
    bandHeight: clampNumber(options?.bandHeight, DEFAULT_REACT_BITS_SOFT_AURORA.bandHeight, -1, 2),
    bandSpread: clampNumber(options?.bandSpread, DEFAULT_REACT_BITS_SOFT_AURORA.bandSpread, 0.1, 4),
    octaveDecay: clampNumber(options?.octaveDecay, DEFAULT_REACT_BITS_SOFT_AURORA.octaveDecay, 0, 1),
    layerOffset: clampNumber(options?.layerOffset, DEFAULT_REACT_BITS_SOFT_AURORA.layerOffset, -6, 6),
    colorSpeed: clampNumber(options?.colorSpeed, DEFAULT_REACT_BITS_SOFT_AURORA.colorSpeed, 0, 4),
    enableMouseInteraction: typeof options?.enableMouseInteraction === "boolean"
      ? options.enableMouseInteraction
      : DEFAULT_REACT_BITS_SOFT_AURORA.enableMouseInteraction,
    mouseInfluence: clampNumber(
      options?.mouseInfluence,
      DEFAULT_REACT_BITS_SOFT_AURORA.mouseInfluence,
      0,
      1,
    ),
  }
