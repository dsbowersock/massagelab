...
import type { BackgroundEffectProps, ChamaacLiquidChromeOptions } from "./css-backgrounds"

type RgbColor = [number, number, number]

type ResolvedLiquidChromeOptions = Required<ChamaacLiquidChromeOptions>
...
const DEFAULT_CHAMAAC_LIQUID_CHROME: ResolvedLiquidChromeOptions = {
  speed: 0.35,
  timeScale: 0.225,
  color: "#C0C0C0",
  color2: "#4A4A4A",
...
// Chamaac UI Liquid Chrome ships as a Three/R3F shader component. This native
// WebGL port keeps the source uniforms while avoiding that dependency stack.
export default function ChamaacLiquidChromeBackground({
  className,
  chamaacLiquidChrome,
}: BackgroundEffectProps) {
  const containerRef = useRef<HTMLDivElement | null>(null)
  const canvasRef = useRef<HTMLCanvasElement | null>(null)
  const speed = chamaacLiquidChrome?.speed
  const timeScale = chamaacLiquidChrome?.timeScale
  const color = chamaacLiquidChrome?.color
  const color2 = chamaacLiquidChrome?.color2
  ...
  return (
    <div
      ref={containerRef}
      className={cn(styles.effectLayer, styles.chamaacLiquidChrome, className)}
      aria-hidden="true"
    >
      <canvas ref={canvasRef} className={styles.chamaacLiquidChromeCanvas} />
    </div>
  )
}
...
function getUniformLocation(context: WebGLRenderingContext, program: WebGLProgram, name: string) {
  ...
  if (location === null) {
    throw new Error(`Missing Chamaac Liquid Chrome shader uniform: ${name}`)
  }

  return location
}

function resolveLiquidChromeOptions(options: ChamaacLiquidChromeOptions): ResolvedLiquidChromeOptions {
  return {
    speed: clampNumber(options.speed, DEFAULT_CHAMAAC_LIQUID_CHROME.speed, 0.01, 2),
    timeScale: clampNumber(options.timeScale, DEFAULT_CHAMAAC_LIQUID_CHROME.timeScale, 0.001, 1),
    color: normalizeHexColor(options.color, DEFAULT_CHAMAAC_LIQUID_CHROME.color),
    color2: normalizeHexColor(options.color2, DEFAULT_CHAMAAC_LIQUID_CHROME.color2),
  }
}
