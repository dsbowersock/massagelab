...
import type { BackgroundEffectProps, ReactBitsDitherOptions } from "./css-backgrounds"

type ResolvedDitherOptions = Required<ReactBitsDitherOptions>
...
const DEFAULT_REACT_BITS_DITHER: ResolvedDitherOptions = {
  color: "#808080",
  waveSpeed: 0.05,
  waveFrequency: 3,
  waveAmplitude: 0.3,
...
// React Bits Dither combines a wave/noise shader with a Bayer dither postprocess.
// MassageLab keeps that two-pass shape while owning the WebGL framebuffer cleanup.
export default function ReactBitsDitherBackground({
  className,
  reactBitsDither,
  ...
  return (
    <canvas
      ref={canvasRef}
      aria-hidden="true"
      className={cn(styles.reactBitsDitherCanvas, className)}
    />
  )
}
...
function compileShader(gl: WebGL2RenderingContext, type: number, source: string): WebGLShader | null {
  ...
  if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
    console.error("React Bits Dither shader compile failed", gl.getShaderInfoLog(shader))
...
function linkProgram(
  ...
  if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
    console.error("React Bits Dither program link failed", gl.getProgramInfoLog(program))
...
function getUniform(gl: WebGL2RenderingContext, program: WebGLProgram, name: string): WebGLUniformLocation {
  ...
  if (!location) {
    throw new Error(`Missing React Bits Dither shader uniform: ${name}`)
  }

  return location
}

function resolveDitherOptions(options: ReactBitsDitherOptions | undefined): ResolvedDitherOptions {
  return {
    color: resolveHex(options?.color, DEFAULT_REACT_BITS_DITHER.color),
    waveSpeed: resolveNumber(options?.waveSpeed, DEFAULT_REACT_BITS_DITHER.waveSpeed, 0, 0.5),
    waveFrequency: resolveNumber(options?.waveFrequency, DEFAULT_REACT_BITS_DITHER.waveFrequency, 0.5, 8),
    waveAmplitude: resolveNumber(options?.waveAmplitude, DEFAULT_REACT_BITS_DITHER.waveAmplitude, 0, 1),
    colorNum: Math.round(resolveNumber(options?.colorNum, DEFAULT_REACT_BITS_DITHER.colorNum, 2, 16)),
    pixelSize: resolveNumber(options?.pixelSize, DEFAULT_REACT_BITS_DITHER.pixelSize, 1, 24),
    mouseInteraction: options?.mouseInteraction ?? DEFAULT_REACT_BITS_DITHER.mouseInteraction,
    mouseRadius: resolveNumber(options?.mouseRadius, DEFAULT_REACT_BITS_DITHER.mouseRadius, 0.05, 3),
  }
}
