...
import type { BackgroundEffectProps, ReactBitsWavesOptions } from "./css-backgrounds"
...
type ResolvedWavesOptions = Required<ReactBitsWavesOptions>
...
const DEFAULT_REACT_BITS_WAVES: ResolvedWavesOptions = {
  lineColor: "#000000",
...
// React Bits Waves is a dependency-free canvas simulation. MassageLab keeps the
// source Perlin line grid and cursor spring math, then wraps it with route-safe
// sizing, reduced-motion, and listener cleanup.
export default function ReactBitsWavesBackground({
  className,
  reactBitsWaves,
  ...
  return (
    <div
      className={cn(styles.effectLayer, styles.reactBitsWaves, className)}
      ref={containerRef}
      aria-hidden="true"
    >
      <canvas ref={canvasRef} className={styles.reactBitsWavesCanvas} />
    </div>
  )
}

function resolveWavesOptions(options: ReactBitsWavesOptions | undefined): ResolvedWavesOptions {
  return {
    lineColor: resolveHex(options?.lineColor, DEFAULT_REACT_BITS_WAVES.lineColor),
    backgroundColor: resolveHex(options?.backgroundColor, DEFAULT_REACT_BITS_WAVES.backgroundColor),
    transparentBackground: options?.transparentBackground ?? DEFAULT_REACT_BITS_WAVES.transparentBackground,
    waveSpeedX: resolveNumber(options?.waveSpeedX, DEFAULT_REACT_BITS_WAVES.waveSpeedX, 0, 0.05),
    waveSpeedY: resolveNumber(options?.waveSpeedY, DEFAULT_REACT_BITS_WAVES.waveSpeedY, 0, 0.05),
    waveAmpX: resolveNumber(options?.waveAmpX, DEFAULT_REACT_BITS_WAVES.waveAmpX, 0, 96),
    waveAmpY: resolveNumber(options?.waveAmpY, DEFAULT_REACT_BITS_WAVES.waveAmpY, 0, 96),
    xGap: resolveNumber(options?.xGap, DEFAULT_REACT_BITS_WAVES.xGap, 4, 40),
    yGap: resolveNumber(options?.yGap, DEFAULT_REACT_BITS_WAVES.yGap, 8, 96),
    friction: resolveNumber(options?.friction, DEFAULT_REACT_BITS_WAVES.friction, 0.8, 0.99),
    tension: resolveNumber(options?.tension, DEFAULT_REACT_BITS_WAVES.tension, 0.001, 0.05),
    maxCursorMove: resolveNumber(options?.maxCursorMove, DEFAULT_REACT_BITS_WAVES.maxCursorMove, 0, 240),
    cursorInteraction: options?.cursorInteraction ?? DEFAULT_REACT_BITS_WAVES.cursorInteraction,
  }
}
