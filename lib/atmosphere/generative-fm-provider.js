...
/**
 * Checks cancellation before constructing shared Generative.fm preparation,
 * then gives this consumer an abortable wait without cancelling that work.
 *
 * @template T
 * @param {() => Promise<T>} startPreparation
 * @param {AbortSignal} [signal]
 * @returns {Promise<T>}
 */
export async function startAbortableGenerativeFmPrewarm(startPreparation, signal) {
  signal?.throwIfAborted()
  return waitForAbortableGenerativeFmPrewarm(startPreparation(), signal)
}
