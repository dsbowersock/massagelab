...
export async function createObservableStreamsRenderedSampleObjects({
  ...
  for (const target of OBSERVABLE_STREAMS_RENDERED_SAMPLE_TARGETS) {
    ...
    for (const noteName of target.notes) {
      ...
      const sourcePath = path.join(resolvedAudioRoot, sourceAsset.sourceRelativePath)
 ...
 * note with the smallest distance, preserving source order for ties.
 *
...
export function selectClosestSourceAsset(sourceAssets, targetNoteName) {
  ...
  return sourceAssets.reduce((closest, asset) => {
    const distance = Math.abs(noteNameToMidi(asset.noteName) - targetMidi)
    return distance < closest.distance ? { asset, distance } : closest
  }, { asset: sourceAssets[0], distance: Math.abs(noteNameToMidi(sourceAssets[0].noteName) - targetMidi) }).asset
}
...
export function decodeWav(bytes) {
  ...
  const frameCount = Math.floor(dataSize / (format.channelCount * bytesPerSample))
