...
export function createAtmosphereMediaSessionController({ mediaSession, createMetadata }) {
  ...
  function setMetadata(metadata) {
    ...
    try {
      ...
      // AtmoShaper identities do not resolve through the ordinary station API.
      // Omit artwork until the dedicated standard mix asset lands.
      if (metadata.id && !metadata.id.startsWith("atmoshaper:")) {
        init.artwork = [
          {
