...
/** @typedef {{ id: string, title: string, enabled: boolean }} FavoriteStation */
/**
...
/**
 * Resolves the DOM-facing state for a Favorite station tile. Active tiles stay
 * focusable but inert, while busy and known-unavailable stations use native
 * disabled semantics so they cannot start a second playback action.
 *
 * @param {FavoriteStation} station
 * @param {{ busy: boolean, playing: boolean }} state
 * @returns {{ ariaLabel: string, ariaDisabled: boolean, disabled: boolean, canPlay: boolean }}
 */
export function getAtmosphereFavoriteStationTileState(station, { busy, playing }) {
  const unavailable = !station.enabled
  const disabled = busy || unavailable

  return {
    ariaLabel: playing
      ? `${station.title} playing`
      : unavailable
        ? `${station.title} unavailable`
        : station.title,
    ariaDisabled: playing || disabled,
    disabled,
    canPlay: !playing && !disabled,
  }
}
