...
export function createAtmosphereInterruptionMonitor({
  ...
  const initialSignalAvailable = isEventTargetLike(audioSession) || isEventTargetLike(audioContext)
  /** @type {AudioSessionLike | AudioContextLike | null} */
  let signalTarget = null
  /** @type {"audio-session" | "audio-context" | null} */
  let signalKind = null
  let capabilityAvailable = initialSignalAvailable
  let started = false
  let disposed = false
  ...
  return {
    ...
    /** Read the authoritative platform state even before its event is delivered. */
    hasCurrentInterruptionSignal: () => !disposed && currentSignalIsInterrupted(),
  }
}
