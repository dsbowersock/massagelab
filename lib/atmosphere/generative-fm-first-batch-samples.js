...
/**
 * Selects the local source WAVs needed for the first planned Generative.fm
 * batch. Rendered pieces keep their source selections in the manifest while
...
export function createGenerativeFmFirstBatchAssetPlans(params = {}) {
  ...
  return renderPlan.pieces.map((piece) => {
    if (piece.status !== GENERATIVE_FM_RENDER_PLAN_STATUS.READY_FOR_UPLOAD_PLANNING) {
      throw new Error(`Cannot create first-batch sample assets for ${piece.id}; source coverage is ${piece.status}.`)
    }
...
/**
 * Builds per-piece public-media upload plans for the first Generative.fm batch.
...
export async function createGenerativeFmFirstBatchR2UploadPlans(params) {
  if (!params.rootPath) throw new Error("rootPath is required for first-batch Generative.fm upload plans.")
  if (!params.publicBaseUrl) throw new Error("publicBaseUrl is required for first-batch Generative.fm upload plans.")
...
function selectSourceAssetsForPiece(pieceId, entries, mappingBySampleNumber) {
  if (pieceId === "aisatsana") return selectVscoPianoMfAssets(entries, mappingBySampleNumber)
  ...
  throw new Error(`Unsupported first-batch Generative.fm piece: ${pieceId}`)
}
...
function sourceMissingNotesForPiece(pieceId, selectedAssets) {
  ...
  if (pieceId === "aisatsana") return [{ instrumentName: "vsco2-piano-mf", noteName: null }]
...
function matchVscoGlockFile(entry) {
  ...
  const match = /^glock_medium_(?<note>[a-g]#?\d)\.wav$/.exec(basename(entry.normalizedPath))
  if (!match?.groups) return null
