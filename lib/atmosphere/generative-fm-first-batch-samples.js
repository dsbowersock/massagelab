...
const VSCO_PIANO_RENDERED_PIECE_IDS = new Set([
  "no-refrain",
  "transmission",
  "trees",
])
const VSCO_PIANO_PIECE_IDS = new Set([
  ...VSCO_PIANO_SOURCE_INDEX_PIECE_IDS,
  ...VSCO_PIANO_RENDERED_PIECE_IDS,
])
...
function selectSourceAssetsForPiece(pieceId, entries, mappingBySampleNumber) {
  if (VSCO_PIANO_PIECE_IDS.has(pieceId)) return selectVscoPianoMfAssets(entries, mappingBySampleNumber)
...
function sourceMissingNotesForPiece(pieceId, selectedAssets) {
  ...
  if (VSCO_PIANO_PIECE_IDS.has(pieceId)) return [{ instrumentName: "vsco2-piano-mf", noteName: null }]
