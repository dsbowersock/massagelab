...
export function createGenerativeFmRenderPlan(params = {}) {
  const coverage = createGenerativeFmSampleCoverage(params)
  ...
  const pieces = pieceIds.map((pieceId) => {
    ...
    if (!registryEntry) {
      throw new Error(`No first-batch render plan entry exists for Generative.fm piece: ${pieceId}`)
    }
  ...
  return {
    ...
    batchId: "initial-piano-vibraphone-glock",
