...
function createGenerativeFmStation(piece) {
  const missingSampleGroups = findMissingSampleGroups(piece.sampleNameGroups)
  const enabled = missingSampleGroups.length === 0
  const stationId = getGenerativeFmStationId(piece.id)
  const isObservableStreams = piece.id === "observable-streams"
  ...
  return {
    ...
    attribution: {
      ...
      notice: isObservableStreams
        ? "Observable Streams by Alex Bainter. MassageLab runs the package with a hosted CC0 VSCO sample adaptation."
        : `${piece.title} by Alex Bainter. MassageLab runs the generator through the Generative.fm package catalog.`,
    },
    runtime: {
