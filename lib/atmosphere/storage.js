...
export function serializeAtmosphereStorage(value) {
  if ((/** @type {any} */ (value))?.status === "unsupported-version") {
    throw new Error("Unsupported Atmosphere storage cannot be serialized")
  }

  const supportedValue =
    /** @type {Partial<AtmosphereStorageState> | null | undefined} */ (value)
  return JSON.stringify(
    normalizeAtmosphereStorageState(supportedValue, supportedValue?.visualizer),
  )
}
...
function normalizeAtmosphereStorageState(value, visualizer) {
  const source = /** @type {Partial<AtmosphereStorageState>} */ (
