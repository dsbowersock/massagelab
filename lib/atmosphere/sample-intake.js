...
export const OBSERVABLE_STREAMS_VSCO_ADAPTATION = Object.freeze({
  id: OBSERVABLE_STREAMS_ADAPTATION_ID,
...
export function createObservableStreamsSampleIntake(params = {}) {
  ...
  const mappingChartEntry = findVscoPianoMappingChartEntry(entries)
  ...
  return {
    ...
    requirements: OBSERVABLE_STREAMS_SAMPLE_REQUIREMENTS.map((requirement) => {
      if (requirement.sourceName === "vsco2-piano-mf") {
        return {
          ...requirement,
          status: pianoMatches.length > 0 && mappingChartEntry ? "candidate-present" : "missing",
...
/**
 * Finds the VSCO upright-piano mapping chart path from a collected file list.
 *
 * @param {Array<string | { relativePath?: string, path?: string, sizeBytes?: number }>} files
 * @returns {string | null}
 */
export function findObservableStreamsVscoPianoMappingChartPath(files = []) {
  const entries = Array.isArray(files) ? files.map(normalizeEntry).filter((entry) => entry.normalizedPath) : []
  return findVscoPianoMappingChartEntry(entries)?.displayPath ?? null
}
 ...
 */
function findVscoPianoMappingChartEntry(entries) {
  return entries.find((entry) => isVscoPianoMappingChart(entry.normalizedPath))
}
