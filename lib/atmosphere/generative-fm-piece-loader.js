 ...
 * @returns {Promise<unknown>} The prepared runtime after Tone is activated.
 */
export async function prepareGenerativeFmPlayback({ loadRuntimeModules, prepareRuntime }) {
  const modulesPromise = loadRuntimeModules()
  const toneActivationPromise = modulesPromise.then(({ Tone }) => Tone.start())
  const preparedPromise = prepareRuntime()
