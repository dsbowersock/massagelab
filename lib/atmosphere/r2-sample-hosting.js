...
export function createObservableStreamsR2UploadPlan({
  ...
  const sampleIndex = buildSampleIndex(sampleObjects)
  const samplePayloadBytes = sampleObjects.reduce((total, asset) => total + (asset.sizeBytes ?? 0), 0)
  const manifest = {
    adaptationId,
    bucket,
  ...
  return {
    ...
    objectCount: sampleObjects.length + 2,
    samplePayloadBytes,
  }
}
...
export async function putAtmosphereObjectToR2(env, object) {
  ...
  const response = await retry(`Upload ${object.objectKey}`, R2_UPLOAD_RETRIES, () =>
    fetch(objectUrl, {
      method: "PUT",
      headers: {
