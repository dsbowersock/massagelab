...
export function createGenerativeFmSampleCoverage(params = {}) {
  const entries = Array.isArray(params.files) ? params.files.map(normalizeEntry).filter((entry) => entry.normalizedPath) : []
  ...
  const hostedSampleGroupSet = new Set(listHostedGenerativeFmSampleGroups())
...
/**
 * @param {{ libraryId: "vsco-2-ce" | "vcsl" | "signature-sounds-beach" | "signature-sounds-choir-teaser" | "signature-sounds-serbian-choir", label: string, pathContains: string[], fileNameIncludes?: string[], extraRequiredPathSuffixes?: string[], note?: string }} params
 */
...
function summarizePieceCoverage({
  ...
  const status = statusForPiece(groups)
  return {
    id: piece.id,
    title: piece.title,
    status,
...
function summarizeLibraries(entries, licenseTextByPath) {
  return Object.values(LIBRARIES).map((library) => {
    ...
    const licenseEvidencePath = entries.find((entry) => (
    ...
    const licenseStatus = textMatches
      ? "license-confirmed"
      : licenseEvidencePath
