...
/**
 * @param {Array<{ instrumentName: string, noteName: string | null, collectionType?: "notes" | "array", publicUrl: string }>} assets
 */
export function buildSampleIndex(assets) {
  return assets.reduce((sampleIndex, asset) => {
    if (asset.collectionType === "array") {
      if (sampleIndex[asset.instrumentName] && !Array.isArray(sampleIndex[asset.instrumentName])) {
      ...
      sampleIndex[asset.instrumentName].push(asset.publicUrl)
      return sampleIndex
    }
    ...
    if (Array.isArray(sampleIndex[asset.instrumentName])) {
