...
export function safePostLegalAcceptanceCallback(value, fallback = DEFAULT_POST_ACCOUNT_CALLBACK) {
  ...
  try {
    ...
    const decodedSegments = parsed.pathname.split("/").map((segment) => {
      ...
      // Encoded separators, delimiters, and percent signs are ambiguous after
      // another parser or proxy pass, so fail closed instead of normalizing them.
      if (/[\\/?#%]/.test(decoded)) throw new URIError("Ambiguous encoded path segment")
      return decoded
    })
