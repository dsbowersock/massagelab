...
/**
 * Distinguishes a Stripe parameter conflict from retryable or transport-level
 * Checkout failures without exposing provider error details to callers.
 */
export class DonationCheckoutAttemptConflictError extends Error {
  constructor({ cause } = {}) {
    super("A conflicting donation Checkout attempt already exists.", { cause })
    this.name = "DonationCheckoutAttemptConflictError"
  }
}

const DONATION_IDEMPOTENCY_KEY_PATTERN = /^massagelab-donation-v1:[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/

/**
 * Rejects malformed client-generated donation attempt keys before creating a
 * Stripe client, keeping request replay state outside Checkout payload data.
 */
function assertDonationIdempotencyKey(idempotencyKey) {
  if (
    typeof idempotencyKey !== "string"
    || !DONATION_IDEMPOTENCY_KEY_PATTERN.test(idempotencyKey)
  ) {
    throw new Error("Donation Checkout idempotency key must use the canonical massagelab-donation-v1 UUID format.")
  }
}

/**
 * Stripe 22.1.1 normalizes raw `idempotency_error` responses into this error
 * type. The explicit key-in-use code is a distinct provider outcome and must
 * remain unchanged for its caller, as must 5xx results that may be transport
 * or provider failures despite an overlapping error type.
 */
function isDonationCheckoutAttemptConflict(error) {
 ...
 *     checkout: {
 *       sessions: {
 *         create: (payload: Record<string, unknown>, options: { idempotencyKey: string }) => Promise<unknown>
 *       }
 *     }
 *   }
 * }} input
...
export async function createStripeDonationCheckoutSession({
  ...
  assertDonationIdempotencyKey(idempotencyKey)
  const stripe = stripeClient ?? getStripeClient(apiKey)
  const metadata = {
  ...
  try {
    return await stripe.checkout.sessions.create(session, { idempotencyKey })
  } catch (error) {
    if (isDonationCheckoutAttemptConflict(error)) {
      throw new DonationCheckoutAttemptConflictError({ cause: error })
    }
    throw error
  }
}
