...
/**
 * Uses a monotonic clock to prevent bounded Stripe operation counts from still
 * occupying an unbounded request duration. Tests inject both the clock and
 * budget so deadline boundaries remain deterministic without real-time waits.
 *
...
async function isCurrentSupporterMembershipCheckoutSession(
  stripe,
  session,
  { currentPriceIds, nowSeconds, requestedPriceId },
) {
  const metadata = objectRecord(session?.metadata)
  if (
 ...
 * Missing subscription IDs fail closed before any network lookup. The caller
 * applies the authority-lookup budget while retaining newest-first blocking
 * precedence.
 */
...
async function resolveExistingStripeMembershipCheckoutCore(
  ...
  for (const [index, completedSession] of uniqueCompletedSessions.entries()) {
    if (index >= MAX_MEMBERSHIP_CHECKOUT_SUBSCRIPTION_AUTHORITY_LOOKUPS) {
...
export async function createStripeCheckoutSession(options) {
  ...
  const session = {
    ...
    ...(supporterTax
      ? {
          // Current Supporter access is an electronically supplied service;
          // the readiness gate above requires Stripe Tax for this payload.
  ...
  const existing = await resolveExistingStripeMembershipCheckout(stripe, {
    ...
    reconciliationBudgetMs: options.reconciliationBudgetMs,
    reconciliationNowMs: options.reconciliationNowMs,
    userId: options.userId,
  })
  if (existing.session) {
