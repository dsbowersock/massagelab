...
const MAX_MEMBERSHIP_CHECKOUT_SUBSCRIPTION_AUTHORITY_CONCURRENCY = 3
const MAX_MEMBERSHIP_CHECKOUT_SUBSCRIPTION_AUTHORITY_LOOKUPS = 25
...
const MEMBERSHIP_CHECKOUT_AUTHORITY_LIMIT_ERROR =
  "Stripe completed membership Checkout subscription authority lookups exceeded the safe limit."
 ...
 * install one absolute request-pool deadline.
 *
...
function createMembershipCheckoutReconciliationBudget(options) {
  ...
  function remainingMs(errorMessage) {
  ...
  return {
    assertWithinBudget(errorMessage) {
      remainingMs(errorMessage)
    },
    remainingMs,
  }
}
 ...
 * both initial reconciliation and completion-during-expiry races.
 */
async function blockingCompletedMembershipCheckoutProjection(
  ...
  if (!subscriptionId) {
    return membershipCheckoutProjection(completedSession)
  }
  ...
  return hasSubscriptionBlockingNewCheckout([subscription])
    ? membershipCheckoutProjection(completedSession)
    : null
}
...
async function firstBlockingCompletedMembershipCheckoutProjection(
  ...
  const missingSubscriptionIndex = completedSessions.findIndex(
    (session) => !stripeId(session.subscription),
  )
  const authoritySessionCount = Math.min(
    completedSessions.length,
    MAX_MEMBERSHIP_CHECKOUT_SUBSCRIPTION_AUTHORITY_LOOKUPS
      + (
        missingSubscriptionIndex >= 0
        && missingSubscriptionIndex
          <= MAX_MEMBERSHIP_CHECKOUT_SUBSCRIPTION_AUTHORITY_LOOKUPS
          ? 1
          : 0
      ),
  ...
  const timeoutMs = Math.ceil(reconciliationBudget.remainingMs(
    MEMBERSHIP_CHECKOUT_AUTHORITY_LIMIT_ERROR,
  ))
  const deadlinePromise = new Promise((_, reject) => {
    deadlineTimer = setTimeout(
      () => reject(new Error(MEMBERSHIP_CHECKOUT_AUTHORITY_LIMIT_ERROR)),
      timeoutMs,
    )
  })
  ...
  async function resolveNextAuthority() {
    while (nextIndex < stopAfterIndex) {
      ...
      try {
        ...
        const blockingSession = await Promise.race([
        ...
        recordOutcome(index, { blockingSession })
        if (blockingSession) {
  ...
  const workerCount = Math.min(
    MAX_MEMBERSHIP_CHECKOUT_SUBSCRIPTION_AUTHORITY_CONCURRENCY,
  ...
  try {
    for (const outcomePromise of outcomePromises) {
      const outcome = await Promise.race([outcomePromise, deadlinePromise])
      if (outcome.error) {
        throw outcome.error
      }
      if (outcome.blockingSession) {
        return outcome.blockingSession
      }
    }

    await workersDone
    if (
      completedSessions.filter((session) => stripeId(session.subscription)).length
      > MAX_MEMBERSHIP_CHECKOUT_SUBSCRIPTION_AUTHORITY_LOOKUPS
    ) {
      throw new Error(MEMBERSHIP_CHECKOUT_AUTHORITY_LIMIT_ERROR)
    }
    return null
  } finally {
...
async function resolveExistingStripeMembershipCheckoutCore(
  ...
  const blockingCompletedSession = await firstBlockingCompletedMembershipCheckoutProjection(
    stripe,
    uniqueCompletedSessions,
    options.env,
    reconciliationBudget,
  )
  if (blockingCompletedSession) {
    return {
      session: blockingCompletedSession,
      latestSessionId: safeString(blockingCompletedSession.id),
    }
  }
  ...
  for (const [index, sessionToExpire] of sessionsToExpire.entries()) {
    ...
    if (confirmed.status === "complete") {
      reconciliationBudget.assertWithinBudget(MEMBERSHIP_CHECKOUT_AUTHORITY_LIMIT_ERROR)
      const blockingSession = await blockingCompletedMembershipCheckoutProjection(
        stripe,
        confirmed,
        options.env,
      )
      ...
      if (blockingSession) {
        return {
          ...
          latestSessionId: safeString(confirmed.id),
        }
      }
    } else {
      reconciliationBudget.assertWithinBudget(MEMBERSHIP_CHECKOUT_EXPIRATION_LIMIT_ERROR)
...
export async function createStripeDonationCheckoutSession({
  ...
  const session = {
    // Automatic Tax is intentionally omitted: one-time support is a
    // non-entitlement payment whose classification remains separate from the
    // taxed digital-access products.
    mode: "payment",
