 ...
 * blocking result or failure is observed.
 */
async function firstBlockingCompletedMembershipCheckoutProjection(
  ...
  const outcomes = new Array(authoritySessionCount)
  let nextIndex = 0
  ...
  async function resolveNextAuthority() {
    while (nextIndex < stopAfterIndex) {
      ...
      try {
        ...
        outcomes[index] = { blockingSession }
        if (blockingSession) {
  ...
  await Promise.all(
    Array.from({ length: workerCount }, () => resolveNextAuthority()),
  )
