...
function membershipCheckoutAuthorityDeadlineError() {
  return MembershipCheckoutAuthorityError.deadline()
}

function membershipCheckoutAuthorityInvariantError() {
  return MembershipCheckoutAuthorityError.invariant()
}

function membershipCheckoutAuthorityReadBudgetError() {
  return MembershipCheckoutAuthorityError.readBudget()
}
...
function createMembershipCheckoutAuthorityDeadline(reconciliationBudget) {
  ...
  const timeoutMs = Math.ceil(reconciliationBudget.remainingMs(
    membershipCheckoutAuthorityDeadlineError,
  ))
  const deadlinePromise = new Promise((_, reject) => {
    deadlineTimer = setTimeout(
      () => reject(membershipCheckoutAuthorityDeadlineError()),
      timeoutMs,
    )
    deadlineTimer?.unref?.()
  })
...
async function firstBlockingCompletedMembershipCheckoutProjection(
  ...
  if (
    ...
    throw membershipCheckoutAuthorityInvariantError()
  }
  ...
  try {
    ...
    if (
      ...
      throw membershipCheckoutAuthorityReadBudgetError()
    }
    return null
  } finally {
