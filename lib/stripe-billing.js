...
async function firstBlockingCompletedMembershipCheckoutProjection(
  ...
  const missingSubscriptionCount = completedSessions.filter(
    (session) => !stripeId(session.subscription),
  ).length
  if (
...
async function resolveExistingStripeMembershipCheckoutCore(
  ...
  for (const [index, sessionToExpire] of sessionsToExpire.entries()) {
    ...
    if (confirmed.status === "complete") {
      ...
      const authorityDeadline =
        createMembershipCheckoutAuthorityDeadline(reconciliationBudget)
      const blockingAuthorityPromise = blockingCompletedMembershipCheckoutProjection(
        stripe,
        confirmed,
        options.env,
      )
      authorityDeadline.clearWhenSettled(blockingAuthorityPromise)
      const blockingAuthority = await authorityDeadline.race(
        blockingAuthorityPromise,
      )
      reconciliationBudget.assertWithinBudget(MEMBERSHIP_CHECKOUT_AUTHORITY_LIMIT_ERROR)
      if (blockingAuthority) {
        return {
