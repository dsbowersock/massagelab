...
async function listStripeMembershipCheckoutSessions(
  ...
  for (let pageNumber = 0; pageNumber < MAX_MEMBERSHIP_CHECKOUT_SESSION_PAGES; pageNumber += 1) {
    ...
    latestExpiredSession ??= matchingSessions.find(
...
async function resolveExistingStripeMembershipCheckout(stripe, options) {
  ...
  try {
    return await resolveExistingStripeMembershipCheckoutCore(stripe, options)
  } catch (error) {
    outcome = "error"
...
async function resolveExistingStripeMembershipCheckoutCore(stripe, options) {
