...
const BACKGROUND_CHECKOUT_EXPIRY_BUFFER_SECONDS = 5 * 60
...
export async function createBackgroundPurchaseCheckoutSession({
  ...
  const checkoutExpiresAt = reservationExpiresAt instanceof Date
    ? Math.floor(reservationExpiresAt.getTime() / 1000) + BACKGROUND_CHECKOUT_EXPIRY_BUFFER_SECONDS
  ...
  const payload = {
    ...
    success_url: successUrl,
    cancel_url: cancelUrl,
    // The route derives this from the persisted reservation plus a fixed safety
    // buffer. It remains valid after order/customer setup and is identical on
    // every replay under the same idempotency key.
