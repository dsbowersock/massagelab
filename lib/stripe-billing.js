...
/**
 * Lists every bounded page needed to find MassageLab-owned membership Sessions.
 * Malformed or non-advancing pagination fails closed instead of permitting a
 * second Checkout attempt.
 */
async function listStripeMembershipCheckoutSessions(stripe, { customerId, userId }) {
  const sessions = []
...
async function resolveExistingStripeMembershipCheckout(stripe, options) {
  ...
  if (openSession) {
    if (typeof openSession.url !== "string" || !openSession.url) {
      throw new Error("Stripe returned an open Checkout Session without a URL.")
    }
    return {
      session: membershipCheckoutProjection(openSession),
 ...
 * webhook persistence catches up.
 */
