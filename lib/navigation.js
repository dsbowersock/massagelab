...
const warnedUnknownPracticeRoles = new Set()
...
function normalizePracticeRoles(practiceRoles) {
  ...
  for (const practiceRole of practiceRoles) {
    ...
    if (calendarOperatorRoles.includes(normalizedRole)) {
      ...
      warnUnknownPracticeRole(rawRole, normalizedRole)
    }
  }
...
function warnUnknownPracticeRole(originalRole, normalizedRole) {
  if (process.env.NODE_ENV === "production") {
    return
  }

  if (warnedUnknownPracticeRoles.has(normalizedRole)) {
    return
  }

  warnedUnknownPracticeRoles.add(normalizedRole)
