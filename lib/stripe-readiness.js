...
export function validateRetrievedMembershipPrice(price, expected) {
  ...
  if (!product) {
    ...
    if (
      ...
      failures.push(`${expected.key} Product must identify amount choice ${expected.amountChoiceId}.`)
    }
  }
...
export function validateSupporterProductTopology(entries) {
  ...
  for (const choiceId of ["support-1", "support-2", "support-5"]) {
    if (productIdsByChoice.get(choiceId)?.size !== 1) {
      failures.push(`Supporter amount choice ${choiceId} must use exactly one Stripe Product.`)
    }
  }

  const productIds = [...productIdsByChoice.values()]
    .flatMap((ids) => [...ids])
  if (new Set(productIds).size !== 3) {
    failures.push("The Supporter catalog must use three distinct amount-specific Stripe Products.")
  }
