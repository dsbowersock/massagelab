...
export function formatDurationParts(timeInMs, settings = {}) {
  ...
  if (!showSeconds) {
    const totalMinutes = clamped === 0 ? 0 : Math.ceil(clamped / 60_000)
...
export function sanitizeChimerSettings(input = {}) {
  ...
  return {
    ...
    timeFormat,
    timerFontColor: normalizeHexColor(input.timerFontColor, fallback.timerFontColor),
    clockFontColor: normalizeHexColor(input.clockFontColor, fallback.clockFontColor),
