...
import { DEFAULT_BACKGROUND_ID, isBackgroundId, normalizeBackgroundId } from "./background-options.js"
import { normalizeSharedBackgroundVisualPreferences } from "./background-palette.js"
import {
...
const CHIMER_BACKGROUND_VISUAL_PROPERTY_KEYS = Object.freeze(
  Object.keys(DEFAULT_CHIMER_SETTINGS).filter((key) => key !== "backgroundVisualPreferences"),
)

/**
 * Normalizes nested shared visual preferences against known Chimer fields.
 * Visual preset values reuse the canonical Chimer clamps without importing the
 * TypeScript renderer registry into this plain-JavaScript persistence layer.
 */
export function normalizeChimerBackgroundVisualPreferences(value) {
  return normalizeSharedBackgroundVisualPreferences(value, {
    isKnownBackgroundId: isBackgroundId,
    getVisualPropertyKeys: () => CHIMER_BACKGROUND_VISUAL_PROPERTY_KEYS,
    normalizeVisualProperties: (properties) => {
      ...
      return Object.fromEntries(
        Object.keys(properties)
          .filter((key) => CHIMER_BACKGROUND_VISUAL_PROPERTY_KEYS.includes(key))
...
export function sanitizeChimerSettings(input = {}, options = {}) {
  ...
  const migratedSettings = migrateLegacyCanvasRevealDotsDefaults(sanitizedSettings)
