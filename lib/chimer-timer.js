...
} from "./static-gradient-background.js"
...
const MASSAGE_LAB_ELECTRIC_MIST_CONTROL_VERSION = 3
const MASSAGE_LAB_ELECTRIC_MIST_DIRECT_PERCENT_VERSION = 2
const MASSAGE_LAB_DOT_FIELD_DEFAULTS_VERSION = 1
const MASSAGE_LAB_SHAPE_GRID_SPEED_VERSION = 1
const MASSAGE_LAB_PHOTON_BEAM_DEFAULTS_VERSION = 1
const MASSAGE_LAB_GRID_FADE_DEFAULTS_VERSION = 1
const MASSAGE_LAB_GRID_LEGACY_FADE_SECONDS_DEFAULT = 1.2
const MASSAGE_LAB_SHAPE_GRID_DEFAULT_SPEED = 0.25
const MASSAGE_LAB_SHAPE_GRID_MAX_SPEED = 2
...
const CHIMER_SETTINGS_WITH_RENDERER_SOURCE_DEFAULTS = Object.freeze({
  ...
  massageLabElectricMistSpeed: 50,
  ...
  massageLabDotFieldDotRadius: 4,
  massageLabDotFieldDotSpacing: 6,
  ...
  massageLabDotFieldDefaultsVersion: MASSAGE_LAB_DOT_FIELD_DEFAULTS_VERSION,
  ...
  massageLabDotGridSimulateCursorInteraction: false,
  massageLabDotGridSimulationSpeed: 1,
  ...
  massageLabGridDistortionSimulateCursorInteraction: false,
  massageLabGridDistortionSimulationSpeed: 1,
  ...
  massageLabGridMotionMantras: DEFAULT_GRID_MOTION_MANTRAS,
  massageLabShapeGridDirection: "right",
  massageLabShapeGridSpeed: MASSAGE_LAB_SHAPE_GRID_DEFAULT_SPEED,
  massageLabShapeGridSpeedVersion: MASSAGE_LAB_SHAPE_GRID_SPEED_VERSION,
  ...
  massageLabPhotonBeamLineCount: 12,
  massageLabPhotonBeamDefaultsVersion: MASSAGE_LAB_PHOTON_BEAM_DEFAULTS_VERSION,
  ...
  massageLabAuroraSpeed: 1,
  massageLabAuroraIntensity: 0.5,
  massageLabAuroraBlur: 10,
  massageLabAuroraReach: 70,
  massageLabDottedGlowSpeed: 1,
  massageLabDottedGlowDotSize: 1.7,
  massageLabDottedGlowDotSpacing: 14,
  massageLabDottedGlowOpacity: 0.58,
  massageLabDottedGlowGlowStrength: 6,
  massageLabBubbleSpeed: 1,
  massageLabBubbleIntensity: 1,
  massageLabBubbleSize: 1,
  massageLabBubbleBlur: 40,
  massageLabBubbleBlendStrength: 18,
  massageLabBackgroundBeamsSpeed: 1,
  massageLabBackgroundBeamsIntensity: 0.82,
  massageLabBackgroundBeamsBeamWidth: 0.6,
  massageLabBackgroundBeamsGlowStrength: 10,
  massageLabCollisionBeamsSpeed: 1,
  massageLabCollisionBeamsIntensity: 1,
  massageLabCollisionBeamsBeamWidth: 1,
  massageLabCollisionBeamsBurstSize: 1,
  massageLabGlowingStarsSpeed: 1,
  massageLabGlowingStarsIntensity: 0.94,
  massageLabGlowingStarsActiveStars: 5,
  massageLabGlowingStarsStarSize: 1,
  massageLabGlowingStarsGlowStrength: 1,
  massageLabMeteorsSpeed: 1,
  massageLabMeteorsIntensity: 0.82,
  massageLabMeteorsCount: 28,
  massageLabMeteorsSize: 2,
  massageLabMeteorsTailLength: 50,
  backgroundLinesDuration: 10,
  backgroundLinesIntensity: 0.68,
  backgroundLinesCount: 26,
  backgroundLinesWidth: 2.3,
  backgroundLinesGlowStrength: 10,
  ...
  massageLabGridFadeDefaultsVersion: MASSAGE_LAB_GRID_FADE_DEFAULTS_VERSION,
...
function normalizeMassageLabElectricMistSpeedPercent(value, fallback, version) {
  ...
  // Version 3 stores the 1–100 display scale where 50% preserves the authored
  // default and 100% maps to the authored 200% ceiling. Version 2 stored the
  // authored percentage directly; version 1 stored the source multiplier.
  const numericVersion = Number(version)
  const percentValue = numericVersion === MASSAGE_LAB_ELECTRIC_MIST_CONTROL_VERSION
    ? number
    : numericVersion === MASSAGE_LAB_ELECTRIC_MIST_DIRECT_PERCENT_VERSION
      ? number / 2
      : number * 50
...
/**
 * Presents Shape Grid's authored 0-2 speed as a familiar 0-100% control while
 * preserving the renderer's finer source values in persisted settings.
 */
export function getMassageLabShapeGridSpeedDisplayPercent(value) {
  const speed = normalizeNumber(
    value,
    MASSAGE_LAB_SHAPE_GRID_DEFAULT_SPEED,
    0,
    MASSAGE_LAB_SHAPE_GRID_MAX_SPEED,
  )
  return Number(((speed / MASSAGE_LAB_SHAPE_GRID_MAX_SPEED) * 100).toFixed(1))
}

/** Converts the Shape Grid control's display percentage back to source speed. */
export function getMassageLabShapeGridSpeedFromDisplayPercent(value) {
  const percent = normalizeNumber(
    value,
    getMassageLabShapeGridSpeedDisplayPercent(MASSAGE_LAB_SHAPE_GRID_DEFAULT_SPEED),
    0,
    100,
  )
  return Number(((percent / 100) * MASSAGE_LAB_SHAPE_GRID_MAX_SPEED).toFixed(2))
}
...
function normalizeMassageLabElectricMistBrightnessPercent(value, fallback, version) {
  ...
  // Version 1 stored the source shader brightness multiplier. Versions 2 and 3
  // store a direct output-intensity percentage, including a true 1% lower bound.
  const numericVersion = Number(version)
  const percentValue = numericVersion === MASSAGE_LAB_ELECTRIC_MIST_DIRECT_PERCENT_VERSION
    || numericVersion === MASSAGE_LAB_ELECTRIC_MIST_CONTROL_VERSION
    ? number
    : number * 100
  return normalizeNumber(percentValue, fallback, 1, 100)
}
...
export function sanitizeChimerSettings(input = {}, options = {}) {
  ...
  const massageLabLightSpeedWarpSpeedVersion = input.massageLabLightSpeedWarpSpeedVersion === MASSAGE_LAB_LIGHT_SPEED_WARP_SPEED_VERSION
    ...
    : 1
  // The app persists the complete settings object, so migrate only the exact
  // former Dot Field tuple. Any changed value means the user customized it.
  const shouldMigrateMassageLabDotFieldDefaults = input.massageLabDotFieldDefaultsVersion !== MASSAGE_LAB_DOT_FIELD_DEFAULTS_VERSION
    && input.massageLabDotFieldDotRadius === 1.5
    && input.massageLabDotFieldDotSpacing === 14
    && input.massageLabDotFieldSparkle === false
  // Only the untouched former default migrates. Existing authored speeds keep
  // their meaning and are merely clamped into the corrected source range.
  const shouldMigrateMassageLabShapeGridSpeed = input.massageLabShapeGridSpeedVersion !== MASSAGE_LAB_SHAPE_GRID_SPEED_VERSION
    && input.massageLabShapeGridSpeed === 1
  // Migrate only the complete former Photon Beam density tuple. If either
  // value differs, the user authored it and it must retain its meaning.
  const shouldMigrateMassageLabPhotonBeamDefaults = input.massageLabPhotonBeamDefaultsVersion !== MASSAGE_LAB_PHOTON_BEAM_DEFAULTS_VERSION
    && input.massageLabPhotonBeamLineCount === 80
    && input.massageLabPhotonBeamSignalCount === 94
  // A version marker distinguishes untouched 1.2-second defaults from values
  // users chose after this 20-second default shipped.
  const shouldMigrateTileGridFadeDefault = input.massageLabGridFadeDefaultsVersion !== MASSAGE_LAB_GRID_FADE_DEFAULTS_VERSION
    && input.tileGridChangeFrequency === MASSAGE_LAB_GRID_LEGACY_FADE_SECONDS_DEFAULT
  const shouldMigrateHexGridFadeDefault = input.massageLabGridFadeDefaultsVersion !== MASSAGE_LAB_GRID_FADE_DEFAULTS_VERSION
    && input.hexGridChangeFrequency === MASSAGE_LAB_GRID_LEGACY_FADE_SECONDS_DEFAULT
  ...
  const sanitizedSettings = {
    ...
    ...toStaticGradientChimerSettingsPatch(
      getStaticGradientBackgroundOptionsFromChimerSettings(input),
    ),
    ...
    massageLabDotFieldDotRadius: shouldMigrateMassageLabDotFieldDefaults
      ? fallback.massageLabDotFieldDotRadius
      : normalizeNumber(input.massageLabDotFieldDotRadius, fallback.massageLabDotFieldDotRadius, 0.5, 8),
    massageLabDotFieldDotSpacing: shouldMigrateMassageLabDotFieldDefaults
      ? fallback.massageLabDotFieldDotSpacing
      : normalizeNumber(input.massageLabDotFieldDotSpacing, fallback.massageLabDotFieldDotSpacing, 4, 48),
    ...
    massageLabDotFieldSparkle: shouldMigrateMassageLabDotFieldDefaults
      ? fallback.massageLabDotFieldSparkle
      : typeof input.massageLabDotFieldSparkle === "boolean"
    ...
    massageLabDotGridSimulateCursorInteraction: typeof input.massageLabDotGridSimulateCursorInteraction === "boolean"
      ? input.massageLabDotGridSimulateCursorInteraction
      : fallback.massageLabDotGridSimulateCursorInteraction,
    massageLabDotGridSimulationSpeed: normalizeNumber(
      input.massageLabDotGridSimulationSpeed,
      fallback.massageLabDotGridSimulationSpeed,
      0.3,
      2,
    ),
    ...
    massageLabGridDistortionSimulateCursorInteraction:
      typeof input.massageLabGridDistortionSimulateCursorInteraction === "boolean"
        ? input.massageLabGridDistortionSimulateCursorInteraction
        : fallback.massageLabGridDistortionSimulateCursorInteraction,
    massageLabGridDistortionSimulationSpeed: normalizeNumber(
      input.massageLabGridDistortionSimulationSpeed,
      fallback.massageLabGridDistortionSimulationSpeed,
      0.3,
      2,
    ),
    ...
    massageLabGridMotionMantras: normalizeGridMotionMantras(
      input.massageLabGridMotionMantras,
      fallback.massageLabGridMotionMantras,
    ),
    ...
    massageLabShapeGridSpeed: shouldMigrateMassageLabShapeGridSpeed
      ? MASSAGE_LAB_SHAPE_GRID_DEFAULT_SPEED
      : normalizeNumber(
          input.massageLabShapeGridSpeed,
          fallback.massageLabShapeGridSpeed,
          0,
          MASSAGE_LAB_SHAPE_GRID_MAX_SPEED,
        ),
    massageLabShapeGridSpeedVersion: MASSAGE_LAB_SHAPE_GRID_SPEED_VERSION,
    ...
    massageLabPhotonBeamLineCount: shouldMigrateMassageLabPhotonBeamDefaults
      ? fallback.massageLabPhotonBeamLineCount
      : normalizeInteger(input.massageLabPhotonBeamLineCount, fallback.massageLabPhotonBeamLineCount, 12, 160),
    massageLabPhotonBeamDefaultsVersion: MASSAGE_LAB_PHOTON_BEAM_DEFAULTS_VERSION,
    ...
    massageLabPhotonBeamSignalCount: shouldMigrateMassageLabPhotonBeamDefaults
      ? fallback.massageLabPhotonBeamSignalCount
      : normalizeInteger(input.massageLabPhotonBeamSignalCount, fallback.massageLabPhotonBeamSignalCount, 0, 220),
    ...
    // Dedicated domain modules sanitize every DNA/Twisted option and emit only
    // their known flat Chimer keys, rather than repeating inline normalizers.
    ...toDnaChimerSettingsPatch(getDnaBackgroundOptionsFromChimerSettings(input)),
    ...toTwistedCubesChimerSettingsPatch(getTwistedCubesBackgroundOptionsFromChimerSettings(input)),
    ...
    massageLabAuroraSpeed: normalizeNumber(input.massageLabAuroraSpeed, fallback.massageLabAuroraSpeed, 0.25, 2),
    massageLabAuroraIntensity: normalizeNumber(input.massageLabAuroraIntensity, fallback.massageLabAuroraIntensity, 0.1, 1),
    massageLabAuroraBlur: normalizeNumber(input.massageLabAuroraBlur, fallback.massageLabAuroraBlur, 0, 30),
    massageLabAuroraReach: normalizeNumber(input.massageLabAuroraReach, fallback.massageLabAuroraReach, 30, 100),
    massageLabDottedGlowSpeed: normalizeNumber(input.massageLabDottedGlowSpeed, fallback.massageLabDottedGlowSpeed, 0.25, 2),
    massageLabDottedGlowDotSize: normalizeNumber(input.massageLabDottedGlowDotSize, fallback.massageLabDottedGlowDotSize, 0.5, 4),
    massageLabDottedGlowDotSpacing: normalizeNumber(input.massageLabDottedGlowDotSpacing, fallback.massageLabDottedGlowDotSpacing, 8, 28),
    massageLabDottedGlowOpacity: normalizeNumber(input.massageLabDottedGlowOpacity, fallback.massageLabDottedGlowOpacity, 0.1, 1),
    massageLabDottedGlowGlowStrength: normalizeNumber(input.massageLabDottedGlowGlowStrength, fallback.massageLabDottedGlowGlowStrength, 0, 12),
    massageLabBubbleSpeed: normalizeNumber(input.massageLabBubbleSpeed, fallback.massageLabBubbleSpeed, 0.25, 2),
    massageLabBubbleIntensity: normalizeNumber(input.massageLabBubbleIntensity, fallback.massageLabBubbleIntensity, 0.1, 1),
    massageLabBubbleSize: normalizeNumber(input.massageLabBubbleSize, fallback.massageLabBubbleSize, 0.5, 2),
    massageLabBubbleBlur: normalizeNumber(input.massageLabBubbleBlur, fallback.massageLabBubbleBlur, 0, 80),
    massageLabBubbleBlendStrength: normalizeNumber(input.massageLabBubbleBlendStrength, fallback.massageLabBubbleBlendStrength, 10, 30),
    massageLabBackgroundBeamsSpeed: normalizeNumber(input.massageLabBackgroundBeamsSpeed, fallback.massageLabBackgroundBeamsSpeed, 0.25, 2),
    massageLabBackgroundBeamsIntensity: normalizeNumber(input.massageLabBackgroundBeamsIntensity, fallback.massageLabBackgroundBeamsIntensity, 0.1, 1),
    massageLabBackgroundBeamsBeamWidth: normalizeNumber(input.massageLabBackgroundBeamsBeamWidth, fallback.massageLabBackgroundBeamsBeamWidth, 0.2, 2),
    massageLabBackgroundBeamsGlowStrength: normalizeNumber(input.massageLabBackgroundBeamsGlowStrength, fallback.massageLabBackgroundBeamsGlowStrength, 0, 20),
    massageLabCollisionBeamsSpeed: normalizeNumber(input.massageLabCollisionBeamsSpeed, fallback.massageLabCollisionBeamsSpeed, 0.25, 2),
    massageLabCollisionBeamsIntensity: normalizeNumber(input.massageLabCollisionBeamsIntensity, fallback.massageLabCollisionBeamsIntensity, 0.1, 1),
    massageLabCollisionBeamsBeamWidth: normalizeNumber(input.massageLabCollisionBeamsBeamWidth, fallback.massageLabCollisionBeamsBeamWidth, 0.5, 4),
    massageLabCollisionBeamsBurstSize: normalizeNumber(input.massageLabCollisionBeamsBurstSize, fallback.massageLabCollisionBeamsBurstSize, 0.5, 2),
    massageLabGlowingStarsSpeed: normalizeNumber(input.massageLabGlowingStarsSpeed, fallback.massageLabGlowingStarsSpeed, 0.25, 2),
    massageLabGlowingStarsIntensity: normalizeNumber(input.massageLabGlowingStarsIntensity, fallback.massageLabGlowingStarsIntensity, 0.1, 1),
    massageLabGlowingStarsActiveStars: normalizeInteger(input.massageLabGlowingStarsActiveStars, fallback.massageLabGlowingStarsActiveStars, 1, 18),
    massageLabGlowingStarsStarSize: normalizeNumber(input.massageLabGlowingStarsStarSize, fallback.massageLabGlowingStarsStarSize, 0.5, 3),
    massageLabGlowingStarsGlowStrength: normalizeNumber(input.massageLabGlowingStarsGlowStrength, fallback.massageLabGlowingStarsGlowStrength, 0, 2),
    massageLabMeteorsSpeed: normalizeNumber(input.massageLabMeteorsSpeed, fallback.massageLabMeteorsSpeed, 0.25, 2),
    massageLabMeteorsIntensity: normalizeNumber(input.massageLabMeteorsIntensity, fallback.massageLabMeteorsIntensity, 0.1, 1),
    massageLabMeteorsCount: normalizeInteger(input.massageLabMeteorsCount, fallback.massageLabMeteorsCount, 4, 48),
    massageLabMeteorsSize: normalizeNumber(input.massageLabMeteorsSize, fallback.massageLabMeteorsSize, 0.5, 5),
    massageLabMeteorsTailLength: normalizeNumber(input.massageLabMeteorsTailLength, fallback.massageLabMeteorsTailLength, 15, 140),
    backgroundLinesDuration: normalizeNumber(input.backgroundLinesDuration, fallback.backgroundLinesDuration, 4, 18),
    backgroundLinesIntensity: normalizeNumber(input.backgroundLinesIntensity, fallback.backgroundLinesIntensity, 0.1, 1),
    backgroundLinesCount: normalizeInteger(input.backgroundLinesCount, fallback.backgroundLinesCount, 6, 26),
    backgroundLinesWidth: normalizeNumber(input.backgroundLinesWidth, fallback.backgroundLinesWidth, 0.5, 6),
    backgroundLinesGlowStrength: normalizeNumber(input.backgroundLinesGlowStrength, fallback.backgroundLinesGlowStrength, 0, 24),
    ...
    tileGridChangeFrequency: shouldMigrateTileGridFadeDefault
      ? TILE_GRID_FADE_SECONDS_DEFAULT
      : clampTileGridFadeSeconds(input.tileGridChangeFrequency, fallback.tileGridChangeFrequency),
    massageLabGridFadeDefaultsVersion: MASSAGE_LAB_GRID_FADE_DEFAULTS_VERSION,
    ...
    hexGridChangeFrequency: shouldMigrateHexGridFadeDefault
      ? TILE_GRID_FADE_SECONDS_DEFAULT
      : clampTileGridFadeSeconds(input.hexGridChangeFrequency, fallback.hexGridChangeFrequency),
...
export function sanitizeChimerSettingsForEntitlements(
  ...
  // Chimer callers that hold the registry snapshot inject its per-background
  // decision. That keeps free choices and future registry changes aligned with
  // selection instead of maintaining a second entitlement allowlist here.
  const canUseSelectedBackground = typeof options.canUseSelectedBackground === "function"
    ? options.canUseSelectedBackground(settings.backgroundId) === true
    : canUsePremiumBackgrounds

  if (canUseSelectedBackground) {
    return settings
  }

  const sanitizedSettings = {
    ...settings,
    ...(!canUseSelectedBackground ? {
      ...
      ...DEFAULT_STATIC_GRADIENT_CHIMER_SETTINGS,
      staticGradientStopPositions: [...DEFAULT_STATIC_GRADIENT_CHIMER_SETTINGS.staticGradientStopPositions],
      ...
      massageLabDotGridSimulateCursorInteraction: DEFAULT_CHIMER_SETTINGS.massageLabDotGridSimulateCursorInteraction,
      massageLabDotGridSimulationSpeed: DEFAULT_CHIMER_SETTINGS.massageLabDotGridSimulationSpeed,
      ...
      massageLabGridDistortionSimulateCursorInteraction: DEFAULT_CHIMER_SETTINGS.massageLabGridDistortionSimulateCursorInteraction,
      massageLabGridDistortionSimulationSpeed: DEFAULT_CHIMER_SETTINGS.massageLabGridDistortionSimulationSpeed,
      ...
      massageLabShapeGridSpeedVersion: DEFAULT_CHIMER_SETTINGS.massageLabShapeGridSpeedVersion,
      ...
      massageLabPhotonBeamDefaultsVersion: DEFAULT_CHIMER_SETTINGS.massageLabPhotonBeamDefaultsVersion,
      ...
      massageLabAuroraSpeed: DEFAULT_CHIMER_SETTINGS.massageLabAuroraSpeed,
      massageLabAuroraIntensity: DEFAULT_CHIMER_SETTINGS.massageLabAuroraIntensity,
      massageLabAuroraBlur: DEFAULT_CHIMER_SETTINGS.massageLabAuroraBlur,
      massageLabAuroraReach: DEFAULT_CHIMER_SETTINGS.massageLabAuroraReach,
      massageLabDottedGlowSpeed: DEFAULT_CHIMER_SETTINGS.massageLabDottedGlowSpeed,
      massageLabDottedGlowDotSize: DEFAULT_CHIMER_SETTINGS.massageLabDottedGlowDotSize,
      massageLabDottedGlowDotSpacing: DEFAULT_CHIMER_SETTINGS.massageLabDottedGlowDotSpacing,
      massageLabDottedGlowOpacity: DEFAULT_CHIMER_SETTINGS.massageLabDottedGlowOpacity,
      massageLabDottedGlowGlowStrength: DEFAULT_CHIMER_SETTINGS.massageLabDottedGlowGlowStrength,
      massageLabBubbleSpeed: DEFAULT_CHIMER_SETTINGS.massageLabBubbleSpeed,
      massageLabBubbleIntensity: DEFAULT_CHIMER_SETTINGS.massageLabBubbleIntensity,
      massageLabBubbleSize: DEFAULT_CHIMER_SETTINGS.massageLabBubbleSize,
      massageLabBubbleBlur: DEFAULT_CHIMER_SETTINGS.massageLabBubbleBlur,
      massageLabBubbleBlendStrength: DEFAULT_CHIMER_SETTINGS.massageLabBubbleBlendStrength,
      massageLabBackgroundBeamsSpeed: DEFAULT_CHIMER_SETTINGS.massageLabBackgroundBeamsSpeed,
      massageLabBackgroundBeamsIntensity: DEFAULT_CHIMER_SETTINGS.massageLabBackgroundBeamsIntensity,
      massageLabBackgroundBeamsBeamWidth: DEFAULT_CHIMER_SETTINGS.massageLabBackgroundBeamsBeamWidth,
      massageLabBackgroundBeamsGlowStrength: DEFAULT_CHIMER_SETTINGS.massageLabBackgroundBeamsGlowStrength,
      massageLabCollisionBeamsSpeed: DEFAULT_CHIMER_SETTINGS.massageLabCollisionBeamsSpeed,
      massageLabCollisionBeamsIntensity: DEFAULT_CHIMER_SETTINGS.massageLabCollisionBeamsIntensity,
      massageLabCollisionBeamsBeamWidth: DEFAULT_CHIMER_SETTINGS.massageLabCollisionBeamsBeamWidth,
      massageLabCollisionBeamsBurstSize: DEFAULT_CHIMER_SETTINGS.massageLabCollisionBeamsBurstSize,
      massageLabGlowingStarsSpeed: DEFAULT_CHIMER_SETTINGS.massageLabGlowingStarsSpeed,
      massageLabGlowingStarsIntensity: DEFAULT_CHIMER_SETTINGS.massageLabGlowingStarsIntensity,
      massageLabGlowingStarsActiveStars: DEFAULT_CHIMER_SETTINGS.massageLabGlowingStarsActiveStars,
      massageLabGlowingStarsStarSize: DEFAULT_CHIMER_SETTINGS.massageLabGlowingStarsStarSize,
      massageLabGlowingStarsGlowStrength: DEFAULT_CHIMER_SETTINGS.massageLabGlowingStarsGlowStrength,
      massageLabMeteorsSpeed: DEFAULT_CHIMER_SETTINGS.massageLabMeteorsSpeed,
      massageLabMeteorsIntensity: DEFAULT_CHIMER_SETTINGS.massageLabMeteorsIntensity,
      massageLabMeteorsCount: DEFAULT_CHIMER_SETTINGS.massageLabMeteorsCount,
      massageLabMeteorsSize: DEFAULT_CHIMER_SETTINGS.massageLabMeteorsSize,
      massageLabMeteorsTailLength: DEFAULT_CHIMER_SETTINGS.massageLabMeteorsTailLength,
      backgroundLinesDuration: DEFAULT_CHIMER_SETTINGS.backgroundLinesDuration,
      backgroundLinesIntensity: DEFAULT_CHIMER_SETTINGS.backgroundLinesIntensity,
      backgroundLinesCount: DEFAULT_CHIMER_SETTINGS.backgroundLinesCount,
      backgroundLinesWidth: DEFAULT_CHIMER_SETTINGS.backgroundLinesWidth,
      backgroundLinesGlowStrength: DEFAULT_CHIMER_SETTINGS.backgroundLinesGlowStrength,
 ...
 * @param {{backgroundPreferenceOptions?: {getVisualPropertyKeys?: (backgroundId: string) => readonly string[] | null}, canUseAccountColorControls?: boolean, canUseSelectedBackground?: (backgroundId: string) => boolean, ownedBackgroundIds?: readonly string[]}} options
 */
