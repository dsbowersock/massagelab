...
// The outer authored layer stays bounded while extending beyond every viewport
// orientation. Scale changes the inner-layer progression, not this allocation.
export const TWISTED_CUBES_VIEWPORT_EXTENT_VMAX = 120

const TWISTED_CUBES_OPTION_BOUNDS = Object.freeze({
...
function sanitizeTwistedCubesOption(key, value) {
  const bounds = TWISTED_CUBES_OPTION_BOUNDS[key]
...
function resolveOutlineAnchors(anchors, sourceAnchors) {
  return Array.from({ length: OUTLINE_ANCHOR_COUNT }, (_, index) => {
    const resolved = parseHexColor(anchors?.[index]) ?? parseHexColor(sourceAnchors?.[index])
...
/**
 * Shrinks or expands the inner cube progression while anchoring the outermost
 * layer to the bounded viewport envelope. This keeps several outer layers
 * edge-filling at the approved 30% default without allocating an enormous
 * transform-scaled 3D surface.
 */
export function getTwistedCubeLayerSizeVmax({ oneBasedIndex, count, scale } = {}) {
  ...
  const progress = coordinates.oneBasedIndex / coordinates.count
  const scaledProgress = progress * (effectiveScale + (1 - effectiveScale) * progress)
  return TWISTED_CUBES_VIEWPORT_EXTENT_VMAX * scaledProgress
}
