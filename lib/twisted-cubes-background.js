...
// Maps the source's large vmin depth range into a visible, bounded orthographic
// projection. At the maximum layer count and spacing the front scale remains
// below 1.51, preserving the no-perspective wireframe composition.
export const TWISTED_CUBES_DEPTH_PROJECTION_DIVISOR = 4000

export const TWISTED_CUBES_OPTION_BOUNDS = Object.freeze({
  layerCount: Object.freeze({ minimum: 6, maximum: 30, integer: true }),
  rotationSpeed: Object.freeze({ minimum: 0.01, maximum: 2 }),
 ...
 * Accepted anchors are six-digit hex or space-separated `hsl(H S% L%)` only;
 * every other CSS color syntax falls back to its corresponding source anchor.
 */
...
/** Projects count-relative depth into a bounded orthographic layer scale. */
export function getTwistedCubeDepthScale({ oneBasedIndex, count, layerDepthSpacing } = {}) {
  const coordinates = normalizeLayerCoordinates({ oneBasedIndex, count })
  const spacing = sanitizeTwistedCubesOption("layerDepthSpacing", layerDepthSpacing)
  const depth = (coordinates.count - coordinates.oneBasedIndex) * spacing
  return 1 + depth / TWISTED_CUBES_DEPTH_PROJECTION_DIVISOR
}
