...
function encryptionKey(rawKey) {
  ...
  return crypto.createHash("sha256").update(normalizedKey || "development-only-totp-key").digest()
}
 ...
 * @param {string} [rawKey]
 */
export function encryptSecret(secret, rawKey = process.env.TOTP_ENCRYPTION_KEY ?? "") {
 ...
 * @param {string} [rawKey]
 */
export function decryptSecret(payload, rawKey = process.env.TOTP_ENCRYPTION_KEY ?? "") {
  ...
  const decipher = crypto.createDecipheriv("aes-256-gcm", encryptionKey(rawKey), Buffer.from(ivValue, "base64url"))
