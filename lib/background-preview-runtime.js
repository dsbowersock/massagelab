...
/**
 * Chooses one codec from the supplied rendition metadata without exposing
 * alternate sources. VP9 wins when the browser reports either support level.
 */
export function chooseSupportedPreviewCodec(renditions, canPlayType) {
  if (!Array.isArray(renditions) || typeof canPlayType !== "function") return null
  for (const codec of CODECS) {
    const rendition = renditions.find((candidate) => candidate?.codec === codec)
    if (!rendition || typeof rendition.mimeType !== "string") continue
    try {
      const support = canPlayType(rendition.mimeType)
