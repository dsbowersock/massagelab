...
function normalizeVisualPreset(value, backgroundId, options) { if (!isRecord(value) || !visualKeys(backgroundId, options)) return null; const id = normalizeId(value.id); if (!id) return null; return { id, name: normalizeName(value.name), timestamp: normalizeTimestamp(value.timestamp ?? value.updatedAt ?? value.createdAt), properties: normalizeProperties(value.properties, backgroundId, options) } }
function normalizeMappings(value, options) {
  ...
  for (const [id, mapping] of Object.entries(value)) {
    ...
    const assignments = Object.fromEntries(
      Object.entries(mapping).filter(([roleId, index]) => (
...
/** Renames a saved Color preset while preserving its palette and mapping payload. */
...
/** Applies a saved Color preset to the persisted palette and its per-background mappings. */
...
/** Returns a transient renderer-properties value after applying a Visual preset; it does not alter the persisted schema. */
