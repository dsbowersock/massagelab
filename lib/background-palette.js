...
function normalizeColorPreset(value) {
...
export function normalizeSharedBackgroundVisualPreferences(value, options = {}) {
  ...
  return { version: BACKGROUND_PALETTE_VERSION, palette: normalizeBackgroundPaletteState(input.palette), colorPresets: (Array.isArray(input.colorPresets) ? input.colorPresets : []).map((preset) => normalizeColorPreset(preset)).filter(Boolean).slice(0, BACKGROUND_COLOR_PRESET_LIMIT), mappingsByBackground: normalizeMappings(input.mappingsByBackground, options), visualPresetsByBackground, defaultVisualPresetByBackground: defaults }
}
...
export function applyBackgroundVisualPreset(preferences, backgroundId, id, properties, options = {}) { const current = normalizeSharedBackgroundVisualPreferences(preferences, options); const preset = (current.visualPresetsByBackground[backgroundId] ?? []).find((item) => item.id === id); return preset ? { ...current, properties: { ...normalizeProperties(properties, backgroundId, options), ...preset.properties }, mapping: clone(preset.mapping) } : { ...current, properties: normalizeProperties(properties, backgroundId, options), mapping: {} } }
