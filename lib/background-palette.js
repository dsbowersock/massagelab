import { DEFAULT_BACKGROUND_ID } from "./background-options.js"
...
/**
 * Returns whether a selected compatible background may use saved Custom or
 * Harmony values. Signed-in account color access intentionally covers only the
 * original free Lamp; other backgrounds still require the feature or ownership.
 */
export function canCustomizeBackgroundColors({ hasCustomColorFeature, hasAccountColorAccess = false, selectedBackgroundId, permanentlyOwnedBackgroundIds }) {
  return hasCustomColorFeature === true
    || (hasAccountColorAccess === true && selectedBackgroundId === DEFAULT_BACKGROUND_ID)
    || (
      typeof selectedBackgroundId === "string"
      && Array.isArray(permanentlyOwnedBackgroundIds)
      && permanentlyOwnedBackgroundIds.includes(selectedBackgroundId)
    )
}
