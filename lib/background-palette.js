...
export const LEGACY_CHIMER_GLOBAL_COLOR_STORAGE_KEY = "massagelab-chimer-global-color-v1"
export const LEGACY_CHIMER_GLOBAL_PALETTE_STORAGE_KEY = "massagelab-chimer-global-palettes-v1"

export const LEGACY_BACKGROUND_COLOR_SETTING_KEYS = Object.freeze([
...
const LEGACY_BACKGROUND_COLOR_SETTING_KEY_SET = new Set(LEGACY_BACKGROUND_COLOR_SETTING_KEYS)
const LEGACY_BACKGROUND_PALETTE_METADATA_SUFFIXES = Object.freeze([
  "PaletteMode",
  "PrimaryColor",
  "Harmony",
])

/** Removes renderer color fallbacks and obsolete per-renderer palette metadata from persisted Chimer state. */
export function omitLegacyBackgroundColorSettings(value) {
  if (!isRecord(value)) return {}
  return Object.fromEntries(Object.entries(value).filter(([key]) => (
    !LEGACY_BACKGROUND_COLOR_SETTING_KEY_SET.has(key)
    && !LEGACY_BACKGROUND_PALETTE_METADATA_SUFFIXES.some((suffix) => key.endsWith(suffix))
  )))
}
...
const ID_LIMIT = 128
const TIMESTAMP_LIMIT = 8640000000000000
...
function normalizeTimestamp(value) { return typeof value === "number" && Number.isFinite(value) && value >= 0 ? Math.min(value, TIMESTAMP_LIMIT) : 0 }
...
function colorRoleIds(id, options) { const ids = options?.getColorRoleIds?.(id); return Array.isArray(ids) ? new Set(ids.filter((roleId) => typeof roleId === "string")) : null }
...
export function normalizeBackgroundColorMapping(value, adapter) {
  ...
  for (const role of roles) if (typeof role?.id === "string") result[role.id] = Number.isInteger(input[role.id]) && input[role.id] >= 0 && input[role.id] < BACKGROUND_PALETTE_SWATCH_COUNT ? input[role.id] : (Number.isInteger(role.defaultSwatch) && role.defaultSwatch >= 0 && role.defaultSwatch < BACKGROUND_PALETTE_SWATCH_COUNT ? role.defaultSwatch : 0)
  return result
}
...
export function resolveBackgroundRoleColors({ palette, adapter, mapping, canCustomize }) {
  ...
  // Source mode bypasses every saved or generated palette assignment.
  const sourceMode = normalized.mode === "source" || !canCustomize; const harmonySwatches = normalized.mode === "harmony" ? generateBackgroundHarmonySwatches(normalized.primaryColor, normalized.harmony) : normalized.swatches
  return Object.fromEntries(roles.filter((role) => typeof role?.id === "string").map((role) => {
    if (sourceMode) return [role.id, normalizeHex(role.sourceColor, DEFAULT_BACKGROUND_PALETTE_STATE.primaryColor)]
    // Harmony roles use generated harmony swatches unless the adapter keeps a
    // role, such as a background color, bound to its independently saved swatch.
    const swatches = normalized.mode === "harmony" && role.harmonyColorSource !== "saved-swatch" ? harmonySwatches : normalized.swatches
    return [role.id, swatches[assignments[role.id]]]
  }))
}

function normalizeProperties(value, backgroundId, options) {
  const allowed = visualKeys(backgroundId, options)
  if (!allowed || !isRecord(value)) return {}
  const filtered = Object.fromEntries(Object.entries(value).filter(([key]) => allowed.has(key)))
  if (typeof options?.normalizeVisualProperties !== "function") return filtered
  const normalized = options.normalizeVisualProperties(clone(filtered), backgroundId)
  if (!isRecord(normalized)) return {}
  return Object.fromEntries(Object.entries(normalized).filter(([key]) => allowed.has(key)))
}
/** Projects a Color preset to its canonical cross-background palette-only shape. */
export function normalizeBackgroundColorPreset(value) {
  ...
  return { id, name: normalizeName(value.name), timestamp: normalizeTimestamp(value.timestamp ?? value.updatedAt ?? value.createdAt), palette }
}
function normalizeRoleMapping(value, allowedRoleIds) {
  if (!isRecord(value) || !allowedRoleIds) return {}
  return Object.fromEntries(
    Object.entries(value).filter(([roleId, index]) => (
...
function normalizeVisualPreset(value, backgroundId, options) {
  const allowedVisualKeys = visualKeys(backgroundId, options)
  const allowedRoleIds = colorRoleIds(backgroundId, options)
  if (!isRecord(value) || !allowedVisualKeys || !allowedRoleIds) return null
  const id = normalizeId(value.id)
  if (!id) return null
  return {
    id,
    name: normalizeName(value.name),
    timestamp: normalizeTimestamp(value.timestamp ?? value.updatedAt ?? value.createdAt),
    properties: normalizeProperties(value.properties, backgroundId, options),
    mapping: normalizeRoleMapping(value.mapping, allowedRoleIds),
  }
}
function normalizeMappings(value, options) {
  ...
  for (const [id, mapping] of Object.entries(value)) {
    if (!knownBackground(id, options) || !isRecord(mapping)) continue
    const allowedRoleIds = colorRoleIds(id, options)
    if (!allowedRoleIds || allowedRoleIds.size === 0) continue
    const assignments = normalizeRoleMapping(mapping, allowedRoleIds)
    if (Object.keys(assignments).length > 0) normalized[id] = assignments
  }
  return normalized
}
...
export function normalizeSharedBackgroundVisualPreferences(value, options = {}) {
  ...
  if (isRecord(input.visualPresetsByBackground)) for (const [id, presets] of Object.entries(input.visualPresetsByBackground)) if (knownBackground(id, options) && Array.isArray(presets)) { const normalized = presets.map((preset) => normalizeVisualPreset(preset, id, options)).filter(Boolean).slice(0, BACKGROUND_VISUAL_PRESET_LIMIT); if (normalized.length) visualPresetsByBackground[id] = normalized }
  const defaults = {}
  ...
  return { version: BACKGROUND_PALETTE_VERSION, palette: normalizeBackgroundPaletteState(input.palette), colorPresets: (Array.isArray(input.colorPresets) ? input.colorPresets : []).map((preset) => normalizeBackgroundColorPreset(preset)).filter(Boolean).slice(0, BACKGROUND_COLOR_PRESET_LIMIT), mappingsByBackground: normalizeMappings(input.mappingsByBackground, options), visualPresetsByBackground, defaultVisualPresetByBackground: defaults }
}
...
/** Removes a saved Color preset; applying a missing ID intentionally leaves preferences unchanged. */
...
/** Applies a cross-background Color preset without changing any background mapping. */
export function applyBackgroundColorPreset(preferences, id, options = {}) { const current = normalizeSharedBackgroundVisualPreferences(preferences, options); const preset = current.colorPresets.find((item) => item.id === id); return preset ? replacePreferences(current, { palette: preset.palette }, options) : current }
...
/** Renames one saved Visual preset without changing its renderer property payload. */
...
/** Deletes one Visual preset and clears it when it was the persisted default for that background. */
...
/** Returns transient renderer properties and mapping after applying a Visual preset. */
export function applyBackgroundVisualPreset(preferences, backgroundId, id, properties, options = {}) { const current = normalizeSharedBackgroundVisualPreferences(preferences, options); const preset = (current.visualPresetsByBackground[backgroundId] ?? []).find((item) => item.id === id); return preset ? { ...current, properties: { ...normalizeProperties(properties, backgroundId, options), ...preset.properties }, mapping: clone(preset.mapping) } : { ...current, properties: normalizeProperties(properties, backgroundId, options), mapping: clone(current.mappingsByBackground[backgroundId] ?? {}) } }
/** Resolves the valid persisted default Visual preset, or null after normalization removes a stale reference. */
...
function legacyGlobalColorRecord(value) {
  const parsed = parseJson(value)
  if (!isRecord(parsed)) return null
  const colors = isRecord(parsed.colors) ? parsed.colors : parsed
  const harmony = parsed.harmony
  if (!LEGACY_COLOR_FIELDS.every((key) => typeof colors[key] === "string" && HEX.test(colors[key].trim()))) return null
  if (harmony !== "custom" && !HARMONIES.has(harmony)) return null
  return { ...colors, harmony }
}

function legacyColorPresets(value) {
  const parsed = parseJson(value)
  if (!Array.isArray(parsed)) return []
  return parsed.flatMap((entry) => {
    if (!isRecord(entry) || !isRecord(entry.colors) || !Array.isArray(entry.generated)) return []
    const id = normalizeId(entry.id)
    const harmony = entry.harmony
    if (!id || (harmony !== "custom" && !HARMONIES.has(harmony))) return []
    if (!LEGACY_COLOR_FIELDS.every((key) => typeof entry.colors[key] === "string" && HEX.test(entry.colors[key].trim()))) return []
    const swatches = LEGACY_COLOR_FIELDS.map((key, index) => normalizeHex(entry.colors[key], DEFAULT_BACKGROUND_PALETTE_STATE.swatches[index]))
    const timestamp = typeof entry.createdAt === "string" ? Date.parse(entry.createdAt) : entry.timestamp
    return [{
      id,
      name: normalizeName(entry.name),
      timestamp: Number.isFinite(timestamp) ? timestamp : 0,
      palette: {
        mode: harmony === "custom" ? "custom" : "harmony",
        primaryColor: normalizeHex(entry.sourceColor, swatches[0]),
        harmony: harmony === "custom" ? DEFAULT_BACKGROUND_PALETTE_STATE.harmony : harmony,
        swatches,
      },
    }]
...
/**
 * Prepares one raw Chimer browser record for a local-first commit.
 * The caller removes legacy keys only after persisting the returned settings.
 *
 * @param {{
 *   rawChimerSettings?: unknown,
 *   rawLegacyGlobalColors?: unknown,
 *   rawLegacySavedPalettes?: unknown,
 *   sanitizeSettings?: (value: {}) => unknown,
 * }} [input]
 */
export function prepareChimerBackgroundPreferenceMigration({
  rawChimerSettings,
  rawLegacyGlobalColors,
  rawLegacySavedPalettes,
  sanitizeSettings = (value) => value,
} = {}) {
  const parsedSettings = parseJson(rawChimerSettings)
  let settings = isRecord(parsedSettings) ? clone(parsedSettings) : {}
  const nestedV1Exists = isRecord(settings.backgroundVisualPreferences)
    && settings.backgroundVisualPreferences.version === BACKGROUND_PALETTE_VERSION

  if (!nestedV1Exists) {
    const legacyColors = legacyGlobalColorRecord(rawLegacyGlobalColors)
    settings = legacyColors
      ? migrateLegacyChimerGlobalColors(settings, legacyColors)
      : {
          ...settings,
          backgroundVisualPreferences: normalizeSharedBackgroundVisualPreferences(),
        }
    settings.backgroundVisualPreferences = normalizeSharedBackgroundVisualPreferences({
      ...settings.backgroundVisualPreferences,
      colorPresets: legacyColorPresets(rawLegacySavedPalettes),
    })
  }

  const legacyKeysToRemove = []
  if (rawLegacyGlobalColors !== null && rawLegacyGlobalColors !== undefined) {
    legacyKeysToRemove.push(LEGACY_CHIMER_GLOBAL_COLOR_STORAGE_KEY)
  }
  if (rawLegacySavedPalettes !== null && rawLegacySavedPalettes !== undefined) {
    legacyKeysToRemove.push(LEGACY_CHIMER_GLOBAL_PALETTE_STORAGE_KEY)
  }
...
/**
 * Keeps background customization aligned with the canonical use decision.
 * Free, permanently owned, and membership-included backgrounds expose the
 * same Custom/Harmony and renderer-property controls; locked backgrounds do not.
 */
export function canCustomizeBackgroundColors({ hasBackgroundAccess }) {
  return hasBackgroundAccess === true
}
/** Makes access denial reversible by resolving only the effective renderer mode. */
export function resolveEffectiveBackgroundPaletteMode({ savedMode, canCustomize }) {
  if (savedMode === "source") return "source"
  if (canCustomize === true && MODES.has(savedMode)) return savedMode
  return "source"
}
