 ...
 * }} input
 * @returns {boolean} Whether motion should be reduced. The app's reduced mode
 * wins first; an explicit system-motion opt-in then overrides forced and
 * route-owned motion.
 */
export function shouldReduceAmbientMotion({
  ...
  // A renderer that opts into respecting the system signal cannot be forced
  // back on by route ownership or the explicit forceMotion escape hatch.
  if (prefersReducedMotion && respectSystemReducedMotion) {
    return true
  }
 ...
 *   respectSystemReducedMotion?: boolean
 * }} input
 * @returns {boolean} Whether the ambient background may animate after the same
 * strict reduced-motion precedence and viewport visibility gates are applied.
 */
