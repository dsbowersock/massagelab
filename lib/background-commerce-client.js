...
function safeReturnPath(value) {
  ...
  // Reject ambiguous URL syntax at the shared auth boundary; after enforcing
  // a single leading slash, colons remain ordinary app-local path characters.
  if (normalized.includes("\\") || /[\u0000-\u001f\u007f]/.test(normalized)) return "/"
  return normalized.split("#")[0]
}

/**
 * Preserves the app-local surface that opened a guest cart and optionally
 * marks the shared dialog cart to reopen after authentication without trusting
 * an external redirect.
 *
 * @param {unknown} pathname
 * @param {unknown} search
 * @param {boolean} [reopenDialogCart]
 * @returns {string}
 */
export function buildBackgroundCartAuthReturnPath(pathname, search, reopenDialogCart = true) {
  ...
  return `${safePath}${nextSearch ? `?${nextSearch}` : ""}`
}
...
/**
 * Allows a preference response to patch cached ownership only when no newer
 * commerce snapshot committed or mutation started after that preference
 * request began.
 *
 * @param {unknown} requestRevision
 * @param {unknown} currentRevision
 * @returns {boolean}
 */
export function shouldApplyPreferenceOwnershipProof(requestRevision, currentRevision) {
...
export function backgroundCommerceReducer(state, action) {
  ...
  switch (action.type) {
    ...
    case "ownership-reconcile": {
      if (!state.snapshot) return state
      const snapshot = normalizeBackgroundCommerceSnapshot({
        ...state.snapshot,
        ownedBackgroundIds: action.ownedBackgroundIds,
      })
      const ownedBackgroundIds = new Set(snapshot.ownedBackgroundIds)
      return {
        ...state,
        snapshot: {
          ...snapshot,
          // The preference route proves only active ownership IDs. Retain a
          // cached detail row only when its active/inactive state agrees with
          // that newer proof, without inventing acquisition provenance.
          ownerships: snapshot.ownerships.filter((ownership) => (
            (ownership.status === "active")
              === ownedBackgroundIds.has(ownership.backgroundId)
          )),
        },
      }
    }
...
export function backgroundCardCommerceState({ background, access, snapshot }) {
  ...
  if (backgroundId && background?.enabled !== false) {
    ...
    } else if (access?.canUse === true && access?.accessSource === "ownership") {
      // A just-completed acquisition may reach the access bridge before the
      // provider snapshot refreshes. Preserve ownership without guessing how
      // it was acquired; an authoritative inactive row above still wins.
      state = "owned"
    ...
    } else if (access?.canUse === true && access?.accessSource === "temporary") {
      // Temporary support is a separate, expiring commerce state and must not
      // be presented as subscription-backed membership access.
      state = "included-temporary"
    } else if (access?.canUse === true && access?.accessSource === "admin") {
      state = "included-admin"
  ...
  return {
    state,
    canSelect,
    showKeepPermanently: ["included-subscription", "included-temporary", "included-admin"].includes(state),
    isInCart,
    isReserved,
...
/**
 * Returns true only when the authoritative commerce snapshot proves an active
 * permanent purchase or credit acquisition. A transient access bridge remains
 * selectable through `state: "owned"` but must not claim permanent ownership.
 */
export function hasActivePermanentOwnership(commerceState) {
  return commerceState?.ownershipStatus === "active"
    && (
      commerceState.state === "owned-credit"
        && commerceState.ownershipSource === "credit"
      || commerceState.state === "owned-purchase"
        && commerceState.ownershipSource === "purchase"
    )
}
