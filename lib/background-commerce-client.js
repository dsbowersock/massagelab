...
function normalizeOwnerships(value) {
  ...
  for (const candidate of value) {
    ...
    ownerships.push({ backgroundId, source, status, acquiredAt })
  }
...
function normalizeCartItems(value) {
  ...
  for (const candidate of value) {
    ...
    if (productType !== "background" || !productKey || !displayName) continue
    ...
    items.push({
      ...
      productKey,
      displayName,
      unitAmount: nonnegativeInteger(candidate.unitAmount),
      currency: "usd",
...
function normalizeCart(value) {
  ...
  return {
    ...
    subtotalAmount: nonnegativeInteger(value.subtotalAmount),
    currency: "usd",
 ...
 * Successful reads and mutations always replace the full normalized snapshot.
 */
