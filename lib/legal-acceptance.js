...
/**
 * Captures the exact current policy set for one order after an explicit,
 * unchecked-by-default combined consent. Account-level history is deliberately
 * not consulted because every digital purchase requires fresh consent.
 *
 * @param {{ acceptedDocumentIds: unknown, combinedConsentAccepted: unknown, now?: Date }} input
 * @returns {{ documentIds: string[], documentVersions: Record<string, string>, combinedConsentAccepted: true, acceptedAt: string }}
 */
export function buildDigitalPurchaseConsent({
  acceptedDocumentIds,
  combinedConsentAccepted,
  now = new Date(),
}) {
  const documents = requiredLegalDocumentsForEvent("digital-purchase")
  const missing = combinedConsentAccepted === true
    ? missingRequiredLegalDocuments({ acceptedDocumentIds, documents })
    : documents

  if (missing.length > 0) {
    throw new Error("Explicit current digital purchase consent is required.")
  }

  return {
    documentIds: documents.map((document) => legalDocumentAcceptanceId(document)),
    documentVersions: Object.fromEntries(
      documents.map((document) => [document.key, document.version]),
    ),
