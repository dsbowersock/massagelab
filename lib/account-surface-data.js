 ...
 * @typedef {{ role?: string, roles?: string[], roleAssignments?: Array<{ role: string, status: string }>, capabilities?: { canUsePremiumBackgrounds?: boolean, hasActiveMembershipBenefits?: boolean } }} AccountSessionUser
 * @typedef {(prismaClient: AccountPrismaClient, userId: string, now: Date) => Promise<any>} MembershipSummaryLoader
 ...
 * @typedef {{ prismaClient: AccountPrismaClient, userId: string, now: Date, getMembershipSummary: MembershipSummaryLoader, getPricingCatalog: PricingCatalogLoader }} MembershipLoadInput
...
export function sessionHasActiveMembershipBenefits(sessionUser) {
  ...
  return Boolean(
    capabilities?.hasActiveMembershipBenefits
      ?? capabilities?.canUsePremiumBackgrounds,
  )
}
...
/**
 * Loads a fresh, bounded activity projection for the signed-in account.
 * Callers must invoke this loader per request rather than placing the result in
 * a shared TTL cache. The projection intentionally excludes the internal actor,
 * notes, failure codes, and delivery metadata.
 *
 * @param {{ prismaClient: AccountPrismaClient, userId: string }} input Database
 * client and authenticated target user identifier.
 * @returns {Promise<{ surface: "activity", activity: Array<{
 *   id: string,
 *   title: string,
 *   explanation: string,
 *   effectiveValue: string | null,
 *   occurredAt: string
 * }> }>} Newest-first safe activity rows, capped at fifty.
 */
async function loadActivitySurface({ prismaClient, userId }) {
  const activity = await prismaClient.userAccountActivity.findMany({
    where: { userId },
...
/**
 * Membership authority remains request-fresh while public display pricing is
 * delegated to the shared catalog owner in membership-pricing.js.
 *
 * @param {MembershipLoadInput} input
 */
async function loadMembershipSurface({ prismaClient, userId, now, getMembershipSummary, getPricingCatalog }) {
  const [membershipSummary, pricingCatalog] = await Promise.all([
    getMembershipSummary(prismaClient, userId, now),
    getPricingCatalog(),
  ])
...
export function createAccountSurfaceDataLoader({
  ...
  /**
   * Only session-independent payloads may enter this shared TTL cache.
   * Request-scoped overview role and membership claims are appended after
   * retrieval so one session cannot reuse another session's claims.
   *
   * @type {Map<string, CacheableSurfaceLoader>}
   */
  const cacheableSurfaceLoaders = new Map([
  ...
  async function getAccountSurfaceData(surface, userId, sessionUser) {
    ...
    if (normalizedSurface === "credentials") {
      // Credentials are request-scoped; never place them in the shared TTL cache.
    ...
    if (normalizedSurface === "activity") {
      // Activity is support-authored evidence and must reflect the current
      // request instead of a shared TTL snapshot.
      return loadActivitySurface({
        prismaClient: await resolvePrismaClient(prismaClient),
        userId,
      })
    }
    if (normalizedSurface === "membership") {
      // Entitlement expiry is request-time authority. Only the independent
      // public pricing catalog may reuse its short-lived cache.
    ...
    if (!cacheableSurfaceLoaders.has(normalizedSurface)) {
      // Local surfaces derive permissions from current session claims.
