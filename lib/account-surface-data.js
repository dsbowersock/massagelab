 ...
 */
function sessionHasActiveMembershipBenefits(sessionUser) {
  return Boolean(sessionUser?.capabilities?.hasActiveMembershipBenefits)
}
...
export function createAccountSurfaceDataLoader({
  ...
  async function loadSurfaceData(key) {
    ...
    if (surface === "overview") {
      return loadOverviewSurface({ prismaClient: resolvedPrismaClient, userId })
    }
