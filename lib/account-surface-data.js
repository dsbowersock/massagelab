 ...
 * @typedef {(input: { prismaClient: AccountPrismaClient, userId: string }) => Promise<any>} CommerceSnapshotLoader
...
async function loadBackgroundCommerceSurface({ prismaClient, userId, getCommerceSnapshot }) {
  const [snapshot, orderRows] = await Promise.all([
    getCommerceSnapshot({ prismaClient, userId }),
    ...
    backgroundCommerce: {
      ...
      orders: orderRows.map((order) => ({
        ...
        items: order.items.map((item) => ({
          ...
          refundedAmount: item.refundItems.reduce((sum, refundItem) => sum + refundItem.amountCents, 0),
...
export function createAccountSurfaceDataLoader({
  ...
  // Short-lived per-key side channel for loadSurfaceData to synchronously read
  // the sessionUser supplied to surfaceDataCache.get(key). This relies on the
  // cache coalescing concurrent loads for the same key; if that changes, or if
  // loaders become asynchronous before reading this map, replace this with an
  // explicit per-request context passed into loadSurfaceData to avoid using a
  // stale or wrong sessionUser.
  ...
  async function getAccountSurfaceData(surface, userId, sessionUser) {
    ...
    // Store the request sessionUser only for the synchronous cache load window.
