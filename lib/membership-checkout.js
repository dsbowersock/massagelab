...
export function createMembershipCheckoutPostHandler({
  ...
  return async function POST(request) {
    ...
    try {
      ...
      if (input.isForm && !isTrustedCheckoutFormOrigin(request)) {
        return accountRedirect("invalid-request")
      }
...
/**
 * Rejects browser form posts that identify another origin before any legal or
 * billing state can be mutated. Requests without browser origin metadata stay
 * compatible with non-browser clients, while Fetch Metadata and Referer offer
 * a fallback when Origin is absent.
 * @param {Request} request
 */
function isTrustedCheckoutFormOrigin(request) {
