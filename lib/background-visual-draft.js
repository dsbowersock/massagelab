...
const SNAPSHOT_KEYS = ["palette", "colorPresets", "properties", "mapping", "visualPresets", "defaultVisualPresetId"]
...
function normalizeSnapshot(value) {
  ...
  return {
    ...
    colorPresets: list(input.colorPresets, BACKGROUND_COLOR_PRESET_LIMIT),
...
function colorPresetMapping(preset, action, current) {
  if (isRecord(preset.mapping)) return preset.mapping
  if (!isRecord(preset.mappingsByBackground)) return current
  const id = presetId(action.backgroundId)
  if (id && isRecord(preset.mappingsByBackground[id])) return preset.mappingsByBackground[id]
  const mappings = Object.values(preset.mappingsByBackground).filter(isRecord)
  return mappings.length === 1 ? mappings[0] : current
}
...
export function reduceBackgroundVisualDraft(state, action = {}) {
  ...
  if (type === "reset-colors") return transition(currentState, { ...current, palette: { ...current.palette, mode: "source" } })
  ...
  if (type === "apply-color-preset") {
    ...
    return preset ? transition(currentState, { ...current, palette: preset.palette, mapping: colorPresetMapping(preset, action, current.mapping) }) : currentState
  }
  if (type === "apply-visual-preset") {
    const preset = current.visualPresets.find((item) => item.id === presetId(action.id))
    return preset ? transition(currentState, { ...current, properties: preset.properties, mapping: isRecord(preset.mapping) ? preset.mapping : current.mapping }) : currentState
  }
  if (type === "save-color-preset" || type === "update-color-preset") return transition(currentState, { ...current, colorPresets: replacePreset(current.colorPresets, action.preset, BACKGROUND_COLOR_PRESET_LIMIT) })
