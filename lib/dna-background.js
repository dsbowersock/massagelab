...
/** Rounds a converted display value without leaking binary-float artifacts. */
function roundToControlStep(value, step) {
...
export function getDnaNodeMotionDisplaySpeed(speed) {
  const displaySpeed = sanitizeDnaOption("nodeMotionSpeed", speed) / DNA_NODE_MOTION_ONE_X
  return roundToControlStep(displaySpeed, DNA_SPEED_MULTIPLIER_BOUNDS.step)
}
...
export function getDnaStrandRotationDisplaySpeed(speed) {
  const displaySpeed = sanitizeDnaOption("strandRotationSpeed", speed) / DNA_STRAND_ROTATION_ONE_X
  return roundToControlStep(displaySpeed, DNA_SPEED_MULTIPLIER_BOUNDS.step)
}
...
export function getDnaScaleDisplayPercent(scale) {
  const displayPercent = (sanitizeDnaOption("scale", scale) / DNA_SCALE_AT_ONE_HUNDRED_PERCENT) * 100
  return roundToControlStep(displayPercent, DNA_SCALE_PERCENT_BOUNDS.step)
}
