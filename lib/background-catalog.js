...
/**
 * Selects a preview video and its poster from the same authored aspect variant.
 * This prevents portrait cards from pairing a vertical video with unrelated
 * square or landscape artwork while retaining the catalog's image fallback.
 *
 * @param {Record<string, any>} option
 * @param {"landscape"|"square"|"vertical"} [preferredVariant]
 */
export function getBackgroundPreviewAssets(option, preferredVariant = "landscape") {
  const variantPairs = preferredVariant === "vertical"
    ? [
        [option.previewVerticalVideoUrl, option.previewVerticalImageUrl],
        [option.previewSquareVideoUrl, option.previewSquareImageUrl],
        [option.previewVideoUrl, option.previewImageUrl],
      ]
    : preferredVariant === "square"
      ? [
          [option.previewSquareVideoUrl, option.previewSquareImageUrl],
  ...
  const authoredPair = variantPairs.find(([videoUrl]) => Boolean(videoUrl))
  if (authoredPair) {
    return { videoUrl: authoredPair[0], posterUrl: authoredPair[1] }
  }
  if (option.previewMediaType === "video" && option.previewMediaUrl) {
    return { videoUrl: option.previewMediaUrl, posterUrl: option.previewImageUrl }
  }

  const media = getBackgroundPreviewMedia(option, preferredVariant)
  return {
    videoUrl: media?.type === "video" ? media.source : undefined,
    posterUrl: media?.type === "image" ? media.source : undefined,
  }
}
...
/**
 * Matches canonical branding and search-only legacy labels without exposing
 * prior names in ordinary catalog presentation.
 *
 * @param {Record<string, any>} option
 * @param {unknown} query
 * @returns {boolean} Whether every normalized query term occurs in the
 * searchable canonical or legacy label text.
 */
export function matchesBackgroundSearch(option, query) {
  ...
  const searchable = [
    ...
    ...(Array.isArray(option.legacyLabels) ? option.legacyLabels : []),
  ].join(" ").toLowerCase()
