...
/**
 * Admits an async account-write completion only while its exact mounted owner,
 * sync permission, and request generation remain current.
 */
export function shouldApplyMusicVisualizerAccountWriteCompletion({
  currentOwner,
  currentRequestId,
  isMounted,
  requestId,
  ...
  return Boolean(
    ...
    && currentOwner?.ownerKey === requestOwnerKey
    && currentOwner?.syncEnabled === true
  )
}

/**
 * Projects only explicitly supplied visualizer fields into a normalized
 * account patch, preserving omitted fields for partial local intent merges.
 *
 * @param {unknown} value
 * @returns {Partial<MusicVisualizerAccountPreferences>}
 */
function projectMusicVisualizerAccountChanges(value) {
  ...
  if (Object.prototype.hasOwnProperty.call(source, "defaultBackgroundId")) {
    changes.defaultBackgroundId = normalizeBackgroundId(source.defaultBackgroundId)
  }
