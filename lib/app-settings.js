...
export function normalizeAppSettings(value) {
  ...
  return {
    appBarPosition: source.appBarPosition === "bottom" ? "bottom" : "top",
...
/**
 * Places the audio toolbar on the edge opposite the sitewide app bar so both
 * persistent controls remain discoverable without stacking over each other.
 *
 * @param {unknown} value
...
export function getAudioPlayerToolbarPlacement(value) {
  ...
  return settings.appBarPosition === "bottom" ? "top" : "bottom"
}
