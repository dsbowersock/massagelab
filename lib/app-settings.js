...
export function normalizeAppSettings(value) {
  ...
  const appBarPosition = source.appBarPosition === "top" ? "top" : "bottom"

  return {
    appBarPosition,
    sidebarPosition: source.sidebarPosition === "right" ? "right" : "left",
    // The main bar owns the vertical edge; retain this field only for stored-preference compatibility.
    sidebarTriggerPosition: appBarPosition,
...
/**
 * Applies only edits made while an authenticated bootstrap was still pending
 * over the eventual server snapshot, so newer local intent is not discarded.
 * `pendingLocalEdits` is intentionally non-null; callers must always supply an
 * object, even when it has no pending fields.
 *
 * @param {unknown} serverSettings
 * @param {Partial<AppSettings>} pendingLocalEdits
 * @returns {AppSettings}
 */
export function reconcileAppSettingsAfterBootstrap(serverSettings, pendingLocalEdits) {
  const definedPendingLocalEdits = Object.fromEntries(
    Object.entries(pendingLocalEdits).filter(([, value]) => value !== undefined),
  )
  return normalizeAppSettings({
    ...normalizeAppSettings(serverSettings),
    ...definedPendingLocalEdits,
  })
}
