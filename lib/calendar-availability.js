...
export const OUTSIDE_PROVIDER_AVAILABILITY_MESSAGE = "That time is outside the provider's availability."
...
/**
 * @param {{ availability: { intervals?: Array<{ startMinute: number, endMinute: number }> }, startMinute: number, endMinute: number, allowOutsideAvailability?: boolean }} input
 */
export function availabilityRangeOverrideState({
  availability,
  startMinute,
  endMinute,
  allowOutsideAvailability = false,
}) {
  const insideAvailability = availabilityContainsRange({ availability, startMinute, endMinute })
  if (insideAvailability) {
    return { allowed: true, outsideAvailability: false, message: "" }
  }

  return {
    allowed: Boolean(allowOutsideAvailability),
