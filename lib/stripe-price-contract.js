...
export function recurringPriceSemanticMismatches(
  ...
  const currencyOptionKeys = (
    ...
    : null
  // Expanded currency_options may be null or empty. When an option exists,
  // accept only one non-null object keyed by the Price's base currency whose
  // amount and tax behavior match the Price-level values.
  const baseCurrencyOption = currencyOptionKeys?.length === 1
    ? currencyOptions[currencyOptionKeys[0]]
    : null
  ...
  if (
    !Object.hasOwn(candidate ?? {}, "currency_options")
    || (
      currencyOptions != null
      && (
