...
export const supporterRoadmapInterestOptions = Object.freeze([
  Object.freeze({ id: "personal_wellness", label: "Personal wellness" }),
  Object.freeze({ id: "backgrounds_and_sound", label: "Backgrounds and sound" }),
  Object.freeze({ id: "therapist_tools", label: "Therapist tools" }),
  Object.freeze({ id: "practice_management", label: "Practice management" }),
  Object.freeze({ id: "anatomy_and_education", label: "Anatomy and education" }),
  Object.freeze({ id: "professional_documentation", label: "Professional documentation" }),
])
...
export const homeToolCatalog = Object.freeze([
  ...
  {
    key: "business_income_planner",
    title: "Business income planner",
    description: "Estimate session pricing, time off, take-home targets, and workload capacity.",
    href: "/tools/business-planner/income",
    action: "Open planner",
    icon: "Calculator",
    status: "Public + account worksheet",
    roleSignals: ["therapist", "student", "educator", "public_wellness"],
    useCaseSignals: ["manage_practice", "run_sessions"],
  },
  {
    key: "atmosphere",
    title: "Music",
    description: "Generative audio stations with a route-persistent player.",
    href: "/music",
    action: "Open Music",
    icon: "Radio",
    status: "Public audio",
...
/** @type {Set<string>} */
const supporterRoadmapInterestIds = new Set(supporterRoadmapInterestOptions.map((option) => option.id))

/**
 * Keeps the roadmap signal intentionally categorical: it accepts only the
 * approved interest IDs, preserving order while rejecting duplicate or
 * free-form values that could contain personal or clinical details.
 *
 * @param {unknown} value
 * @returns {string[]}
 */
export function normalizeSupporterRoadmapInterests(value) {
  ...
  for (const interest of value) {
    if (typeof interest === "string" && supporterRoadmapInterestIds.has(interest) && !seen.has(interest)) {
      seen.add(interest)
...
const defaultHomeToolsByRole = {
  therapist: ["business_income_planner", "local_notes", "calendar_booking", "chimer", "atmosphere", "anatomime"],
  student: ["education_flashcards", "business_income_planner", "anatomime", "chimer", "atmosphere", "account_memberships"],
  educator: ["anatomime", "education_flashcards", "business_income_planner", "chimer", "atmosphere", "account_memberships"],
...
const roleHintSignalByUseCase = {
  ...
  run_sessions: ["chimer", "business_income_planner", "atmosphere", "anatomime"],
  manage_practice: ["business_income_planner", "calendar_booking", "account_memberships"],
