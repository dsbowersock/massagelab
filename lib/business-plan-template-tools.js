...
function buildPolicySummary(input) {
  const populatedPrivacySteps = input.privacySteps.filter((step) => step.trim().length > 0)
  ...
  return [
    ...
    populatedPrivacySteps.length > 0
      ? `Privacy steps: ${populatedPrivacySteps.join(" ")}`
...
function normalizeStringList(value, fallback) {
  ...
  return Array.isArray(value) ? [] : [...fallback]
}
