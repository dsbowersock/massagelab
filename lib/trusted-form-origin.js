...
/**
 * Builds the checkout-origin allowlist from one server-configured HTTP(S) URL.
 * Non-web or invalid inputs return an empty set. Only MassageLab's public apex
 * and `www` hostnames expand to one another; request data never contributes a
 * trusted host.
 *
 * @param {string} expectedOrigin Canonical public site URL.
 * @returns {Set<string>} Exact configured origin plus its approved public alias.
 */
