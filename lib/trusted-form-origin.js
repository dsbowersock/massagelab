 ...
 * Contradictory Origin and Referer evidence fails closed. Metadata-free JSON
 * remains available to non-browser API clients, while forms without Origin or
 * Referer require exact same-origin Fetch Metadata and an allowlisted request
 * URL origin.
 *
 * @param {Request} request
...
export function isTrustedCheckoutFormOrigin(request, expectedOrigin) {
  ...
  if (!origin && !referer) {
    if (fetchSite) {
      return fetchSite === "same-origin"
        && allowedOrigins.has(requestUrlOrigin(request))
    }
  ...
  try {
    if (origin) evidenceOrigins.push(new URL(origin).origin)
  ...
  if (allowedOrigins.has(evidenceOrigin)) return true
  if (fetchSite !== "same-origin") return false

  const currentOrigin = requestUrlOrigin(request)
  return allowedOrigins.has(currentOrigin) && evidenceOrigin === currentOrigin
}
