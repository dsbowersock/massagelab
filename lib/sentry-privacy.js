...
const DISALLOWED_OPERATIONAL_CODE_PATTERN = /^(?:user|account|visitor|session|device|patient|select|insert|update|delete|drop|alter|create)(?:[._:-]|$)/i
...
/**
 * Accepts SDK operation/origin codes while rejecting identity-shaped,
 * query-like, URL-like, SQL-like, and freeform strings.
 *
 * @param {unknown} value
 */
function sanitizeOperationalCode(value) {
  ...
  return SAFE_OPERATIONAL_CODE_PATTERN.test(code)
    && !DISALLOWED_OPERATIONAL_CODE_PATTERN.test(code)
    ? code
    : undefined
}
...
function sanitizeTraceFields(value) {
  ...
  const parentSpanId = typeof value.parent_span_id === "string" && SAFE_SPAN_ID_PATTERN.test(value.parent_span_id)
    ...
    : undefined
  const op = sanitizeOperationalCode(value.op)
  const status = typeof value.status === "string" && SAFE_TRACE_STATUSES.has(value.status)
    ? value.status
    : undefined
  const origin = sanitizeOperationalCode(value.origin)
...
function sanitizePrimitiveEntries(value, allowedKeys) {
  ...
  for (const [key, entryValue] of Object.entries(value)) {
    ...
    if (key === "sentry.op" || key === "sentry.origin") {
      const code = sanitizeOperationalCode(entryValue)
...
export function sanitizeSentryOperation(value) {
  ...
  return sanitizeOperationalCode(source) ?? FILTERED_VALUE
}
