...
function scrubDiagnosticMessages(event) {
  // Exception messages can be constructed from user-entered content, so filter
  // them completely instead of trying to detect every possible clinical phrase.
  if (typeof event.message === "string") {
    event.message = FILTERED_VALUE
  }

  const logentry = event.logentry
  if (isRecord(logentry)) {
    if (typeof logentry.message === "string") {
      logentry.message = FILTERED_VALUE
    }

    if (typeof logentry.formatted === "string") {
      logentry.formatted = FILTERED_VALUE
    }
  }
  ...
  for (const exceptionValue of exception.values) {
    if (isRecord(exceptionValue) && typeof exceptionValue.value === "string") {
      exceptionValue.value = FILTERED_VALUE
    }
  }
}
