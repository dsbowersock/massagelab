 ...
 * }>} slots
 */
function buildAvailabilityBands(slots) {
  ...
  for (const slot of sortedSlots) {
    ...
    if (!previous || slot.startMinutes > previous.endMinutes) {
      bands.push({
        id: `${slot.dateKey}-${slot.startMinutes}-${slot.endMinutes}`,
    ...
    previous.id = `${slot.dateKey}-${previous.startMinutes}-${previous.endMinutes}`
...
export function buildSequenceWeekGrid(options, timeZone, requestedWeekStartKey = "") {
  ...
  const weekStartKeys = [...new Set(slots.map((slot) => slot.weekStartKey))].sort()
  const selectedWeekStartKey = weekStartKeys.includes(requestedWeekStartKey)
    ? requestedWeekStartKey
    : ((weekStartKeys[0] ?? requestedWeekStartKey) || sequenceWeekStartKey(new Date(), timeZone))
  ...
  const days = Array.from({ length: 7 }, (_, index) => {
    ...
    return {
      ...
      slots: daySlots,
      bands: buildAvailabilityBands(daySlots),
    }
  })
