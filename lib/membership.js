...
/**
 * Full administrators receive every currently implemented non-PHI paid
 * capability without creating or impersonating a Stripe subscription.
 */
const ADMIN_FEATURES = Object.freeze([
  ...PAID_FEATURES,
  ...THERAPIST_DOCUMENTATION_FEATURES,
  ...THERAPIST_CALENDAR_FEATURES,
  ...TEAM_PRACTICE_CALENDAR_FEATURES,
])
/**
 ...
 *   featureAccess: Array<{ featureKey: string, sources: Array<{ source: "admin" | "membership" | "student" | "temporary", expiresAt: string | null }> }>,
...
export function buildEntitlements({
  ...
  const features = [...new Set([
    ...
    ...(adminAccess === true ? ADMIN_FEATURES : []),
  ...
  const featureDetails = features.map((key) => {
    ...
    // Legacy featureDetails exposes one strongest source. Non-expiring Admin
    // access outranks temporary expiry there, while featureAccess below keeps
    // every active source and sorts Admin first for additive provenance.
    if (adminAccess === true && ADMIN_FEATURES.includes(key)) {
      return { key, source: "ADMIN", expiresAt: null }
    }
  ...
  const featureAccess = features.map((featureKey) => ({
    ...
    sources: deduplicateFeatureSources([
      ...
      ...(adminAccess === true && ADMIN_FEATURES.includes(featureKey)
...
function compareFeatureSources(left, right) {
  const sourceOrder = { admin: 0, membership: 1, student: 2, temporary: 3 }
...
/**
 * Loads the complete, expiry-then-ID-sorted active temporary-access candidate
 * set for one user at one request time. The Prisma predicate mirrors the
 * append-only ledger's half-open `[startsAt, expiresAt)` interval and excludes
 * revocations; returned rows synthesize `revocation: null` for buildEntitlements.
 * The limit-plus-one `take` is a fail-closed safety sentinel, not display
 * truncation: unsupported keys, more than 100 rows for one feature, or more
 * than 500 rows total throw without returning a partial entitlement set.
 *
 * @param {{ temporaryFeatureGrant: { findMany: Function } }} prismaClient
 * @param {string} userId
 * @param {Date} now
 * @returns {Promise<Array<{id: string, featureKey: string, startsAt: Date, expiresAt: Date, revocation: null}>>}
 * @throws {Error} When persistence violates the supported key or volume bounds.
 */
...
/** Resolves uncached entitlements with verified Admin access kept separate from billing state. */
export async function getUserEntitlementState(prismaClient, userId, now = new Date()) {
  const [subscriptions, studentAccess, temporaryGrants, adminAccess] = await Promise.all([
    ...
    hasVerifiedAdminAccess(prismaClient, userId),
  ])

  return buildEntitlements({ adminAccess, subscriptions, studentAccess, temporaryGrants, now })
}

/** Loads the durable full-admin role without relying on session claims. */
async function hasVerifiedAdminAccess(prismaClient, userId) {
  const assignment = await prismaClient.userRole.findFirst({
    where: { userId, role: "ADMIN", status: "VERIFIED" },
    select: { id: true },
  })
...
/**
 * Builds the billing summary and effective features together. Verified Admin
 * may add non-PHI features but never changes paidLevel or Stripe evidence.
 */
export async function getUserMembershipSummary(prismaClient, userId, now = new Date()) {
  const [stripeCustomer, subscriptions, studentAccess, temporaryGrants, adminAccess] = await Promise.all([
  ...
  const entitlements = buildEntitlements({ adminAccess, subscriptions, studentAccess, temporaryGrants, now })
