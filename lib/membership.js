 ...
 */
export const TEMPORARY_ACCESS_FEATURE_KEYS = Object.freeze([
...
const TEMPORARY_ACCESS_FEATURE_KEY_SET = new Set(TEMPORARY_ACCESS_FEATURE_KEYS)
const TEMPORARY_ACCESS_PER_FEATURE_ACTIVE_LIMIT = 100
const TEMPORARY_ACCESS_TOTAL_ACTIVE_LIMIT = TEMPORARY_ACCESS_PER_FEATURE_ACTIVE_LIMIT * TEMPORARY_ACCESS_FEATURE_KEYS.length
...
export function buildEntitlements({
  ...
  const activeTemporaryGrants = safeTemporaryGrants.flatMap((grant) => {
    if (!grant || grant.revocation !== null || !TEMPORARY_ACCESS_FEATURE_KEY_SET.has(grant.featureKey)) {
      return []
    }
...
export async function loadActiveTemporaryGrants(prismaClient, userId, now) {
  const rows = await prismaClient.temporaryFeatureGrant.findMany({
    ...
    take: TEMPORARY_ACCESS_TOTAL_ACTIVE_LIMIT + 1,
  })
  if (!Array.isArray(rows) || rows.length > TEMPORARY_ACCESS_TOTAL_ACTIVE_LIMIT) {
  ...
  for (const grant of rows) {
    if (!TEMPORARY_ACCESS_FEATURE_KEY_SET.has(grant.featureKey)) {
    ...
    if (featureCount > TEMPORARY_ACCESS_PER_FEATURE_ACTIVE_LIMIT) {
      throw new Error(`Temporary access has more than ${TEMPORARY_ACCESS_PER_FEATURE_ACTIVE_LIMIT} active grants for one feature and cannot be resolved safely.`)
    }
