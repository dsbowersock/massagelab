// @ts-check

import { normalizeSupporterRoadmapInterests } from "./onboarding-preferences.js"
import {
  normalizeChimerBackgroundVisualPreferences,
  sanitizeChimerSettings,
} from "./chimer-timer.js"

export const USER_PREFERENCES_VERSION = 1

export const LOCAL_PREFERENCE_KEYS = Object.freeze({
  appSettings: "massage-lab-settings",
  therapistSettings: "massage-lab-therapist-settings",
  chimerSettings: "massagelab-chimer-settings",
  anatomimeSettings: "massagelab-anatomime-settings",
  notePreferences: "massagelab-note-preferences",
  calendarPreferences: "massagelab-calendar-preferences",
})

/**
 * Resolves roadmap-interest state after an optimistic account-preference save.
 * Failed writes restore the last persisted selection, while successful writes
 * adopt the server-sanitized response.
 *
 * @param {{
 *   previousInterests?: unknown,
 *   responseInterests?: unknown,
 *   submittedInterests?: unknown,
 *   saveSucceeded?: boolean,
 * }} [input]
 * @returns {string[]}
 */
export function resolveSupporterRoadmapInterestsAfterSave({
  previousInterests,
  responseInterests,
  submittedInterests,
  saveSucceeded = false,
} = {}) {
  const successfulInterests = Array.isArray(responseInterests)
    ? responseInterests
    : submittedInterests ?? previousInterests

  return normalizeSupporterRoadmapInterests(
    saveSucceeded ? successfulInterests : previousInterests,
  )
}

const FORBIDDEN_SYNC_KEYS = new Set([
  "clientName",
  "clientDob",
  "clientDateOfBirth",
  "dateOfBirth",
  "dob",
  "intakeContent",
  "intakeDraft",
  "incidentJournal",
  "journalEntries",
  "movementData",
  "notes",
  "painJournal",
  "romDraft",
  "romMeasurements",
  "sensationJournal",
  "soapContent",
  "soapDraft",
  "treatmentDetails",
  "activityEntries",
  "bodySensationEntries",
  "clientWellnessEntries",
  "clientWellnessReminderSchedules",
  "emotionEntries",
  "sleepEntries",
  "wellnessEntries",
  "wellnessJournal",
  "wellnessReminderSchedules",
  "wellnessSummary",
])

/**
 * @param {unknown} user
 */
export function canSyncAccountPreferences(user) {
  const account = objectOrEmpty(user)
  return typeof account.id === "string" && account.id.trim().length > 0
}

/**
 * @param {unknown} session
 */
export function canSyncAccountPreferencesFromSession(session) {
  const sessionPayload = objectOrEmpty(session)
  return canSyncAccountPreferences(sessionPayload.user)
}

/**
 * @param {unknown} value
 * @returns {Record<string, unknown>}
 */
function objectOrEmpty(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {}
  }

  return /** @type {Record<string, unknown>} */ (value)
}

/**
 * Sorts object keys recursively while preserving array order and scalar values.
 *
 * @param {unknown} value
 * @returns {unknown}
 */
function stablePreferenceValue(value) {
  if (Array.isArray(value)) {
    return value.map(stablePreferenceValue)
  }
  if (!value || typeof value !== "object") {
    return value
  }
  const record = /** @type {Record<string, unknown>} */ (value)
  return Object.fromEntries(
    Object.keys(record)
      .sort()
      .map((key) => [key, stablePreferenceValue(record[key])]),
  )
}

/**
 * @param {unknown} value
 * @returns {unknown}
 */
export function removeForbiddenPreferenceFields(value) {
  if (Array.isArray(value)) {
    return value.map(removeForbiddenPreferenceFields)
  }

  if (!value || typeof value !== "object") {
    return value
  }

  return Object.fromEntries(
    Object.entries(/** @type {Record<string, unknown>} */ (value))
      .filter(([key]) => !FORBIDDEN_SYNC_KEYS.has(key))
      .map(([key, nestedValue]) => [key, removeForbiddenPreferenceFields(nestedValue)]),
  )
}

/**
 * Preserves ordinary app settings while constraining roadmap interests to the
 * approved categorical values before they can reach UserPreference.appSettings.
 *
 * @param {unknown} value
 * @returns {Record<string, unknown>}
 */
function sanitizeAppSettings(value) {
  const settings = objectOrEmpty(removeForbiddenPreferenceFields(value))

  if (!("supporterRoadmapInterests" in settings)) {
    return settings
  }

  return {
    ...settings,
    supporterRoadmapInterests: normalizeSupporterRoadmapInterests(settings.supporterRoadmapInterests),
  }
}

/**
 * @param {unknown} value
 * @param {{ backgroundPreferenceOptions?: Record<string, unknown> }} [options]
 * @returns {Record<string, unknown>}
 */
function sanitizeChimerPreferenceSnapshot(value, options = {}) {
  const settings = objectOrEmpty(removeForbiddenPreferenceFields(value))
  if (!("backgroundVisualPreferences" in settings)) {
    return settings
  }
  return {
    ...settings,
    backgroundVisualPreferences: normalizeChimerBackgroundVisualPreferences(
      settings.backgroundVisualPreferences,
      options.backgroundPreferenceOptions,
    ),
  }
}

/**
 * @param {Partial<Record<keyof typeof LOCAL_PREFERENCE_KEYS, unknown>>} snapshot
 * @param {{ backgroundPreferenceOptions?: Record<string, unknown> }} [options]
 */
export function buildUserPreferencePayload(snapshot = {}, options = {}) {
  return {
    version: USER_PREFERENCES_VERSION,
    app_settings: sanitizeAppSettings(snapshot.appSettings),
    chimer_settings: sanitizeChimerPreferenceSnapshot(snapshot.chimerSettings, options),
    anatomime_settings: objectOrEmpty(removeForbiddenPreferenceFields(snapshot.anatomimeSettings)),
    note_preferences: objectOrEmpty(removeForbiddenPreferenceFields(snapshot.notePreferences)),
    calendar_preferences: objectOrEmpty(removeForbiddenPreferenceFields(snapshot.calendarPreferences)),
  }
}

/**
 * Serializes preference writes and collapses not-yet-started work to the newest
 * complete snapshot. This prevents an older HTTP request from committing after
 * a newer local intent while retaining completion identity for UI state.
 *
 * @param {{
 *   send: (request: { requestBody: string | null, requestId: number, ownerKey?: string | null }) => Promise<boolean>,
 *   onComplete?: (
 *     request: { requestBody: string | null, requestId: number, ownerKey?: string | null },
 *     succeeded: boolean,
 *   ) => void,
 * }} dependencies
 */
export function createSerializedPreferenceWriter({
  send,
  onComplete = () => undefined,
}) {
  /** @type {{ requestBody: string | null, requestId: number, ownerKey?: string | null } | null} */
  let activeRequest = null
  /** @type {{ requestBody: string | null, requestId: number, ownerKey?: string | null } | null} */
  let queuedRequest = null
  /** @type {Array<() => void>} */
  let idleResolvers = []

  const resolveIdle = () => {
    if (activeRequest || queuedRequest) {
      return
    }
    const resolvers = idleResolvers
    idleResolvers = []
    for (const resolve of resolvers) {
      resolve()
    }
  }

  const drain = async () => {
    if (activeRequest) {
      return
    }

    while (queuedRequest) {
      const request = queuedRequest
      queuedRequest = null
      activeRequest = request
      let succeeded = false
      try {
        succeeded = await send(request)
      } catch {
        succeeded = false
      }
      try {
        onComplete(request, succeeded)
      } catch {
        // Completion observers must not wedge the serialized transport. A
        // later request still drains and whenIdle still resolves.
      } finally {
        activeRequest = null
      }
    }

    resolveIdle()
  }

  return {
    /**
     * Supersedes only queued work. An in-flight request finishes before the
     * newest body starts, preserving server commit order.
     *
     * @param {{ requestBody: string | null, requestId: number, ownerKey?: string | null }} request
     */
    enqueue(request) {
      if (!request.requestBody) {
        return
      }
      queuedRequest = request
      void drain()
    },
    whenIdle() {
      if (!activeRequest && !queuedRequest) {
        return Promise.resolve()
      }
      return new Promise((resolve) => {
        idleResolvers.push(() => resolve(undefined))
      })
    },
  }
}

/**
 * Serializes app-settings subpatches for one account shell and coalesces queued
 * fields with explicit shallow-merge semantics. A Settings snapshot and a
 * Music visualizer snapshot therefore cannot overwrite each other by racing
 * two server-side read/merge/write transactions.
 * Exactly one transport is active globally; owner keys partition queued
 * coalescing only and do not create concurrent transport lanes.
 *
 * @param {{
 *   send: (request: { ownerKey: string, patch: Record<string, unknown> }) => Promise<boolean>,
 * }} dependencies
 */
export function createSerializedAppSettingsPatchWriter({ send }) {
  /** @type {{ ownerKey: string, patch: Record<string, unknown>, resolve: Array<(value: boolean) => void> } | null} */
  let activeBatch = null
  /** @type {Map<string, { ownerKey: string, patch: Record<string, unknown>, resolve: Array<(value: boolean) => void> }>} */
  const queuedBatches = new Map()
  /** @type {string[]} */
  const queuedOwnerKeys = []
  let disposed = false

  const drain = async () => {
    if (activeBatch) return

    while (queuedOwnerKeys.length > 0) {
      const ownerKey = queuedOwnerKeys.shift()
      const batch = ownerKey ? queuedBatches.get(ownerKey) : null
      if (!batch) continue
      queuedBatches.delete(batch.ownerKey)
      activeBatch = batch
      let succeeded = false
      try {
        succeeded = await send({ ownerKey: batch.ownerKey, patch: batch.patch })
      } catch {
        succeeded = false
      }
      for (const resolve of batch.resolve) resolve(succeeded)
      activeBatch = null
    }
  }

  return {
    /**
     * @param {{ ownerKey: string, patch: Record<string, unknown> }} request
     * @returns {Promise<boolean>}
     */
    enqueue(request) {
      if (disposed || !request.ownerKey || !request.patch || typeof request.patch !== "object") {
        return Promise.resolve(false)
      }

      return new Promise((resolve) => {
        const queuedBatch = queuedBatches.get(request.ownerKey)
        if (queuedBatch) {
          queuedBatch.patch = { ...queuedBatch.patch, ...request.patch }
          queuedBatch.resolve.push(resolve)
        } else {
          queuedBatches.set(request.ownerKey, {
            ownerKey: request.ownerKey,
            patch: { ...request.patch },
            resolve: [resolve],
          })
          queuedOwnerKeys.push(request.ownerKey)
        }
        void drain()
      })
    },
    /**
     * Retains the active transport's real result, rejects queued and future
     * writes with false, and does not cancel the active request. The caller
     * owns any transport-level cancellation or abort signal.
     */
    dispose() {
      disposed = true
      for (const queuedBatch of queuedBatches.values()) {
        for (const settle of queuedBatch.resolve) settle(false)
      }
      queuedBatches.clear()
      queuedOwnerKeys.length = 0
    },
  }
}

/** Retains the established Chimer-specific API over the shared writer owner. */
export const createSerializedChimerPreferenceWriter = createSerializedPreferenceWriter

/**
 * Keeps every Chimer preference-write trigger on the same serialized
 * transport while exposing action-specific entry points for integration code.
 *
 * @param {{ enqueue: (request: { requestBody: string | null, requestId: number }) => void } | null | undefined} writer
 */
export function createChimerPreferenceSyncRouter(writer) {
  /** @param {{ requestBody: string | null, requestId: number }} request */
  const enqueue = (request) => writer?.enqueue(request)

  return {
    automatic: enqueue,
    visualApply: enqueue,
    visualRetry: enqueue,
  }
}

/**
 * Freezes one locally applied, fully sanitized Chimer payload as a request
 * body so a failed cloud write can retry byte-for-byte.
 *
 * @param {unknown} chimerSettings
 * @param {{
 *   backgroundPreferenceOptions?: Record<string, unknown>,
 *   requestId?: number,
 * }} [options]
 */
export function createChimerPreferenceSyncRequest(chimerSettings, options = {}) {
  const safeSettings = sanitizeChimerSettings(
    objectOrEmpty(removeForbiddenPreferenceFields(chimerSettings)),
    { backgroundPreferenceOptions: options.backgroundPreferenceOptions },
  )
  return {
    status: "pending",
    requestBody: JSON.stringify({ chimerSettings: safeSettings }),
    requestId: normalizePreferenceRequestId(options.requestId),
    sanitizedSettings: safeSettings,
  }
}

/**
 * Compares normalized Chimer snapshots structurally. Persisted JSON object key
 * order is not meaningful, while array order remains part of the contract.
 *
 * @param {unknown} left
 * @param {unknown} right
 * @param {{ backgroundPreferenceOptions?: Record<string, unknown> }} [options]
 */
export function areChimerPreferenceSnapshotsEqual(left, right, options = {}) {
  const normalizationOptions = {
    backgroundPreferenceOptions: options.backgroundPreferenceOptions,
  }
  const normalizedLeft = sanitizeChimerSettings(
    objectOrEmpty(left),
    normalizationOptions,
  )
  const normalizedRight = sanitizeChimerSettings(
    objectOrEmpty(right),
    normalizationOptions,
  )
  return JSON.stringify(stablePreferenceValue(normalizedLeft))
    === JSON.stringify(stablePreferenceValue(normalizedRight))
}

/**
 * Adopts the server-authoritative Chimer snapshot returned by an initial
 * account seed. A malformed success response must not enable synchronization,
 * because the server may have sanitized the submitted device snapshot against
 * access that changed after the preceding GET.
 *
 * @param {unknown} responseBody
 * @param {{ backgroundPreferenceOptions?: Record<string, unknown> }} [options]
 * @returns {{
 *   settings: Record<string, unknown>,
 *   featureKeys: string[],
 *   premiumBackgroundAccessSource: "subscription" | "temporary" | "admin" | null,
 *   ownedBackgroundIds: string[],
 * } | null}
 */
export function resolveChimerPreferenceSeedResult(responseBody, options = {}) {
  const response = objectOrEmpty(responseBody)
  if (
    response.accessAuthoritative !== true
    || !response.chimerSettings
    || typeof response.chimerSettings !== "object"
    || Array.isArray(response.chimerSettings)
  ) {
    return null
  }

  return {
    settings: sanitizeChimerSettings(response.chimerSettings, {
      backgroundPreferenceOptions: options.backgroundPreferenceOptions,
    }),
    featureKeys: Array.isArray(response.features)
      ? [...new Set(response.features.filter((feature) => typeof feature === "string"))]
      : [],
    premiumBackgroundAccessSource: normalizePremiumBackgroundAccessSource(
      response.premiumBackgroundAccessSource,
    ),
    ownedBackgroundIds: Array.isArray(response.ownedBackgroundIds)
      ? [...new Set(response.ownedBackgroundIds.filter((id) => typeof id === "string"))]
      : [],
  }
}

/**
 * Keeps only the three provenance values the Chimer carousel can present.
 * @param {unknown} value
 */
export function normalizePremiumBackgroundAccessSource(value) {
  return value === "subscription" || value === "temporary" || value === "admin" ? value : null
}

/**
 * Confirms that a successful preference response persisted the exact submitted
 * Chimer snapshot. A server-authoritative access change may sanitize the write;
 * treating that mismatch as stale keeps Retry available instead of silently
 * leaving the local snapshot out of sync with the account.
 *
 * @param {unknown} requestBody
 * @param {unknown} responseBody
 * @param {{ backgroundPreferenceOptions?: Record<string, unknown> }} [options]
 */
export function doesChimerPreferenceWriteResponseMatch(
  requestBody,
  responseBody,
  options = {},
) {
  let requestPayload
  try {
    requestPayload = typeof requestBody === "string" ? JSON.parse(requestBody) : null
  } catch {
    return false
  }
  if (
    !requestPayload
    || typeof requestPayload !== "object"
    || Array.isArray(requestPayload)
    || !responseBody
    || typeof responseBody !== "object"
    || Array.isArray(responseBody)
    || !("chimerSettings" in responseBody)
  ) {
    return false
  }

  return areChimerPreferenceSnapshotsEqual(
    requestPayload.chimerSettings,
    responseBody.chimerSettings,
    options,
  )
}

/**
 * Starts a new attempt for the exact latest failed locally committed body.
 *
 * @param {{ requestBody?: unknown } | null | undefined} request
 * @param {number} requestId
 */
export function createChimerPreferenceSyncRetry(request, requestId) {
  const requestBody = typeof request?.requestBody === "string" ? request.requestBody : null
  return {
    status: requestBody ? "pending" : "local",
    requestBody,
    requestId: normalizePreferenceRequestId(requestId),
  }
}

/**
 * Applies a completion only when it belongs to the latest pending attempt.
 * Late results return the current state unchanged.
 *
 * @param {{ requestBody?: unknown, requestId?: unknown }} currentRequest
 * @param {{ requestBody?: unknown, requestId?: unknown }} completedRequest
 * @param {boolean} saveSucceeded
 */
export function resolveChimerPreferenceSyncRequest(currentRequest, completedRequest, saveSucceeded) {
  if (
    normalizePreferenceRequestId(currentRequest?.requestId)
      !== normalizePreferenceRequestId(completedRequest?.requestId)
    || currentRequest?.requestBody !== completedRequest?.requestBody
  ) {
    return currentRequest
  }
  if (saveSucceeded) {
    return {
      status: "synced",
      requestBody: null,
      requestId: normalizePreferenceRequestId(currentRequest.requestId),
    }
  }
  return {
    status: "stale",
    requestBody: typeof currentRequest?.requestBody === "string" ? currentRequest.requestBody : null,
    requestId: normalizePreferenceRequestId(currentRequest.requestId),
  }
}

/** @param {unknown} value */
function normalizePreferenceRequestId(value) {
  return typeof value === "number" && Number.isSafeInteger(value) && value >= 0 ? value : 0
}

/**
 * @param {unknown} therapistSettings
 */
export function buildTherapistProfilePayload(therapistSettings = {}) {
  const settings = objectOrEmpty(removeForbiddenPreferenceFields(therapistSettings))

  return {
    therapist_name: typeof settings.name === "string" ? settings.name : "",
    therapist_location: typeof settings.location === "string" ? settings.location : "",
    license_number: typeof settings.licenseNumber === "string" ? settings.licenseNumber : "",
    license_organization: typeof settings.licenseOrganization === "string" ? settings.licenseOrganization : "",
    npi_number: typeof settings.npiNumber === "string" ? settings.npiNumber : "",
  }
}

/**
 * @param {{
 *   cloudPreferences?: Record<string, unknown> | null
 *   localPreferences?: Record<string, unknown> | null
 * }} input
 */
export function choosePreferenceSource({ cloudPreferences, localPreferences }) {
  if (cloudPreferences && Object.keys(cloudPreferences).length > 0) {
    return {
      source: "cloud",
      preferences: cloudPreferences,
    }
  }

  return {
    source: "local",
    preferences: localPreferences ?? {},
  }
}
