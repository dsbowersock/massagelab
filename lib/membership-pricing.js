...
export const MEMBERSHIP_PLAN_DETAILS = Object.freeze({
  SUPPORTER: {
    ...
    description: "For users who want the current Chimer color upgrade and want to help keep MassageLab development moving without ads or data sales.",
    currentFeatures: [
  ...
  THERAPIST: {
    ...
    description: "For massage therapists using the current local-first professional tools while supporting the next careful build steps.",
    currentFeatures: [
      "Everything in Supporter",
      "Full solo calendar service catalog",
      "Appointment and class scheduling depth",
      "Local encrypted vault access for SOAP, intake, journal, and therapist ROM tools",
      "Therapist membership status on your account",
    ],
    roadmapNotes: [
      "Funds local transcription experiments, therapist-reviewed SOAP assistance, and usability work around professional records.",
      "Helps fund the legal, security, and operating review required before any hosted PHI storage is offered.",
    ],
  },
  PRACTICE: {
    name: "Team/Practice",
    eyebrow: "Team and clinic path",
    description: "For teams or practices using current team scheduling paths while supporting future compliant practice infrastructure.",
    currentFeatures: [
      "Everything in Therapist",
      "Multi-provider calendar team access",
      "Team/Practice membership status on your account",
    ],
    roadmapNotes: [
      "Reserved for practice management workflows",
      "Funds future managed sync planning, BAAs, audit controls, access controls, and practice-ready documentation infrastructure.",
    ],
  },
})
...
export async function getMembershipPricingCatalog({ env = process.env, stripeClient } = {}) {
  ...
  const priceEntries = PAID_MEMBERSHIP_LEVELS.flatMap((membershipLevel) => (
    BILLING_INTERVALS.map((interval) => {
      ...
      const key = `${membershipLevel}:${interval}`
