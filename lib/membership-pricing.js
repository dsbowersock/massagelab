...
/**
 * A long-lived display catalog is complete only when every public Supporter
 * slot retained its configured Price identity and projected the expected
 * recurring interval, amount, and currency from Stripe.
 */
function isCompleteCatalog(catalog) {
  ...
  return SUPPORTER_AMOUNT_CHOICES.every((expectedChoice) => {
    ...
    return BILLING_INTERVALS.every((interval) => {
      ...
      return typeof price?.priceId === "string"
 ...
 * share a catalog build, complete results live for five minutes, and degraded
 * results retry after a short interval. Clearing advances the generation so a
 * stale provider completion cannot replace a newer catalog.
 *
 * @param {{
      ...
      const loadPromise = buildMembershipPricingCatalog({ env, stripeClient })
        .then((catalog) => {
          if (loadGeneration === generation) {
            cachedValue = catalog
            expiresAt = now() + (
              isCompleteCatalog(catalog) ? successTtlMs : incompleteTtlMs
            )
          }
          return catalog
        })
