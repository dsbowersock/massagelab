...
/**
 * @param {unknown} value
 * @returns {value is "USER" | "STUDENT" | "LICENSED_THERAPIST" | "CLIENT" | "EDITOR" | "ANATOMY_ADMIN" | "ADMIN"}
 */
...
export function canManageAnatomyContent(roles) {
  const normalizedRoles = normalizeRoles(roles)
  return normalizedRoles.includes("ANATOMY_ADMIN") || normalizedRoles.includes("ADMIN")
}
...
export function buildAccountCapabilities(roleAssignments, options = {}) {
  ...
  // Either current paid feature key qualifies the account as having active
  // membership benefits; UI status must not depend on colors alone.
  const canUseChimerCustomColors = hasFeature(options.features, FEATURE_KEYS.chimerCustomColors)
  ...
  return {
    ...
    canUseLocalClinicalTools: hasFeature(options.features, FEATURE_KEYS.therapistDocumentationTools),
    canUseChimerCustomColors,
    canUsePremiumBackgrounds,
    hasActiveMembershipBenefits: canUseChimerCustomColors || canUsePremiumBackgrounds,
