...
/** Creates a normalized live-session recipe without starting audio. @param {{ id: string, name?: string }} options */
...
/**
 * Returns a normalized new recipe without mutating the input. Adding an
 * exclusive station, binaural, or isochronic layer replaces its predecessor.
 * A reviewed ambient concept that exposes one selected recording also replaces
 * its prior selection so changing that recording cannot stack two copies.
 * @param {AtmoShaperRecipe} recipe
 * @param {AtmoShaperLayer} layer
 */
export function addAtmoShaperLayer(recipe, layer) {
  const normalized = normalizeLayer(layer)
  const retained = recipe.layers.filter((candidate) => {
    if (ATMOSHAPER_EXCLUSIVE_KINDS.has(normalized.kind)) return candidate.kind !== normalized.kind
    if (!isSingleSelectionAmbientLayer(normalized)) return true
    return candidate.kind !== "ambient" || candidate.sourceId !== normalized.sourceId
  })
  return normalizeAtmoShaperRecipe({ ...recipe, layers: [...retained, normalized] })
}

/** A selected recording is configuration inside one ambient concept, not another mix layer. @param {AtmoShaperLayer} layer */
function isSingleSelectionAmbientLayer(layer) {
  return layer.kind === "ambient" && typeof layer.settings.selectedSourceId === "string"
}

/**
 * Returns a normalized new recipe without mutating the input. A missing layer
 * returns a normalized copy unchanged; layer kind changes remain invalid.
 * @param {AtmoShaperRecipe} recipe
...
export function updateAtmoShaperLayer(recipe, layerId, patch) {
  return normalizeAtmoShaperRecipe({
    ...
    layers: recipe.layers.map((layer) => (
      layer.id === layerId ? updateLayer(layer, patch) : layer
    )),
  })
}

/**
 * Validates structured-clone-safe recipe data and clamps audio parameters to
 * the documented runtime-safe bounds.
 * @param {unknown} recipe
 * @returns {AtmoShaperRecipe}
 */
export function normalizeAtmoShaperRecipe(recipe) {
  ...
  const exclusiveKinds = new Set()
  for (const layer of layers) {
    if (layerIds.has(layer.id)) {
      throw new Error(`Duplicate AtmoShaper layer id: ${layer.id}`)
    }
    layerIds.add(layer.id)
    if (ATMOSHAPER_EXCLUSIVE_KINDS.has(layer.kind)) {
      if (exclusiveKinds.has(layer.kind)) {
        throw new Error(`AtmoShaper recipe can contain only one exclusive ${layer.kind} layer`)
      }
      exclusiveKinds.add(layer.kind)
    }
  }
...
/**
 * Returns a normalized new recipe without mutating the input. A missing layer
 * returns a normalized copy unchanged.
 * @param {AtmoShaperRecipe} recipe
 * @param {string} layerId
 */
...
/**
 * Returns a normalized new recipe without mutating the input. A missing layer
 * returns a normalized copy unchanged after validating the target index.
 * @param {AtmoShaperRecipe} recipe
 * @param {string} layerId
...
/** Rejects kind changes so an update cannot silently replace a separate exclusive layer. @param {AtmoShaperLayer} layer @param {Partial<AtmoShaperLayer>} patch */
function updateLayer(layer, patch) {
  if (patch.kind !== undefined && patch.kind !== layer.kind) {
    throw new Error("AtmoShaper layer kind cannot change during an update")
  }
  return normalizeLayer({ ...layer, ...patch, id: layer.id, kind: layer.kind })
}
