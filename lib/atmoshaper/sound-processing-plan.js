...
async function assertExternalOutputRoot(repoRoot, outputRoot) {
  ...
  let canonicalRepo
  try {
    canonicalRepo = await realpath(resolve(repoRoot))
    if (!(await stat(canonicalRepo)).isDirectory()) throw new Error("not a directory")
  } catch {
    throw new Error("The repository or worktree root could not be resolved")
  }
  const lexicalRepo = resolve(repoRoot)
  if (isSameOrInside(lexicalRepo, lexicalOutput) || isSameOrInside(canonicalRepo, lexicalOutput)) {
  ...
  if (isSameOrInside(canonicalRepo, canonicalPath)) {
    throw new Error("Sound processing output root must be outside the repository or worktree")
  }
}
