 ...
 *
 * @param {{ createAdapter: (layer: AtmoShaperLayer, isCurrent: () => boolean) => AtmoShaperLayerHandle | Promise<AtmoShaperLayerHandle>, onSnapshot?: (snapshot: { status: string, recipe: AtmoShaperRecipe | null, layers: Record<string, AtmoShaperLayerState>, activeLayers: Record<string, AtmoShaperLayer>, preview: AtmoShaperPreviewState | null }) => void }} options
 */
export function createAtmoShaperMixController({ createAdapter, onSnapshot = () => undefined }) {
  ...
  async function startPreview(nextLayer) {
    ...
    let stagedHandle
    try {
      const preparedHandle = await createAdapter(nextLayer, () => lease === previewRequestId && !disposed)
