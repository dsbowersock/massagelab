...
/**
 * Creates encoded sidecar descriptors for every WAV payload in an existing hosted
...
function printPlans(webAudioPlans) {
  ...
  const formatId = webAudioPlans[0]?.format.id ?? "sidecar"
  console.log(`Generative.fm ${formatId} R2 upload plan: ${webAudioPlans.length} pieces, ${totalObjects} objects`)
...
function printUsage() {
  ...
  console.error("  npm run atmosphere:samples:generative:web-audio:r2:upload -- <audio-sample-root> [--dry-run] [--piece <piece-id>] [--format opus|aac|mp3] [--public-base-url <url>] [--object-prefix-base <prefix>]")
}
